 ...
 * @version 2013-11-20
 */
public class KonkaMobileSailDataSearchForPriceAction extends BaseAction {
	...
	public ActionForward list(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (StringUtils.isBlank(chart_type)) {
			dynaBean.set("chart_type", "1");// 默认结算量占比
		}
		String dept_id_in = "";
	...
	public ActionForward chart(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		DynaBean dynaBean = (DynaBean) form;
		String search_first_flag = (String) dynaBean.get("search_first_flag");// 查询标识
		String chart_type = (String) dynaBean.get("chart_type");// 图形报表x轴类别
		...
		if (StringUtils.isNotBlank(search_first_flag) && "1".equals(search_first_flag)) {
			...
			if (null != list && list.size() > 0) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					String label = obj[0] == null ? "" : obj[0].toString();
					...
					if ("1".equals(label)) {
						...
						label = "5000-6999";
					} else if ("7".equals(label)) {
						label = "7000-9999";
					} else if ("8".equals(label)) {
						label = "10000-14999";
					} else if ("9".equals(label)) {
						label = "≥15000";
					}
					baseChart.setCategory_label(label);
					baseChartList.add(baseChart);
					if (StringUtils.isNotBlank(chart_type) && "2".equals(chart_type)) {
						// 零售量占比
						BaseChart baseChart1 = new BaseChart();
						baseChart1.setValue(obj[6] == null ? "0" : (obj[6].equals("-") ? "0" : obj[6].toString()));
						baseChartList_1.add(baseChart1);
					} else {
						// 结算量占比
						BaseChart baseChart1 = new BaseChart();
						baseChart1.setValue(obj[3] == null ? "0" : (obj[3].equals("-") ? "0" : obj[3].toString()));
						baseChartList_1.add(baseChart1);
					}
				}
				if (StringUtils.isNotBlank(chart_type) && "2".equals(chart_type)) {
					MsLineChart msLineChart1 = new MsLineChart();
					msLineChart1.setSeries_name("零售量占比");
					...
					MsLineChart msLineChart1 = new MsLineChart();
					msLineChart1.setSeries_name("结算量占比");
			...
			String caption = "结算量占比";
			if (StringUtils.isNotBlank(chart_type) && "2".equals(chart_type)) {
				caption = "零售量占比";
			}
			model.put("caption", caption);
			render(response, getFacade().getTemplateService().getContent("chart/MSBar3D_R.ftl", model),
	...
	public List<?> getKonkaMobileSailDataList(VADetailsOfSalesData entity) {
		...
		String sql = " SELECT a3.* FROM (SELECT decode (a2.price_type,NULL,b2.price_type,a2.price_type) AS price_type, ";
		sql += "              round(value (b2.R3_PRICE, 0)/10000,2) AS R3_PRICE,value (b2.R3_NUM, 0) AS R3_NUM,value (b2.R3_SCALE, '0') AS R3_SCALE, ";
		sql += "              round(value (a2.ALL_PRICE, 0)/10000,2) AS ALL_PRICE,value (a2.ALL_NUM, 0) AS ALL_NUM,value (a2.SALE_SCALE, '0') AS SALE_SCALE ";
		sql += "       FROM    (SELECT a.price_type,a.all_price,a.all_num,decode (to_char (b.all_num),0,'-',to_char(round (a.all_num * 100 / b.all_num, 2))) AS sale_scale ";
		sql += "                FROM    (SELECT c1.price_type,value (sum (c1.ALL_PRICE), 0) AS all_price,value (sum (c1.ALL_NUM), 0) AS all_num,1 AS flag ";
		sql += "                         FROM (SELECT CASE WHEN value (t.SINGLE_PRICE,0) >= 0 AND value (t.SINGLE_PRICE,0) <= 999 THEN 1 ";
		sql += "                                           WHEN value (t.SINGLE_PRICE,0) >= 1000 AND value (t.SINGLE_PRICE,0) <= 1999 THEN 2 ";
		sql += "                                           WHEN value (t.SINGLE_PRICE,0) >= 2000 AND value (t.SINGLE_PRICE,0) <= 2999 THEN 3 ";
		sql += "                                           WHEN value (t.SINGLE_PRICE,0) >= 3000 AND value (t.SINGLE_PRICE,0) <= 3999 THEN 4 ";
		sql += "                                           WHEN value (t.SINGLE_PRICE,0) >= 4000 AND value (t.SINGLE_PRICE,0) <= 4999 THEN 5 ";
		sql += "                                           WHEN value (t.SINGLE_PRICE,0) >= 5000 AND value (t.SINGLE_PRICE,0) <= 6999 THEN 6 ";
		sql += "                                           WHEN value (t.SINGLE_PRICE,0) >= 7000 AND value (t.SINGLE_PRICE,0) <= 9999 THEN 7 ";
		sql += "                                           WHEN value (t.SINGLE_PRICE,0) >= 10000 AND value (t.SINGLE_PRICE,0) <= 14999 THEN 8 ";
		sql += "                                           WHEN value (t.SINGLE_PRICE,0) >= 15000 THEN 9 END AS price_type, ";
		sql += "                                        t.ALL_PRICE AS all_price,t.NUM AS all_num ";
		...
		sql += "                              ) c1  WHERE 1 = 1 GROUP BY c1.PRICE_TYPE) a ";
		sql += "                         LEFT JOIN (SELECT 1 AS flag,value (sum (t.ALL_PRICE), 0) AS all_price, value (sum (t.NUM), 0) AS all_num ";
		sql += "                                    FROM V_A_DETAILS_OF_SALES_DATA t WHERE 1 = 1 ";
		...
		sql += "                                   ) b ON a.flag = b.flag) a2 ";
		sql += "          FULL OUTER JOIN (SELECT a1.price_type,a1.r3_price,a1.r3_num,decode (to_char (b1.r3_num),0,'-',to_char(round (a1.r3_num * 100 / b1.r3_num, 2))) AS r3_scale ";
		sql += "                           FROM    (SELECT c1.price_type,value (sum (c1.R3_PRICE), 0) AS r3_price,value (sum (c1.R3_NUM), 0) AS r3_num,1 AS flag ";
		sql += "                                    FROM (SELECT CASE WHEN value (t1.COLUMN_13, 0) >= 0 AND value (t1.COLUMN_13, 0) <= 999 THEN 1 ";
		sql += "                                                      WHEN value (t1.COLUMN_13, 0) >= 1000 AND value (t1.COLUMN_13, 0) <= 1999 THEN 2 ";
		sql += "                                                      WHEN value (t1.COLUMN_13, 0) >= 2000 AND value (t1.COLUMN_13, 0) <= 2999 THEN 3 ";
		sql += "                                                      WHEN value (t1.COLUMN_13, 0) >= 3000 AND value (t1.COLUMN_13, 0) <= 3999 THEN 4 ";
		sql += "                                                      WHEN value (t1.COLUMN_13, 0) >= 4000 AND value (t1.COLUMN_13, 0) <= 4999 THEN 5 ";
		sql += "                                                      WHEN value (t1.COLUMN_13, 0) >= 5000 AND value (t1.COLUMN_13, 0) <= 6999 THEN 6 ";
		sql += "                                                      WHEN value (t1.COLUMN_13, 0) >= 7000 AND value (t1.COLUMN_13, 0) <= 9999 THEN 7 ";
		sql += "                                                      WHEN value (t1.COLUMN_13, 0) >= 10000 AND value (t1.COLUMN_13, 0) <= 14999 THEN 8 ";
		sql += "                                                      WHEN value (t1.COLUMN_13, 0) >= 15000 THEN 9 END AS price_type, ";
		sql += "                                                  t1.column_12 AS r3_num, t1.column_14 AS r3_price ";
		sql += "                                          FROM CHANNEL_DATA_IMPORT t1 LEFT JOIN MV_ORG_OF_CUSTOMER t ON t1.column_1 = t.R3_CODE LEFT JOIN KONKA_PE_PD_MODEL t2 ON t1.column_11 = t2.MD_NAME ";
		sql += "                                          WHERE 1 = 1  and t1.column_12<>0 ";
		...
		sql += "                                          ) c1 WHERE 1 = 1 GROUP BY c1.PRICE_TYPE) a1 ";
		sql += "                                   LEFT JOIN (SELECT 1 AS flag,value (sum (t1.column_14), 0) AS r3_price,value (sum (t1.column_12), 0) AS r3_num ";
		sql += "                                              FROM CHANNEL_DATA_IMPORT t1 LEFT JOIN MV_ORG_OF_CUSTOMER t ON t1.column_1 = t.R3_CODE ";
		sql += "                                              LEFT JOIN KONKA_PE_PD_MODEL t2 ON t1.column_11 = t2.MD_NAME WHERE 1 = 1  and t1.column_12<>0  ";
		...
		sql += "                          ) b1 ON a1.flag = b1.flag) b2 ON a2.price_type = b2.price_type WHERE 1 = 1) a3 ORDER BY a3.price_type ASC ";
