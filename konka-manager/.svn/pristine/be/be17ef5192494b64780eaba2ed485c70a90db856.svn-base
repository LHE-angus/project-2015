 ...
 * @version 2013-11-22
 */
public class KonkaMobileSailDataSearchForPriceSizeAction extends BaseAction {
	...
	public List<?> getKonkaMobileSailDataList(VADetailsOfSalesData entity) {
		...
		String sql = " SELECT a3.* FROM (SELECT decode (a2.price_size, NULL, b2.price_size, a2.price_size) AS price_size, ";
		sql += "              value (b2.R3_PRICE, 0) AS R3_PRICE, value (b2.R3_NUM, 0) AS R3_NUM, value (b2.md_count, 0) AS r3_md_count, ";
		sql += "              value (b2.R3_SCALE, 0) AS R3_SCALE, value (a2.ALL_PRICE, 0) AS ALL_PRICE, value (a2.ALL_NUM, 0) AS ALL_NUM, ";
		sql += "              value (a2.md_count, 0) AS sale_md_count, value (a2.SALE_SCALE, 0) AS SALE_SCALE ";
		sql += "       FROM    (SELECT c2.*, (value(c2.md_count,0) - value(d2.md_count,0))  AS sale_scale ";
		sql += "                FROM    (SELECT d1.price_size, sum (d1.all_price) AS all_price, sum (d1.all_num) AS all_num, count (DISTINCT d1.MODEL_NAME) AS md_count ";
		sql += "                         FROM (SELECT CASE ";
		sql += "                                        WHEN c1.all_price >= 10000 THEN 1 ";
		sql += "                                        WHEN c1.all_price >= 5000 AND c1.all_price < 10000 THEN 2 ";
		sql += "                                        WHEN c1.all_price >= 2000 AND c1.all_price < 5000 THEN 3 ";
		sql += "                                        WHEN c1.all_price >= 1000 AND c1.all_price < 2000 THEN 4 ";
		sql += "                                        WHEN c1.all_price >= 500 AND c1.all_price < 1000 THEN 5 ";
		sql += "                                        WHEN c1.all_price >= 200 AND c1.all_price < 500 THEN 6 ";
		sql += "                                        WHEN c1.all_price >= 100 AND c1.all_price < 200 THEN 7 ";
		sql += "                                        WHEN c1.all_price >= 50 AND c1.all_price < 100 THEN 8 ";
		sql += "                                        WHEN c1.all_price < 50 THEN 9 ";
		sql += "                                      END AS price_size, c1.all_price, c1.all_num, c1.MODEL_NAME ";
		sql += "                               FROM (SELECT t.MODEL_NAME, round (value (sum (t.ALL_PRICE), 0) / 10000, 2) AS all_price, ";
		sql += "                                            value (sum (t.NUM), 0) AS all_num ";
		...
		sql += "         GROUP BY t.MODEL_NAME) c1) d1 GROUP BY d1.price_size) c2 ";
		sql += "            LEFT JOIN (SELECT d1.price_size, count (DISTINCT d1.MODEL_NAME) AS md_count ";
		sql += "                       FROM (SELECT CASE ";
		sql += "                                        WHEN c1.all_price >= 10000 THEN 1 ";
		sql += "                                        WHEN c1.all_price >= 5000 AND c1.all_price < 10000 THEN 2 ";
		sql += "                                        WHEN c1.all_price >= 2000 AND c1.all_price < 5000 THEN 3 ";
		...
		sql += "                                        WHEN c1.all_price >= 50 AND c1.all_price < 100 THEN 8 ";
		sql += "                                        WHEN c1.all_price < 50 THEN 9 ";
		sql += "                                    END AS price_size, c1.MODEL_NAME ";
		sql += "                             FROM (SELECT t.MODEL_NAME, round (value (sum (t.ALL_PRICE), 0) / 10000, 2) AS all_price ";
		sql += "                                   FROM V_A_DETAILS_OF_SALES_DATA t WHERE 1 = 1 ";
		...
		if (entity.getMap().get("date_begin") != null
			...
			sql += " and t.SALE_DATE >= to_date(?,'yyyy-MM-dd hh24:mi:ss') ";
			String year = String
					.valueOf(Integer.valueOf(entity.getMap().get("date_begin").toString().substring(0, 4)) - 1);
			array.add(year + entity.getMap().get("date_begin").toString().substring(4, 10) + " 00:00:00");
		}
		if (entity.getMap().get("date_end") != null
				&& StringUtils.isNotBlank(entity.getMap().get("date_end").toString())) {
			sql += " and t.SALE_DATE <= to_date(?,'yyyy-MM-dd hh24:mi:ss') ";
			String year = String
					.valueOf(Integer.valueOf(entity.getMap().get("date_end").toString().substring(0, 4)) - 1);
			array.add(year + entity.getMap().get("date_end").toString().substring(4, 10) + " 23:59:59");
		}
		if (entity.getIs_del() != null) {
			sql += " and t.IS_DEL = ? ";
			array.add(String.valueOf(entity.getIs_del()));
		...
		sql += "     GROUP BY t.MODEL_NAME) c1) d1 GROUP BY d1.price_size) d2 ON c2.price_size = d2.price_size) a2 ";
		sql += "     FULL OUTER JOIN (SELECT c2.*,(value(c2.md_count,0) - value(d2.md_count,0)) AS r3_scale ";
		sql += "                      FROM    (SELECT d1.price_size, sum (d1.r3_price) AS r3_price, sum (d1.r3_num) AS r3_num, ";
		sql += "                                      count (DISTINCT d1.MD_NAME) AS md_count ";
		sql += "                               FROM (SELECT CASE ";
		sql += "                                                WHEN c1.r3_price >= 10000 THEN 1 ";
		sql += "                                                WHEN c1.r3_price >= 5000 AND c1.r3_price < 10000 THEN 2 ";
		sql += "                                                WHEN c1.r3_price >= 2000 AND c1.r3_price < 5000 THEN 3 ";
		sql += "                                                WHEN c1.r3_price >= 1000 AND c1.r3_price < 2000 THEN 4 ";
		sql += "                                                WHEN c1.r3_price >= 500 AND c1.r3_price < 1000 THEN 5 ";
		sql += "                                                WHEN c1.r3_price >= 200 AND c1.r3_price < 500 THEN 6 ";
		sql += "                                                WHEN c1.r3_price >= 100 AND c1.r3_price < 200 THEN 7 ";
		sql += "                                                WHEN c1.r3_price >= 50 AND c1.r3_price < 100 THEN 8 ";
		sql += "                                                WHEN c1.r3_price < 50 THEN 9 ";
		sql += "                                            END AS price_size, c1.r3_price, c1.r3_num, c1.MD_NAME ";
		sql += "                                     FROM (SELECT t2.MD_NAME, value (sum (t1.column_12), 0) AS r3_num, ";
		sql += "                                                  round (value (sum (t1.column_14), 0) / 10000, 2) AS r3_price ";
		...
		sql += "   GROUP BY t2.MD_NAME) c1) d1 GROUP BY d1.price_size) c2 ";
		sql += "   LEFT JOIN (SELECT d1.price_size, count (DISTINCT d1.MD_NAME) AS md_count ";
		sql += "              FROM (SELECT CASE ";
		sql += "                               WHEN c1.r3_price >= 10000 THEN 1 ";
		sql += "                               WHEN c1.r3_price >= 5000 AND c1.r3_price < 10000 THEN 2 ";
		sql += "                               WHEN c1.r3_price >= 2000 AND c1.r3_price < 5000 THEN 3 ";
		...
		sql += "                               WHEN c1.r3_price >= 50 AND c1.r3_price < 100 THEN 8 ";
		sql += "                               WHEN c1.r3_price < 50 THEN 9 ";
		sql += "                           END AS price_size, c1.MD_NAME ";
		sql += "                    FROM (SELECT t2.MD_NAME, round (value (sum (t1.column_14),0) / 10000, 2) AS r3_price ";
		sql += "                          FROM CHANNEL_DATA_IMPORT t1 LEFT JOIN MV_ORG_OF_CUSTOMER t ON t1.column_1 = t.R3_CODE ";
		sql += "                          LEFT JOIN KONKA_PE_PD_MODEL t2 ON t1.column_11 = t2.MD_NAME WHERE 1 = 1   and t1.column_12 <>0 ";
		...
		sql += "   GROUP BY t2.MD_NAME) c1) d1 GROUP BY d1.price_size) d2 ON c2.price_size = d2.price_size) b2 ";
		sql += "   ON a2.price_size = b2.price_size WHERE 1 = 1) a3 ORDER BY a3.price_size ASC ";
