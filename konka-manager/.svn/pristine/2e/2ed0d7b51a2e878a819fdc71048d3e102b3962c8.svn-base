 ...
 <!--  <div class="rtabcont1">
    <fieldset style="border:1px solid #ccc; color:#666;padding-left:10px;">
      ...
      <ul style="margin-left:20px;">
        <li>1. 本页面可查询已审核的订单，支持下单日期最多跨度为90天的查询；</li>
        <li>2. 以下列表数据是根据您的数据级别可查询“<c:choose>
        ...
        <li>3. 以上4中职位查询分公司全部数据的审核状态为已审核不表示为当前人已审核，表示订单已有审核记录；</li>
        <li>4. 2013年8月9日14:10:51前的订单<a href="${ctx}/jxc/KonkaJxcOrderInfoView.do?mod_id=205400" style="color:#00F;text-decoration:underline;">去这里查询</a>；</li>
      </ul>
    </fieldset> 
  </div>-->
  <div class="rtabcont1">
    <html-el:form action="/admin/KonkaOrderSearch">
      ...
      <table width="100%" border="0" cellspacing="1" cellpadding="0" class="rtable1">
       <tr>
			...
			<td><html-el:text property="trade_index_like" size="20" maxlength="40" styleId="trade_index_like" /></td>
			<td align="right"><strong class="fb">提交日期：</strong></td>
			<td><html-el:text property="order_date_start" styleId="order_date_start" size="10" maxlength="10" readonly="true"
					onclick="new Calendar(2011, 2021, 0).show(this);" style="cursor:pointer;text-align:center;" title="点击选择日期" /> 至 <html-el:text property="order_date_end"
					styleId="order_date_end" size="10" maxlength="10" readonly="true" onclick="new Calendar(2011, 2021, 0).show(this);" style="cursor:pointer;text-align:center;"
					title="点击选择日期" /></td>
			<td align="right"><strong class="fb">分公司：</strong></td>
			<td colspan="3"><c:if test="${show_fgs and show_fgs_jb and show_fgs_gr}">
		...
		<tr>
			...
			<td><html-el:select property="kh_confirm_state" styleClass="webinput" styleId="kh_confirm_state">
					<html-el:option value="">-请选择-</html-el:option>
			...
			<td><html-el:select property="send_type" styleClass="webinput" styleId="send_type">
					<html-el:option value="">-请选择-</html-el:option>
			...
			<td><html-el:select property="or_audit_state" styleClass="webinput" styleId="or_audit_state">
					<html-el:option value="">-请选择-</html-el:option>
		...
		<tr>
			...
			<td><html-el:select property="customer_type_index" styleId="customer_type_index">
					<html-el:option value="">请选择...</html-el:option>
			...
			<td><html-el:select property="sync_state" styleClass="webinput" styleId="sync_state">
					<html-el:option value="">-请选择-</html-el:option>
		...
		<tr>
			...
			<td><html-el:text property="vbedl_like" styleClass="webinput" styleId="vbedl_like" maxlength="40" /></td>
			<td align="right"><strong class="fb">R3物流单号：</strong></td>
			<td><html-el:select property="vbedl_null" styleClass="webinput" styleId="vbedl_null">
					<html-el:option value="">-请选择-</html-el:option>
		...
		<tr>
			...
			<td><html-el:text property="r3_id" styleClass="webinput" title="不能超过18位数字" styleId="r3_id" maxlength="18" /></td>
			...
			<td><html-el:select property="is_change" styleClass="webinput" styleId="is_change">
					<html-el:option value="">-请选择-</html-el:option>
					<html-el:option value="0">未变更</html-el:option>
		...
		<tr>
			...
			<td><html-el:text property="sync_user" size="20" maxlength="40" styleId="sync_user" /></td>
			<td align="right"><strong class="fb">同步时间：</strong></td>
			<td><html-el:text property="sync_date_start" styleId="sync_date_start" size="10" maxlength="10" readonly="true" onclick="new Calendar(2011, 2021, 0).show(this);"
					style="cursor:pointer;text-align:center;" title="点击选择日期" /> 至 <html-el:text property="sync_date_end" styleId="sync_date_end" size="10" maxlength="10"
					readonly="true" onclick="new Calendar(2011, 2021, 0).show(this);" style="cursor:pointer;text-align:center;" title="点击选择日期" /></td>
		
		<td align="right"><strong class="fb">退货类型：</strong></td>
			<td><html-el:select property="return_type" styleClass="webinput" styleId="return_type">
					<html-el:option value="">-请选择-</html-el:option>
  ...
  <div class="rtabcont1" style="color: #465F6D;">&nbsp;&nbsp;注：本页订单商品数量为：${pageCount}&nbsp;台;商品总价为：￥${pageMoney}&nbsp;元;商品总折扣为：￥${pageDiscount}元。
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="rtable2">
      ...
      <tr class="tabtt1">
        ...
        <td width="9%" nowrap="nowrap" align="center">同步|打印</td>
        ...
        <td width="6%" nowrap="nowrap" align="center">订单变更</td>
        ...
        <td width="5%" nowrap="nowrap" align="center">创建时间</td>
        ...
        <tr>
           ...
           <td align="center" nowrap="nowrap">
	          <logic-el:match name="popedom" value="+16+">
		          <c:choose>
		              <c:when test="${fn:contains(popedom,'+16+') and ( role_id_eq_38 or role_id_eq_41 or role_id_eq_33 or role_id_eq_54 or role_id_eq_55 or role_id_eq_56 or role_id_eq_43 or role_id_eq_44 or role_id_eq_46) and cur.next_audit_role_id eq -1 and ((cur.sync_state eq 0  and empty cur.r3_id) or cur.is_change eq 1)  and (cur.kh_confirm_state eq 1 or cur.kh_confirm_state eq 0)  and cur.audit_state ne 4}">
		              <span style="cursor:pointer;" class="fblue" id="tongbu_${cur.id}" onclick="tongbu(${cur.id})">同步订单</span>|</c:when>
		              <c:otherwise><span style="color: gray;" title="提示：此订单暂不可同步，可能是：&#13;1、未被审核完成；&#13;2、被修改过但客户暂未确认；&#13;3、已被同步订单；&#13;4、您没有权限;&#13;5、订单被作废">同步订单</span>|</c:otherwise>
		          </c:choose>
		           <c:choose>
	            <c:when test="${cur.sync_state eq 1 and cur.is_sh eq 0 and (role_id_eq_30 or role_id_eq_57)}">
	            <span style="cursor:pointer;" class="fblue" id="tbwl_${cur.id}" onclick="tbwl(${cur.id})">同步物流</span>|
	            </c:when>
	            <c:otherwise>
	            ...
	            <span style="cursor:pointer;" class="fblue" onclick="window.showModalDialog('${ctx}/jxc/KonkaJxcOrderAudit.do?method=print&' + encodeURI('id=' + ${cur.id} +'&random=' + Math.random()), window, 'dialogWidth:900px;status:no;dialogHeight:580px')">打印</span>
	            <span style="cursor:pointer; " class="fblue" onclick="window.open('${ctx}/jxc/KonkaJxcOrderAudit.do?method=print&printType=a4&' + encodeURI('id=' + ${cur.id} +'&random=' + Math.random()), '订单打印', 'dialogWidth=900px,status=no,dialogHeight=580px,resizable=yes,scrollbars=yes')">A4打印</span>
          </td>
          ...
          <td align="center" nowrap="nowrap"><fmt:formatDate value="${cur.order_date}" pattern="yyyy-MM-dd hh:mm:ss"/></td>
          ...
          <td align="right" nowrap="nowrap"><span class="kz-price-12">
            <fmt:formatNumber value="${cur.money + cur.good_discount_price}" type="currency" />
            </span>
          </td>
 		  <td align="center" nowrap="nowrap">
 			  <c:choose>
          ...
          <td align="center" nowrap="nowrap">
          <c:choose>
               <c:when test="${cur.is_change eq 0}"><span style="color:grey;">未变更</span></c:when>
               <c:when test="${cur.is_change eq 1}"><span style="color:green;">变更未同步</span></c:when>
               <c:when test="${cur.is_change eq 2}"><span style="color:red;">变更并同步</span></c:when>
             </c:choose>
          </td>
          ...
          <span id="message_${cur.id}" onmouseover="show('${cur.id}')" onmouseout="hide('${cur.id}')")">${fn:split('未发送提醒,未变更,已发送提醒', ',')[cur.kh_confirm_state + 1]}
          <c:if test="${cur.kh_confirm_state eq 1 }">  
          ...
          <c:if test="${af.map.dept_type eq 1}"> 
            <c:if test="${cur.kh_confirm_state ne 0}">
	            <c:choose>
		           <c:when test="${(cur.map.messagecount <= 5) and (cur.is_delivered eq 0) and (cur.audit_state ne 4)}">
		           <input  type="button" value="发送提醒" onclick="sendmsg('${cur.id}','${cur.map.messagecount}')" />
		           </c:when>
		           <c:otherwise >
           ...
           <c:if test="${af.map.dept_type eq 2}"> 
            <c:choose>
	           <c:when test="${cur.kh_confirm_state ne 0 and cur.audit_state ne 4 and cur.map.messagecount <= 5 and cur.is_delivered eq 0}">
	           		<input  type="button" value="发送提醒" onclick="sendmsg('${cur.id}','${cur.map.messagecount}')" />
	           </c:when>
	           <c:otherwise >
	           </c:otherwise>
           ...
           <div id="msgshow_${cur.id}"  style="background-color:pink;display:none;position:absolute;left:500px;top:380px;z-index:99999">
	           <ul>
	            <c:forEach var="cur2" items="${cur.map.KonkaOrderInfoMessageSendList}" varStatus="vs2">
	           <li>接收人：${cur2.receiver_name}&emsp;手机号码：${cur2.receiver_phone}&emsp;时间：<fmt:formatDate value="${cur2.send_date}" pattern="yyyy年MM月dd日"/></li>
	           </c:forEach>
	           </ul>
           </div>
          </td>
          ...
          <td align="center" nowrap="nowrap">
          	<c:choose>
              <c:when test="${cur.audit_state eq 1 and  not empty cur.map.next_audit_role_name}">
     				<span style="cursor:pointer;" id="cuiban" class="fblue" onclick="sendmessageprocess(${cur.id});">催办</span>|</c:when>
              <c:otherwise><span style="color: gray;">催办</span>|</c:otherwise>
            </c:choose>
            
            <c:choose>
              <c:when test="${(role_id_eq_38 or role_id_eq_41 or role_id_eq_33 or role_id_eq_54 or role_id_eq_55 or role_id_eq_56 or role_id_eq_43 or role_id_eq_44 or role_id_eq_46) and (cur.audit_state eq 3) and empty cur.map.vbedl}"><span style="cursor:pointer;" class="fblue" onclick="doNeedMethod('是否作废此订单！', 'KonkaOrderSearch.do', 'cancel','id=${cur.id}&cust_id=${cur.cust_id}&r3_id=${cur.r3_id}&mod_id=${af.map.mod_id}&'+$('#bottomPageForm').serialize())">作废</span>|</c:when>
              <c:otherwise><span style="color: gray;" title="提示：此订单暂不可作废，可能是：&#13;1、未被审核完成；&#13;2、您不是产品会计；&#13;3、订单已被作废。">作废</span>|</c:otherwise>
            </c:choose>
            
            <c:choose>
              <c:when test="${(cur.add_user_id eq af.map.user_id) and (cur.audit_state eq 0)}"><span style="cursor:pointer;" class="fblue" onclick="doNeedMethod('是否删除此订单！', 'KonkaOrderSearch.do', 'delete','id=${cur.id}&mod_id=${af.map.mod_id}&'+$('#bottomPageForm').serialize())">删除</span>|</c:when>
              <c:otherwise><span style="color: gray;">删除</span>|</c:otherwise>
            </c:choose>
           <c:choose>
              <c:when test="${(fn:contains(popedom,'+16+') or role_id_eq_38 or role_id_eq_41 or role_id_eq_33 or role_id_eq_54 or role_id_eq_55 or role_id_eq_56 or role_id_eq_43 or role_id_eq_44 or role_id_eq_46) and cur.next_audit_role_id eq -1  and (cur.kh_confirm_state eq 1 or cur.kh_confirm_state eq 0)  and cur.audit_state ne 4 and empty cur.map.vbedl}"><span style="cursor:pointer;" class="fblue" id="biangeng_${cur.id}" onclick="doNeedMethod('是否变更此订单！', 'KonkaOrderChange.do', 'change','id=${cur.id}&mod_id=${af.map.mod_id}&'+$('#bottomPageForm').serialize())">变更</span>|</c:when>
        ...
        <tr>
          <td height="60" align="center">
            ...
            <script type="text/javascript">
            			var recordCount = ${af.map.pager.recordCount};
            			var pageSize =${af.map.pager.pageSize};
            			var currentPage =${af.map.pager.currentPage};
						var pager = new Pager(document.bottomPageForm, recordCount, pageSize, currentPage);
...
$(document).ready(function(){
	...
	//同步到sap生成61单号
	function tongbu(id){
				doNeedMethod(null, 'KonkaOrderSearch.do', 'odrerTb','id=' + id + '&is_th=1&mod_id=${af.map.mod_id}&'+$('#bottomPageForm').serialize());
	...
	//单笔订单获取最新的物流信息
	function tbwl(id){
		doNeedMethod(null, 'KonkaOrderSearch.do', 'orderWl','id=' + id + '&is_th=1&mod_id=${af.map.mod_id}&'+$('#bottomPageForm').serialize());
	}
