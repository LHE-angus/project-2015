 ...
 * @version 2013-09-07
 */
public class KonkaCategorySailDataAction extends BaseAction {
	...
	public ActionForward list(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		String p_index1 = (String) dynaBean.get("p_index1");// 行政区划省
		String p_index2 = (String) dynaBean.get("p_index2");// 行政区划市
		String p_index3 = (String) dynaBean.get("p_index3");// 行政区划县
		...
		if (peProdUser.getUser_type() == 0) {
			...
			if (null == konkaDept) {
				...
				/*
				 * if(konkaDept.getDept_type() != null &&
				 * konkaDept.getDept_type() ==4){
				 * request.setAttribute("l4_dept_name",
				 * konkaDept.getDept_name()); dynaBean.set("l4_dept_id",
				 * konkaDept.getDept_id().toString()); dynaBean.set("dept_type",
				 * "4"); } if(konkaDept.getDept_type() != null &&
				 * konkaDept.getDept_type() ==5){ dynaBean.set("l4_dept_id",
				 * konkaDept.getPar_id().toString()); KonkaDept parDept = new
				 * KonkaDept(); parDept.setDept_id(konkaDept.getPar_id());
				 * parDept =
				 * super.getFacade().getKonkaDeptService().getKonkaDept
				 * (parDept); dynaBean.set("l4_dept_name",
				 * parDept.getDept_name()); request.setAttribute("l5_dept_name",
				 * konkaDept.getDept_name()); dynaBean.set("l5_dept_id",
				 * konkaDept.getDept_id().toString()); dynaBean.set("dept_type",
				 * "5"); }
				 */
		...
		//行政区划
		String p_index="";
		if (StringUtils.isNotBlank(p_index1)) {
			BaseProvinceListFour p = new  BaseProvinceListFour();
			p.setP_index(new Long(p_index1));
			p = super.getFacade().getBaseProvinceListFourService().getBaseProvinceListFour(p);
			request.setAttribute("p_name1", p.getP_name());
			p_index=p_index1;
		} 
		if (StringUtils.isNotBlank(p_index2)) {
			BaseProvinceListFour p = new  BaseProvinceListFour();
			p.setP_index(new Long(p_index2));
			p = super.getFacade().getBaseProvinceListFourService().getBaseProvinceListFour(p);
			request.setAttribute("p_name2", p.getP_name());
			p_index=p_index2;
		} 
		if (StringUtils.isNotBlank(p_index3)) {
			BaseProvinceListFour p = new  BaseProvinceListFour();
			p.setP_index(new Long(p_index3));
			p = super.getFacade().getBaseProvinceListFourService().getBaseProvinceListFour(p);
			request.setAttribute("p_name3", p.getP_name());
			p_index=p_index3;			
		} 
		...
		if (StringUtils.isNotBlank(search_first_flag) && "1".equals(search_first_flag)) {
			...
			//查询数据 暂未实现
			//List<VADetailsOfSalesData> entityList = super.getFacade().getVADetailsOfSalesDataService().getVADetailsOfSalesDataList(entity);
