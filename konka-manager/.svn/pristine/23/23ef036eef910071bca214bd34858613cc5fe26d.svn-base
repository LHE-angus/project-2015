...
<div class="maincont">
  ...
  <div class="member_right padbot45">
    <div class="position"><a href="${ctx }/member/Index.do">首页</a> &gt; <a href="${ctx }/member/center/Index.do">会员中心</a> &gt; 完善用户资料</div>
     <div class="membertab3" id="div_user" >
      ...
      <p style="margin-left:20px;margin-top:15px;font-size:16px;"> 会员信息</p>
        ...
        <tr>
          ...
          <tr>
            ...
            <td align="left"><html-el:text styleClass="input_txt" style="width:160px;" property="link_phone" styleId="link_phone"  maxlength="11"></html-el:text><span style="color:#f00;display:none;" id="link_phone_erro_exist_error">该手机号已被使用，请重新输入！</span><span style="color:#f00;display:none;" id="link_phone_erro">手机号不能含空白字符！</span> </td>
          </tr> 
           <tr>
            <td align="right"><font color="red">*</font>邮箱：</td>  
            <td align="left"><html-el:text styleClass="input_txt" property="email"  styleId="email" maxlength="30" style="width:160px;"></html-el:text><span style="color:#f00;display:none;" id="email_erro_exist_error">该邮箱已被使用，请重新输入！</span><span style="color:#f00;display:none;" id="email_erro">邮箱不能含空白字符！</span>
         ...
         <p style="margin-left:20px;margin-top:15px;font-size:16px;">设置密码</p>
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="member_form_table1">
         <c:if test="${empty af.map.pass_word }">
          <tr>
            <td width="20%" align="right"><font color="red">* 登录密码：</font></td>
            <td width="80%" align="left"><html-el:password property="pass_word"  maxlength="20" styleId="pass_word" styleClass="input_txt" style="width:160px;" ></html-el:password><font>&nbsp;&nbsp;请使用6-20位大小写字母与数字混合密码！</font>
            <span style="font-size:10px;font-color:red;"> 
          	<br/>注：登陆密码是推荐游客登陆平台，产品页面隐藏佣金、积分、个人中心等相关信息。</span>
          	<font color="red" id="pass_word_msg"></font>
            </td>
          ...
          <tr>
            <td width="20%" align="right"><font color="red">* 支付密码：</font></td>
            <td width="80%" align="left"><html-el:password property="pay_pwd"  maxlength="20" styleId="pay_pwd" styleClass="input_txt" style="width:160px;" ></html-el:password><font>&nbsp;&nbsp;请使用6-20位大小写字母与数字混合密码！</font>
            <span style="font-size:10px;font-color:red;"> 
          	<br/>注：支付密码是本人登陆平台，产品页面显示佣金、积分、个人中心等相关信息。</span> 	<font color="red" id="pay_word_msg"></font>
            </td>
          </tr>
          <tr>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="member_form_table1">
         ...
         <div style="margin-left:10px;margin-top:40px;font-size:14px;"> <br/> 温馨提示，该账号仅限员工及亲友使用购机，商品不能二次销售，祝您购物愉快.</div>   
      </html-el:form>
    </div>
  </div>
  <div class="clear"></div>
...
$(document).ready(function(){
		...
		<c:if test="${empty af.map.pass_word }">
		$("#pass_word").attr("dataType", "Require").attr("msg", "请使用6-20位大小写字母与数字混合密码！");
		$("#pass_word_repeat").attr("datatype","Repeat").attr("to", "pass_word").attr("msg","两次输入的密码不一致！");
		</c:if><c:if test="${empty af.map.pay_pwd }">
		$("#pay_pwd").attr("dataType","User_name").attr("Require","true").attr("msg", "请使用位大小写字母与数字混合密码！");
		...
		$("#link_phone").blur(function(){
			...
			if(null == $(this).val() || $(this).val() == ''){
				$("#link_phone_exist_error").hide();
				$("#link_phone_erro").hide();
				$(this).css("background-color", "#fff");
				return ;
			}
			if($(this).val().indexOf(' ')>-1){
				$("#link_phone_exist_error").hide();
				$("#link_phone_erro").show();
				return;
			}
			...
			$.ajax({
				type: "POST",
				url: "RegUser.do",
				data: "method=validateMobile&link_phone=" + $(this).val(),
				dataType: "json",
				error: function(request, settings) {
					alert("检查手机号重复失败，请稍候再次尝试。");
					$("#link_phone_exist_error").show();
				...
				success: function(oper) {
					if (oper.result) {
						...
						$("#link_phone_exist_error").hide();
						$("#link_phone").css("background-color", "#fff");
					}
				}
			});
		});	
		...
		$("#email").blur(function(){
			...
			$.ajax({
				type: "POST",
				url: "RegUser.do",
				data: "method=validateEmail&email=" + $(this).val(),
				dataType: "json",
				error: function(request, settings) {
					alert("检查邮箱重复失败，请稍候再次尝试。");
		...
		$("#btn_submit").click(function(){    
			...
			<c:if test="${empty af.map.pass_word }">
			if(pwCheck($("#pass_word").val())<2){ 
				alert("您的登录密码安全性等级过弱,\n(建议使用6-20位大小写字母与数字混合密码)"); 
				return false;
			}</c:if>
			<c:if test="${empty af.map.pay_pwd }">
			if(pwCheck($("#pay_pwd").val())<2){
				alert("您的支付密码安全性等级过弱,\n(建议使用6-20位大小写字母与数字混合密码)");
				 return false;
			}</c:if>
