                  ...
                  <td>待选择区
                    <html-el:select property="select1" multiple="true" style="width:290px;height:358px;" styleId="select1" onchange="moveSelected(this.form.select1, this.form.select2);">
                      <c:forEach var="cur" items="${entityList}" varStatus="vs">
                        <option value="${cur.c_index}">${cur.c_name}</option>
                      </c:forEach>
                    </html-el:select></td>
                  <td>已选择区
                    <html-el:select property="select2" multiple="true" style="width:290px;height:358px;" styleId="select2" onchange="moveSelected(this.form.select2, this.form.select1);">
                      <c:if test="${not empty selectList}">
                        <html-el:optionsCollection label="c_name" value="c_index"  name="selectList" />
                      </c:if>
                    </html-el:select></td>
                </tr>
              </table></td>
...
function getQueryNames(key_word) {
   	...
   	$.ajax({
		type: "POST",
		cache: false,
		url: "${ctx}/manager/chengduoa/SelectKonkaOaCategory.do",
		data: "method=" + "getQueryNames" + "&key_word=" + $("#key_word").val() +"&select_indexes="+ $("#select_indexes").val(),
		dataType: "json",
		error: function(request, settings){},
		success: function(data) {
			if (data.length >= 1) {
				for(var i = 0; i < data.length - 1; i++) {
					var opt = new Option( data[i].c_name,data[i].c_index); 
