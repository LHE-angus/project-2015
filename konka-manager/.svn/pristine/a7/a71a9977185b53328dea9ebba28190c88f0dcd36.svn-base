          ...
          <c:forEach items="${bcomp_pd_list_top_5}" var="cur" varStatus="vs">
            ...
            <li style="padding:0;" class="${li_class}">
              <div class="listpic1 box_117_80" style="height:110px;">
              	<table border="0" cellspacing="0" cellpadding="0" width="120"><tr><td height="110" style="overflow:hidden;" width="120" valign="middle" align="center"><a href="<c:url value='/sfmall/PdShow.do?goods_id=${cur.id}' />"><img src="${ctx}/${cur.main_pic}" align="absmiddle"  width="100"/></a></td></tr></table>
              </div>
              <div class="listrxt1">
                <h3><a href="<c:url value='/sfmall/PdShow.do?goods_id=${cur.id}' />"><font style="font-size:13px;">康佳${fnx:abbreviate(cur.pd_sn, 2 * 7, '.')}<br/>${fn:escapeXml(cur.pd_size)}英寸</font></a></h3>
                <h4>价格：￥<font style="font-size:18px;"> <fmt:formatNumber value="${cur.ecGoodsPrice.original_price}" pattern="0" /></font></h4>
              </div>
              <div class="clear"></div>
            </li>
          </c:forEach>
        ...
        <div class="listbox3">
          ...
          <h3 class="lsitit2">商品筛选    <c:if test="${not empty af.map.prod_type  }">-</c:if> <c:if test="${af.map.prod_type eq 0  }">电视</c:if><c:if test="${af.map.prod_type eq 1  }">白电</c:if><c:if test="${af.map.prod_type eq 3  }">小家电</c:if><c:if test="${af.map.prod_type eq 10  }">配件</c:if> </h3>
            <ul class="listul5">
              ...
              <li><strong>屏幕尺寸：</strong> <a href="#" ${empty af.map.pd_size_type ? "style='background: #FF6600;color:#fff;padding:2px;'":""} class="pd_size_type_class" title="">全部</a> 
              <a href="#" ${af.map.pd_size_type eq 31 ? "style='background: #FF6600;color:#fff;padding:2px;'":""} class="pd_size_type_class" title="31,32英寸以下">32英寸以下</a> <a href="#" ${af.map.pd_size_type eq 32 ? "style='background: #FF6600;color:#fff;padding:2px;'":""} class="pd_size_type_class" title="32,32英寸">32英寸</a> 
              ...
              <a href="#" ${af.map.pd_size_type eq 42 ? "style='background: #FF6600;color:#fff;padding:2px;'":""} class="pd_size_type_class" title="42,42/43英寸">42/43英寸</a> 
              ...
              <a href="#" ${af.map.pd_size_type eq 58 ? "style='background: #FF6600;color:#fff;padding:2px;'":""} class="pd_size_type_class" title="59,58英寸以上">58英寸以上</a> </li></c:if> <li><strong>分类标签：</strong> <a href="#" ${empty af.map.label_of_cate ? "style='background: #FF6600;color:#fff;padding:2px;'":""} class="label_of_cate_class" title="">全部</a> 
      ...
      <tbody>
        <tr>
          ...
          <td><div class="listul2">
              <div class="rpic2 box"><a title="" href="<c:url value='/sfmall/PdShow.do?goods_id=${cur.id}' />"><img alt="" src="${ctx}/${fn:substringBefore(cur.main_pic, '.')}.jpg" align="absmiddle" style="max-height:216px;" width="288"/></a></div>
              <div class="ltt2" ><a href="<c:url value='/sfmall/PdShow.do?goods_id=${cur.id}' />" title="${fn:escapeXml(cur.pd_name)}">
               <font style="font-szie:13px;">	 <c:if test="${fn:contains(cur.pd_name, 'HYUNDAI')}">HYUNDAI</c:if>
               <c:if test="${not fn:contains(cur.pd_name, 'HYUNDAI')}">康佳</c:if>&nbsp;${fn:escapeXml(cur.pd_sn)} <c:if test="${not empty cur.pd_size }">&nbsp;${fn:escapeXml(cur.pd_size)}英寸</c:if><br/></font>
               <font class="redfont">${fnx:abbreviate(cur.pd_name, 2 * 18, '')}</font></a></div>
              <div class="ltt3"> 
                <p style="color:#666666;"><font color="#d40207" style="font-size:12px;">￥<a href="<c:url value='/sfmall/PdShow.do?goods_id=${cur.id}' />" style="color:red;font-size:18px;font-family:arial,宋体b8b\4f53;"><fmt:formatNumber value="${cur.map.original_price}" pattern="0" />
                </a></font>&nbsp; 
