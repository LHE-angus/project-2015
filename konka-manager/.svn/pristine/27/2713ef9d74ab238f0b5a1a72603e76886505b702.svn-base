...
<html-el:form action="/manager/JxcKonkaOrderRegister?method=listpdselect">
  <strong class="fb">产品型号：</strong>
  <html-el:text property="md_name_like" styleId="md_name_like" onkeyup="upperCase(this.id)" maxlength="40" />
  ...
  <input class="but1" type="button" name="reset" value=" 清空所选 " onclick="location.href='JxcKonkaOrderRegister.do?method=listpdselect';" ${fn:length(entity2List) eq 0 ? 'disabled="disabled"' : ''} />
  <input class="but1" type="button" name="reset" value=" 确认以上产品，下一步&gt;&gt; " onclick="location.href='JxcKonkaOrderRegister.do?method=addInMobile&pd_ids=' + this.form.pd_ids.value;" ${fn:length(entity2List) eq 0 ? 'disabled="disabled"' : ''} /></div>
  <div class="fb"><strong class="fb">待选择</strong><c:if test="${not empty af.map.md_name_like}">（<span style="color:#F00;font-weight:700;">“${af.map.md_name_like}”</span> 的搜索结果）</c:if></div>
  <table width="100%" border="1" cellspacing="0" cellpadding="3" style="border-collapse:collapse;">
    <tr class="tabtt1">
      <td width="5%" nowrap="nowrap" align="center">序号</td>
      <td nowrap="nowrap" align="center">产品型号</td>
    ...
    <tr>
      <td height="80"><div style="text-align:left; padding-right: 5px;"> 
          ...
          <script type="text/javascript">
				...
				pager.addHiddenInputs("method", "listpdselect");
