...
MzTreeView.prototype.setStyle = function()
{
  ...
  */
  ...
  if(this.wordLine)
  {
    ...
    {\
      vertical-align: middle; font-size: 21px; height: 20px; color: #D4D0C8;\
    }\
    ...
    {\
      ...
      cursor: "+ (this.navigator=="msie" ? "pointer" : "pointer") +";\
      padding: 0 2px 0 2px;\
      text-align: center;\
  ...
  if(!isRoot)
  {
    ...
    else
    {
      ...
      HTML += "<SPAN>"+ this.word2image(node.parentNode.childAppend) +"<IMG "+
        ...
        "src='"+ this.icons[node.iconExpand].src +"' style='cursor:pointer;'></SPAN>";
    }
  }
  HTML += "<IMG "+
    "align='absMiddle' "+
