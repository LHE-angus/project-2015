...
public class LoginAction extends BaseAction {
	...
	public ActionForward showLoginForm(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		if (null != userInfo&&"1".equals(session.getAttribute("shopUser"))) {
	...
	public ActionForward login(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		if (StringUtils.isBlank(user_name)) {
			...
			super.renderJavaScript(response, "window.onload=function(){alert('" + msg + "');location.href='login.do?method=showLoginForm'}");
			return null;
		}
		...
		if (!f){
			...
			session.setAttribute("shopUser", "1");
			session.setAttribute("touchUser", null);
			session.setAttribute("memberUser", null);
			session.setAttribute("userIp", IpUtils.getRemortIP(request));
			OperLog t = new OperLog();
			t.setOper_time(new Timestamp(Calendar.getInstance().getTime().getTime()));
			t.setOper_ip(IpUtils.getIpAddr(request));
			t.setLink_tab("EC_USER");
...
//				}
				...
				shoppingCar.setUser_ip((String)super.getSessionAttribute(request, "userIp"));
 			...
 			List<HashMap<String, String>> shopingCarList =shoppingCarTableList;// ShoppingCarJsonUtil.joinShoppingCarList(shopingCarCookieList,shoppingCarTableList);
				...
				//if (shopingCarList != null && shopingCarList.size() > 0) {
					...
					esc.setUser_ip((String)super.getSessionAttribute(request, "userIp"));
					...
					for (int i = 0; i < shopingCarList.size(); i++) {
						...
						try {
							...
							ecShoppingCart.setUser_ip((String)super.getSessionAttribute(request, "userIp"));
