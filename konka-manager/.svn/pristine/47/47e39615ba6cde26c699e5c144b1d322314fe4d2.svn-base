...
<div class="togo">
  ...
  <table border="0" width="100%"  cellspacing="0" cellpadding="0">
     <tr><c:forEach items="${entityList}" var="cur" varStatus="vs">
       <td width="49%" >
	         <div style="padding:10px;width:420px;border: 1px #F3F3F3 solid;">
	         	<a href="?method=view&lucky_id=${cur.id }" title="${fn:escapeXml(cur.title)}"><img src="${ctx}/${fn:substringBefore(cur.main_pic, '.')}.jpg" height="336" width="425" /></a>
	            <div style="margin-right:5px;margin-left: 12px; margin-top: 10px;line-height:21px;font-size:13px;font-color:#666666; font-family:arial,宋体b8b\4f53;">
		         	<a href="?method=view&lucky_id=${cur.id }" title="${fn:escapeXml(cur.title)}">${fn:escapeXml(cur.title)}<div><font class="redfont">${fn:escapeXml(cur.brief)}</font></div></a>
	            </div>
	            <div class="pri">
		            <font color="#d40207" style="font-size:14px">￥<font style="font-size:18px">0</font></font> &nbsp;&nbsp;&nbsp;
		            <font color="#005AA0">已有${cur.map.buy_count }人参与</font>
	             </div>
	          </div>
        </td>
        <c:if test="${vs.count mod 2 eq 0 and not vs.last}">
        </tr>
        <tr></c:if>
        </c:forEach>
       </tr>
