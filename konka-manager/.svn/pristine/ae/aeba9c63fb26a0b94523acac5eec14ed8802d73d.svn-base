 ...
 * @version 2013-11-20
 */
public class KonkaMobileSailDataSearchForSizeSecAction extends BaseAction {
	...
	public ActionForward chart(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (StringUtils.isNotBlank(search_first_flag) && "1".equals(search_first_flag)) {
			...
			if (null != list && list.size() > 0) {
				for (int i = 0; i < list.size(); i++) {
					...
					if ("0".equals(label)) {
						...
						label = "70寸及以上";
					} else if ("9999".equals(label)) {
						label = "其他";
					}
					baseChart.setLabel(label);

					if (StringUtils.isNotBlank(chart_type) && "2".equals(chart_type)) {
						baseChart.setValue(obj[6] == null ? "0" : obj[6].toString());
					} else {
						baseChart.setValue(obj[2] == null ? "0" : obj[2].toString());
					}
			...
			render(response, getFacade().getTemplateService().getContent("chart/Pie3D.ftl", model),
	...
	public List<?> getKonkaMobileSailDataList(VADetailsOfSalesData entity) {
		...
		String sql = " SELECT a3.* FROM (SELECT decode (a2.SIZE_SEC,NULL,b2.SIZE_SEC,a2.SIZE_SEC) AS SIZE_SEC, round(value (b2.R3_PRICE, 0)/10000,2) AS R3_PRICE , value (b2.R3_NUM, 0) AS R3_NUM, ";
		sql += " value (b2.R3_SINGLE_PRICE, '0') AS R3_SINGLE_PRICE, value (b2.R3_SCALE, '0') AS R3_SCALE, round(value (a2.ALL_PRICE, 0)/10000,2) AS ALL_PRICE, ";
		sql += " value (a2.ALL_NUM, 0) AS ALL_NUM,  value (a2.SALE_SINGLE_PRICE, '0') AS SALE_SINGLE_PRICE, value (a2.SALE_SCALE, '0') AS SALE_SCALE ";
		sql += " FROM    (SELECT a.SIZE_SEC, a.all_price, a.all_num, decode (to_char (a.all_num), 0, '-', to_char(round (a.all_price / a.all_num, 2))) AS SALE_SINGLE_PRICE, ";
		sql += "               decode (to_char (b.all_num), 0, '-', to_char(round (a.all_num * 100 / b.all_num, 2))) AS sale_scale ";
		sql += "   FROM    (SELECT t5.SIZE_SEC, sum (t5.all_price) AS all_price, sum (t5.all_num) AS all_num, 1 AS flag ";
		//sql += " FROM (SELECT decode (t.SIZE_SEC,0,t.SIZE_SEC,1,t.SIZE_SEC,2,t.SIZE_SEC,3,t.SIZE_SEC,4,t.SIZE_SEC,5,t.SIZE_SEC,6,t.SIZE_SEC,7,t.SIZE_SEC,8,t.SIZE_SEC,9,t.SIZE_SEC,9999) AS SIZE_SEC, ";
		sql += " FROM (SELECT decode (t.SIZE_SEC,0,t.SIZE_SEC,1,t.SIZE_SEC,2,t.SIZE_SEC,3,t.SIZE_SEC,4,t.SIZE_SEC,5,t.SIZE_SEC,6,t.SIZE_SEC,7,t.SIZE_SEC,9999) AS SIZE_SEC, ";
		sql += "                                 value (sum (t.ALL_PRICE), 0) AS all_price, value (sum (t.NUM), 0) AS all_num FROM V_A_DETAILS_OF_SALES_DATA t WHERE 1 = 1 ";
		...
		sql += "                  GROUP BY t.SIZE_SEC) t5 WHERE 1 = 1 GROUP BY t5.SIZE_SEC) a LEFT JOIN ";
		sql += "         (SELECT 1 AS flag,value (sum (t.ALL_PRICE), 0) AS all_price,value (sum (t.NUM), 0) AS all_num FROM V_A_DETAILS_OF_SALES_DATA t WHERE 1 = 1 ";
		...
		sql += "                       ) b ON a.flag = b.flag) a2 FULL OUTER JOIN ";
		sql += "  (SELECT a1.SIZE_SEC,a1.r3_price,a1.r3_num,decode (to_char (a1.r3_num),0,'-',to_char(round (a1.r3_price / a1.r3_num, 2))) AS R3_SINGLE_PRICE, ";
		sql += "       decode (to_char (b1.r3_num),0,'-',to_char(round (a1.r3_num * 100 / b1.r3_num, 2))) AS r3_scale ";
		sql += " FROM    (SELECT t3.SIZE_SEC,sum (t3.r3_num) AS r3_num,sum (t3.r3_price) AS r3_price,1 AS flag ";
		//sql += "            FROM (SELECT decode (t2.SIZE_SEC,0,t2.SIZE_SEC,1,t2.SIZE_SEC,2,t2.SIZE_SEC,3,t2.SIZE_SEC,4,t2.SIZE_SEC,5,t2.SIZE_SEC,6,t2.SIZE_SEC,7,t2.SIZE_SEC,8,t2.SIZE_SEC,9,t2.SIZE_SEC,9999) AS SIZE_SEC, ";
		sql += "            FROM (SELECT decode (t2.SIZE_SEC,0,t2.SIZE_SEC,1,t2.SIZE_SEC,2,t2.SIZE_SEC,3,t2.SIZE_SEC,4,t2.SIZE_SEC,5,t2.SIZE_SEC,6,t2.SIZE_SEC,7,t2.SIZE_SEC,9999) AS SIZE_SEC, ";
		sql += "                         value (sum (t1.column_12), 0) AS r3_num,value (sum (t1.column_14), 0) AS r3_price ";
		sql += "                    FROM CHANNEL_DATA_IMPORT t1 LEFT JOIN MV_ORG_OF_CUSTOMER t ON t1.column_1 = t.R3_CODE LEFT JOIN KONKA_PE_PD_MODEL t2 ON t1.column_11 = t2.MD_NAME WHERE 1 = 1 and t1.column_12 <>0 ";
		if (StringUtils.isNotBlank(entity.getDept_id_in())) {
			sql += " and t.CUR_DEPT_ID in (" + entity.getDept_id_in() + ") ";
		}
		if (entity.getDept_id() != null) {
		...
		sql += "    GROUP BY t2.SIZE_SEC) t3 WHERE 1 = 1 GROUP BY t3.SIZE_SEC) a1 LEFT JOIN (SELECT 1 AS flag, value (sum (t1.column_14), 0) AS r3_price,value (sum (t1.column_12), 0) AS r3_num ";
		sql += "    FROM CHANNEL_DATA_IMPORT t1 LEFT JOIN MV_ORG_OF_CUSTOMER t ON t1.column_1 = t.R3_CODE LEFT JOIN KONKA_PE_PD_MODEL t2 ON t1.column_11 = t2.MD_NAME WHERE 1 = 1  and t1.column_12 <>0 ";
		...
		sql += "    ) b1 ON a1.flag = b1.flag) b2 ON a2.SIZE_SEC = b2.SIZE_SEC WHERE 1 = 1) a3 ORDER BY a3.SIZE_SEC ASC ";
