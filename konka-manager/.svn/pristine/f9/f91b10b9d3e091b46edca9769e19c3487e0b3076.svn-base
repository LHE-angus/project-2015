...
<div class="shop_list">
	...
	<div class="b_r">
		<div class="shop_right">
	    			...
	    			<c:forEach items="${cate3List}" var="cur" varStatus="vs">
	    				...
	    				<li>
		        			<div class="t_l">
					          ...
					          <p><font class="red"><fmt:formatNumber value="${cur.buy_price}" type="currency" /></font></p>
					          <h3 class="p3" onclick="location.href='${ctx}/customer/manager/PShow.do?method=OrderNow&id=${cur.id}'">立即购买</h3>
		        			</div>
		        			<div class="t_r">
      	...
      	<html-el:form action="/manager/PShow.do" method="post" styleClass="class_form">
      		...
      		<html-el:hidden property="mark" styleId="mark_val" />
	        ...
	        <div class="list_s">
	        			...
	        			<tr>
	        				...
	        				<td>
	        					<span class="ContSearchBtnWrap">
					        		<span><cite id="mark_text"></cite><i></i></span>
	            ...
	            <div class="list_s_box">
	            	<div class="filter">
	              		<div class="filter_l">品类：</div>
	              		<div class="filter_r">
	              			<ul id="mark">
				              <li class="cdr"><a rel="" hidefocus="true">全部</a></li>
				              <li><a rel="3d" hidefocus="true">3D电视</a></li>
				              <li><a rel="int" hidefocus="true">智能电视</a></li>
				              <li><a rel="www" hidefocus="true">网络电视</a></li>
	              			</ul>
	              		</div>
	            	</div>
	              	<div class="filter">
		...
		<script type="text/javascript">
			$(function(){
				...
				function Tsfn(obj,type){
					...
					if(type=='TsClick'){	
						if(Tsname=="Discount"){
							...
							// 品类特殊处理
							if(Tsname == "mark"){
								if(TsVal == "3d"){ $("#label_3d").val("1"); $("#label_int").val(""); $("#label_www").val("");} 
								else if(TsVal == "int"){ $("#label_3d").val(""); $("#label_int").val("1"); $("#label_www").val("");} 
								else if(TsVal == "www"){ $("#label_3d").val(""); $("#label_int").val(""); $("#label_www").val("1"); }
								else { $("#label_3d").val(""); $("#label_int").val(""); $("#label_www").val("0"); }
							}
				...
				//回显
				if ("" != "${af.map.mark}")
					echo("${af.map.mark}", "mark");
				if ("" != "${af.map.size_val}") 
					echo("${af.map.size_val}", "size");
				if ("" != "${af.map.price_val}") 
					echo("${af.map.price_val}", "price");
				if ("" != "${af.map.resolution_val}") 
					echo("${af.map.resolution_val}", "resolution");
				if ("" != "${af.map.cate_val}") 
					echo("${af.map.cate_val}", "cate");
				...
				function echo(val, type){
					...
					$("a", "#" + type).each(function(){
						...
						if ($(this).attr("rel") == val) {
							...
							// 品类特殊处理
							if(type == "mark"){
								if(val == "3d"){ $("#label_3d").val("1"); $("#label_int").val(""); $("#label_www").val("");} 
								else if(val == "int"){ $("#label_3d").val(""); $("#label_int").val("1"); $("#label_www").val("");} 
								else if(val == "www"){ $("#label_3d").val(""); $("#label_int").val(""); $("#label_www").val("1"); }
								else { $("#label_3d").val(""); $("#label_int").val(""); $("#label_www").val("0"); }
							}
						}
					});
 		...
 		<div id="nav" class="six_list">
 			<div class="six_one">
 				...
 				<form id="bottomPageForm" name="bottomPageForm" method="post" action="PShow.do">
	 				...
	 				<li>
	 					<p style="padding:10px;margin-bottom:10px;" title="${cur.pd_desc}">
	 						<c:if test="${not empty cur.main_pic}">
				  				<c:set value="${fn:split(cur.main_pic, ',')[0]}" var="main_pic_path" />
				  				<img src="${ctx}/${fn:substringBefore(main_pic_path, '.')}_240.jpg" width="234" height="234" onclick="location.href='${ctx}/customer/manager/PShow.do?method=OrderNow&id=${cur.id}'" />
			  				</c:if>
	 					</p>
	 					<p class="l_list">康佳${fnx:abbreviate(cur.map.md_name, 2*22, '')}&nbsp;&nbsp;<c:out value="${cur.pd_size}" />英寸</p>
	 					<p class="l_list"><font class="red"><fmt:formatNumber value="${cur.buy_price}" type="currency" /></font></p>
	 					<div class="bbq">
	 						<div class="bcd">加入购物车</div>
	 						<div class="bdd" style="cursor:pointer;" onclick="location.href='${ctx}/customer/manager/PShow.do?method=OrderNow&id=${cur.id}'"><font class="orange">立刻购买</font></div>
	 					</div>
	 					<c:if test="${not empty sup_class}">
