    ...
    <div class="pageContent">
      <html-el:form action="/admin/SelectEntyShopByArea">
        ...
        <table width="100%" border="0" align="left" class="list">
          <tr>
            <td>搜索网点名：
              <html-el:text property="key_word" styleId="key_word" size="24" maxlength="100" style="width:100px;" />
              &nbsp;&nbsp;
              <input type="button" value="搜 索" onclick="getQueryNames();" /></td>
          </tr>
          <tr>
                  ...
                  <td>待选择区
                      <html-el:select property="select1" multiple="true" style="width:290px;height:358px;" styleId="select1" onchange="moveSelected(this.form.select1, this.form.select2);">
                        <c:forEach var="cur" items="${entityList}" varStatus="vs">
                          <option value="${cur.shop_id}">${cur.shop_name}</option>
                        </c:forEach>
                      </html-el:select>
                  </td>
                  <td>已选择区
                    <html-el:select property="select2" multiple="true" style="width:290px;height:358px;" styleId="select2" onchange="moveSelected(this.form.select2, this.form.select1);">
                      <c:if test="${not empty selectList}">
                        <html-el:optionsCollection label="shop_name" value="shop_id"  name="selectList" />
                      </c:if>
                    </html-el:select></td>
                </tr>
              </table></td>
...
function getQueryNames() {
   	...
   	$.ajax({
		type: "POST",
		cache: false,
		url: "${ctx}/manager/admin/SelectEntyShopByArea.do",
		//data:{'method':'getQueryNames', 'this_userid_dept':'${userInfo.dept_id}', 'shop_type':'1'},
		data: "method=getQueryNames&this_userid_dept=${userInfo.dept_id}&shop_type=1&key_word=" + $("#key_word").val()+"&areas_ids="+$("#areas_ids").val(),
		dataType: "json",
