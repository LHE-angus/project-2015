...
public class ShoppingCarAction extends BaseMemberAction {
	...
	public ActionForward list(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		for (EcShoppingCart temp : ecShoppingCartList) {
			...
			// 查询产品价格
			List<EcGoodsPrice> ecGoodsPriceList = getEcGoodsPriceListWithPindexAndGoodsId(ecUser.getUser_type(),  konkaBcompPd.getId(), p_index_array, ecUser.getDept_id(),ecUser.getCust_id());
			if (null != ecGoodsPriceList && 0 != ecGoodsPriceList.size())
				konkaBcompPd.setEcGoodsPrice(ecGoodsPriceList.get(0));

			// 查询库存
			List<EcStocks> ecStocksList = getEcStocksListWithPindexAndGoodsId(1,  konkaBcompPd.getId(), p_index_array, null);
			int r3store_num=0;
			if(konkaBcompPd.getProd_type()!=null&&konkaBcompPd.getProd_type().intValue()==0){
			try{
				if(konkaBcompPd!=null){
					r3store_num=super.getStockCount(konkaBcompPd.getPd_sn()).intValue();
				}
			}catch(Exception e){}; 
			}
		...
		return new ActionForward("/../epp/mobile/ShoppingCar/step_one.jsp");
	}
	...
	public ActionForward buyByGoodsId(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		// 查询库存
		List<EcStocks> ecStocksList = getEcStocksListWithPindexAndGoodsId(ecUser.getUser_type(), konkaBcompPd.getId(), p_index_array, null);
		int r3store_num=0;
		if(konkaBcompPd.getProd_type()!=null&&konkaBcompPd.getProd_type().intValue()==0){
		try{
			if(konkaBcompPd!=null){
		...
		if(goods_ids!=null&&goods_ids.length>0){
			for(int i=0;i<goods_ids.length;i++){
				...
				if(konkaBcompPd!=null){
					...
					// 查询库存
					ecStocksList = getEcStocksListWithPindexAndGoodsId(ecUser.getUser_type(), konkaBcompPd.getId(), p_index_array, null);
					if (null != ecStocksList && 0 != ecStocksList.size()){
	...
	public ActionForward stepTwo(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		for (String id : goods_ids) { 
			...
			if(ecBcompPdRebates!=null&&ecBcompPdRebates.getB_value()!=null&&ecBcompPdRebates.getB_value().floatValue()>0.00001){
			konkaBcompPd.setEcBcompPdRebates(ecBcompPdRebates);
			request.setAttribute("is_rebate", "1");
			
			if (ecBcompPdRebates.getB_type() == 0){ // 按比例
				all_rebates=all_rebates.add(price.multiply(new BigDecimal(buy_num)).multiply(ecBcompPdRebates.getB_value()).divide(new BigDecimal("100")).setScale(2, BigDecimal.ROUND_HALF_UP));
			}
			if (ecBcompPdRebates.getB_type() == 1){ // 按固定
				all_rebates=all_rebates.add(new BigDecimal(buy_num).multiply(ecBcompPdRebates.getB_value()).setScale(2, BigDecimal.ROUND_HALF_UP));
			}
			}
			konkaBcompPdList.add(konkaBcompPd);	 
		}
		...
		return new ActionForward("/../epp/mobile/ShoppingCar/step_two.jsp");
	}
	...
	public ActionForward createOrder(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		if(payPrice.doubleValue()<0.001|| pshowOrder.getPay_way()==0 || pshowOrder.getPay_way()==1 || pshowOrder.getPay_way()==9){
			super.renderJavaScript(response, "window.onload=function(){location.href='" + super.getCtxPath(request)
					+ "/epp/mobile/center/Orders.do?';}");
			return null;
		}
		super.renderJavaScript(response, "window.onload=function(){location.href='" + super.getCtxPath(request)
				+ "/epp/mobile/Payment.do?trade_index=" + trade_index + "&pay_way=" + pshowOrder.getPay_way() + "';}");
		return null;
	}
	...
	public ActionForward checkPayPwd(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		EcUser ecUser = (EcUser) super.getSessionAttribute(request, Constants.EPP_USER);
			...
			return new ActionForward("/../epp/mobile/ShoppingCar/checkpwd.jsp");
		}
