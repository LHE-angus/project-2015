...
<div class="maincont">
  ...
  <div class="touch_right padbot45">
    <div class="position"><a href="${ctx }/touch/Index.do">首页</a> &gt; <a href="${ctx }/touch/center/Index.do">会员中心</a> &gt; <a href="${ctx }/touch/center/Orders.do">我的订单</a> </div>
    <div class="touchtab3">
      <p style="margin-top:15px;font-size:16px;"> 我的订单</p>
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="touch_form_table0"  style="border:1px solid #e3e3e3;">
        <tr >
        ...
        <tr>
          <td colspan="2">订单状态：
            ...
            <c:if test="${af.map.state eq 0 }"><c:if test="${af.map.pay_way eq 0 or af.map.pay_way eq 1 }">待审核  </c:if> <c:if test="${af.map.pay_way ne 9 and af.map.pay_way ne 0 and af.map.pay_way ne 1 }">待付款  </c:if></c:if>
            ...
            <c:if test="${af.map.state eq 60 }">交易完成</c:if>
          	<c:if test="${af.map.pay_way ne 9 and af.map.pay_way ne 0 and  af.map.pay_way ne 1}">
             <c:if test="${af.map.state eq 0 }">&nbsp;&nbsp; &nbsp;&nbsp; <a class="btn btn-4" href="<c:url value='/touch/Payment.do?trade_index=${af.map.trade_index}' />"><s></s>付款</a></c:if>
            </c:if><c:if test="${af.map.pay_way eq 9 }"> 线下处理  </c:if>
          </td>
        </tr>
      </table>
      <ul class="touchtit3" >
        <li class="curli" id="li1">订单跟踪 </li><li id="li2">付款信息</li>
      </ul>
      <table id="tbl1" width="100%" border="0" cellpadding="0" cellspacing="0" class="touch_form_table0" >
      ...
      <table id="tbl2" width="100%" border="0" cellpadding="0" cellspacing="0" class="touch_form_table0" style="border:1px solid #e3e3e3;display:none;" >
        <tr>
          <td align="left">付款方式：
...
$(document).ready(function(){  
  ...
  $("#syncBtn").click(function(){
		...
		$.ajax({
			type: "post",
			url: "${ctx}/touch/center/Orders.do?method=sfList",
			data: {"id" : id},
			dataType: "json",
