...
<div align="center">
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="Tab">
    <tr>
      <c:forEach var="cur" items="${entityList}" varStatus="vs1">
        <td width="20%" title="${cur.brand_name}" nowrap="nowrap"><html-el:text property="${cur.brand_id}" value="${cur.brand_name},${cur.brand_id}" styleId="${cur.brand_id}" style="display:none;" />
          ${cur.brand_name}</td>
        <td width="10%"><input name="button" type="button" class="bgButtonChoose" style="cursor:pointer" id="button" onclick="window.returnValue=document.getElementById('${cur.brand_id}').value;window.close();" />
        </td>
    ...
    <c:if test="${empty entityList}">
	    <tr>
	    	<td align="center">
	    		<span style="color:#F00;font-size:13px;">抱歉！没有查询到品牌结果</span>
	    	</td>
	    </tr>
    </c:if>
  </table>
  <form id="bottomPageForm" name="bottomPageForm" method="post" action="JxcStockBill.do">
