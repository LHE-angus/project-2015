...
<body><div class="top_class"><span class="lspan"><a href="javascript:void(0);history.back();"><img src="${ctx}/styles/epp/mobile/images/return.gif" /></a></span><h3>订单明细</h3></div>
<div id="content" >
<div class="mainbox">
<div class="maincont"> 
<form action="<c:url value='/webservice/wap/ShoppingCar.do' />" method="post" id="step_two_form">
...
<div style="width:100%;background-color:#ffffff;">
   <div style="padding: 10px;line-height:26px;font-size:18px;font-family: Microsoft YaHei ;">订单明细信息 </div> 
    ...
    <div style="margin-left:3px;padding: 3px;border: solid 1px #E6E6E6;">
     ...
     <b>订单状态 </b><c:if test="${cur1.state eq -30 }">退货成功</c:if><c:if test="${cur1.state eq -20 }">处理失败</c:if><c:if test="${cur1.state eq -10 }">已取消</c:if><c:if test="${cur1.state eq 0 }"><c:if test="${cur1.pay_way eq 0 or cur1.pay_way eq 1 }"> 待审核  </c:if> <c:if test="${cur1.pay_way ne 0 and cur1.pay_way ne 1}">待付款  </c:if></c:if><c:if test="${cur1.state eq 5 }">待确认</c:if><c:if test="${cur1.state eq 10 }">已确认待处理</c:if><c:if test="${cur1.state eq 20 }">订单处理中</c:if> <c:if test="${cur1.state eq 30 }">订单处理中</c:if><c:if test="${cur1.state eq 40 }">已发货</c:if><c:if test="${cur1.state eq 50 }">已换货</c:if><c:if test="${cur1.state eq 60 }">交易完成</c:if> 
  	</div>   	
