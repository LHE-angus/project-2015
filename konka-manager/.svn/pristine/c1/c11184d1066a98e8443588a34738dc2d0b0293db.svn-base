...
public class mLoginAction extends BaseAction {
	...
	public ActionForward login(ActionMapping mapping, ActionForm form,
		...
		// "login.failed.verificationCode.isEmpty");
		// super.renderText(response, msg);
		// return null;
		// }
		//
		// if (!verificationCode.equals((String) session
		// .getAttribute("verificationCode"))) {
		// msg = super.getMessage(request,
		// "login.failed.verificationCode.invalid");
		// super.renderText(response, msg);
		// return null;
		// }
		// }
		...
		if (null == userInfo) {
			...
			// cg.setCookieName("user_name_mobile");
			// cg.addCookie(response, URLEncoder.encode(user_name, "UTF-8"));
			// cg.setCookieName("password_mobile");
			// cg.addCookie(response, URLEncoder.encode(entity.getPassword(),
			// "UTF-8"));
			// cg.setCookieName("is_remember_mobile");
			...
			session.setAttribute(Constants.ROLE_MOBILE, peRoleUser);
	...
	private void setCookies2RequestScope(HttpServletRequest request)
		...
		if (null != user_name) {
			request.setAttribute("user_name_mobile", URLDecoder.decode(
					user_name.getValue(), "UTF-8"));
		}
		if (null != password) {
			request.setAttribute("password_mobile", DEFAULT_PASSWORD_MOBILE);
		}
		if (null != is_remember) {
			request.setAttribute("is_remember_mobile", URLDecoder.decode(
					is_remember.getValue(), "UTF-8"));
		}
	}
