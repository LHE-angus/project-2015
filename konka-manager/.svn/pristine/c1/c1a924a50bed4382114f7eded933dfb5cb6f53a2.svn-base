...
<form action="<c:url value='/member/Payment.do' />" method="post" id="pay_form" target="_blank">
  ...
  <table cellpadding="0" cellspacing="0" border="0" align="center" style="width:550px;margin-top:70px;">
  	 <tr>
  		...
  		<td width="30%"><p style="margin-top:10px;font-size:12px;">稍后支付&gt;&gt;<a href="${ctx }/member/center/Orders.do?">查看我的订单</a></p></td>
	  </tr>
	  ...
	  <tr>
	  	 <td height="140" colspan="2">
  			...
  			<c:if test="${is_daier ne 1 or not empty af.map.is_tuangou}">
  			<label class="bank bank--alipay btn_sub" data="2">支付宝</label> 
  		 	</c:if>
  		 	<c:if test="${empty af.map.is_tuangou}">  
...
$(document).ready(function(){ 
	...
	$(document).delegate("#heckOrderStatus", "click", function(){
		location.href = "${ctx}/member/Payment.do?method=checkOrderStatus&trade_index=${af.map.trade_index}"
	}); 
