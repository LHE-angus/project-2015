 ...
 * @version 2013-11-21
 */
public class KonkaMobileSailDataSearchForMdNameAction extends BaseAction {
	...
	public ActionForward list(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		String order_type = (String) dynaBean.get("order_type");//排序类型
	...
	public List<?> getKonkaMobileSailDataList(VADetailsOfSalesData entity) {
		...
		String sql = " SELECT a3.* FROM (SELECT decode (a2.MODEL_NAME, NULL, b2.MD_NAME, a2.MODEL_NAME) AS MODEL_NAME, ";
		...
		sql += "              			 value (b2.R3_SCALE, '0') AS R3_SCALE, value (b2.total_r3_scale, '0') AS total_r3_scale, round(value (a2.ALL_PRICE, 0)/10000,2) AS ALL_PRICE, ";
		sql += "              			 value (a2.ALL_NUM, 0) AS ALL_NUM, value (a2.SALE_SINGLE_PRICE, '0') AS SALE_SINGLE_PRICE, value (a2.SALE_SCALE, '0') AS SALE_SCALE, ";
		sql += "              			 value (a2.total_sale_scale, '0') AS total_sale_scale ";
		sql += "       					 FROM    (SELECT c.*, d.total_sale_scale ";
		sql += "                      			  FROM    (SELECT a.MODEL_NAME, a.all_price, a.all_num, decode (to_char (a.all_num), 0, '-', to_char(round (a.all_price / a.all_num, 2))) AS SALE_SINGLE_PRICE, ";
		sql += "                                     	   decode (to_char (b.all_price), 0, '-', to_char(round (a.all_price * 100 / b.all_price, 4))) AS sale_scale ";
		sql += "                                		   FROM    (SELECT t.MODEL_NAME, value (sum (t.ALL_PRICE), 0) AS all_price, value (sum (t.NUM), 0) AS all_num, 1 AS flag ";
		...
		sql += "                  ) b ON a.flag = b.flag) c ";
		sql += "                  LEFT JOIN (SELECT a.MODEL_NAME, decode (to_char (b.all_price), 0, '-', to_char(round (a.all_price * 100 / b.all_price, 4))) AS total_sale_scale ";
		sql += "                             FROM    (SELECT t.MODEL_NAME, value (sum (t.ALL_PRICE), 0) AS all_price, value (sum (t.NUM), 0) AS all_num, 1 AS flag ";
		sql += "                                      FROM V_A_DETAILS_OF_SALES_DATA t WHERE 1 = 1 ";
		...
		if (entity.getMap().get("date_begin") != null
				&& StringUtils.isNotBlank(entity.getMap().get("date_begin").toString())) {
			sql += " and t.SALE_DATE >= to_date(?,'yyyy-MM-dd hh24:mi:ss') ";
			array.add(entity.getMap().get("date_begin").toString().substring(0, 4) + "-01-01 00:00:00");
		}
		if (entity.getMap().get("date_end") != null
				&& StringUtils.isNotBlank(entity.getMap().get("date_end").toString())) {
			sql += " and t.SALE_DATE <= to_date(?,'yyyy-MM-dd hh24:mi:ss') ";
		...
		sql += "          ) b ON a.flag = b.flag) d ON c.MODEL_NAME = d.MODEL_NAME) a2 ";
		sql += "          FULL OUTER JOIN (SELECT c1.*, d1.total_r3_scale ";
		sql += "                           FROM    (SELECT a1.MD_NAME, a1.r3_price, a1.r3_num, decode (to_char (a1.r3_num), 0, '-', to_char(round (a1.r3_price / a1.r3_num, 2))) AS R3_SINGLE_PRICE, ";
		sql += "                                    decode (to_char (b1.r3_price), 0, '-', to_char(round (a1.r3_price * 100 / b1.r3_price, 4))) AS r3_scale ";
		sql += "                                    FROM    (SELECT t2.MD_NAME, value (sum (t1.column_12), 0) AS r3_num, value (sum (t1.column_14), 0) AS r3_price, 1 AS flag ";
		...
		sql += "       GROUP BY t2.MD_NAME) a1 LEFT JOIN ";
		sql += "              (SELECT 1 AS flag, value (sum (t1.column_14), 0) AS r3_price, value (sum (t1.column_12), 0) AS r3_num ";
		sql += "               FROM CHANNEL_DATA_IMPORT t1 LEFT JOIN MV_ORG_OF_CUSTOMER t ON t1.column_1 = t.R3_CODE LEFT JOIN KONKA_PE_PD_MODEL t2 ";
		...
		sql += "      ) b1 ON a1.flag = b1.flag) c1 LEFT JOIN ";
		sql += "        (SELECT a1.MD_NAME, decode (to_char (b1.r3_price), 0, '-', to_char(round (a1.r3_price * 100 / b1.r3_price, 4))) AS total_r3_scale ";
		sql += "         FROM    (SELECT t2.MD_NAME, value (sum (t1.column_12), 0) AS r3_num, value (sum (t1.column_14), 0) AS r3_price, 1 AS flag ";
		sql += "                  FROM CHANNEL_DATA_IMPORT t1 LEFT JOIN MV_ORG_OF_CUSTOMER t ON t1.column_1 = t.R3_CODE ";
		sql += "                  LEFT JOIN KONKA_PE_PD_MODEL t2 ON t1.column_11 = t2.MD_NAME WHERE 1 = 1 ";
		...
		sql += "   ) b1 ON a1.flag = b1.flag) d1 ON c1.MD_NAME = d1.MD_NAME) b2 ON a2.MODEL_NAME = b2.MD_NAME) a3 ORDER BY ";
		
		if(entity.getMap().get("order_type") != null
				&& StringUtils.isNotBlank(entity.getMap().get("order_type").toString())){
			if("1".equals(entity.getMap().get("order_type").toString())){
				sql += " a3.R3_PRICE desc, ";
			}else if("2".equals(entity.getMap().get("order_type").toString())){
				sql += " a3.ALL_PRICE desc, ";
			}
		}
		sql += " a3.MODEL_NAME ASC  ";
