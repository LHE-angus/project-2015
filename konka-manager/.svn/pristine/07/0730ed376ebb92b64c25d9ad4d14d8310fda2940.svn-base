...
<head>
<meta charset="utf-8">
<title>康佳  - <c:out value="${konkaBcompPd.pd_name}" /></title>
...
<!-- second start -->
<div class="maincont" > 
  <div class="position"><a href="<c:url value='/zxmall/Index.do' />">首页</a> &gt; 康佳&nbsp;<c:out value="${konkaBcompPd.pd_sn}" /></div>
  <div class="viewbox1" style="min-height:450px;">
  	<!-- 产品展示详细 -->
    <jsp:include page="../__inc/_pd_images.jsp" flush="true" />
    <!-- 表单开始 -->
    <form action="<c:url value='/zxmall/ShoppingCar.do' />" id="form_pd_buy" method="post">
    ...
    <div class="viewright">
      <div class="viewtit1"><h3>康佳&nbsp;<c:out value="${konkaBcompPd.pd_sn}" /></h3><h4><c:out value="${konkaBcompPd.pd_name}" /></h4><div class="clear"></div>
      </div>
      <div class="viewtab1">
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="proxltab">
        	<c:if test="${zxmall.user_type eq 2 or empty zxmall}">
             <tr>
              <td colspan="3" height="22">会 员 价：<strong class="yel16b redfont" id="price_tag">￥<fmt:formatNumber value="${konkaBcompPd.ecGoodsPrice.price}" pattern="#,#00" /></strong><img id="price_tag_loading" style="display:none;" src="${ctx}/images/loading5.gif" /></td>
            </tr>
            <tr>
            ...
            <c:if test="${zxmall.user_type eq 1 and touch eq 1 }">
            <tr><c:if test="${not empty konkaBcompPd.integral and konkaBcompPd.integral >1}">
              <c:if test="${konkaBcompPd.integral_type eq 0}">
              	<c:set var="integral" value="${konkaBcompPd.ecGoodsPrice.price * konkaBcompPd.integral* zxmall.ecBaseCardLevel.card_type_discount / 10000}" />
              </c:if>
              <c:if test="${konkaBcompPd.integral_type eq 1}">
              	<c:set var="integral" value="${konkaBcompPd.integral * zxmall.ecBaseCardLevel.card_type_discount/100}" />
              </c:if>
              </c:if> <c:if test="${ empty konkaBcompPd.integral or konkaBcompPd.integral <1}">
              <c:set var="integral" value="${konkaBcompPd.ecGoodsPrice.price * zxmall.ecBaseCardLevel.card_type_discount / 100}" />
              </c:if> 
      ...
      <div class="viewtab2">
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="proxltab">
          <tbody>
            <tr>
              <td width="10%">购买数量：</td>
              <td width="90%"><div class="addtab"><img src="${ctx}/styles/zxmall/images/edd.gif" id="num_jian" style="cursor:pointer;" /></div>
                <div class="proinput1">
                  <input type="text" class="proinput2" name="buy_num" id="buy_num" value="1" onfocus="javascript:setOnlyInt(this);" maxlength="3" />
                </div>
                <div class="addtab"><img src="${ctx}/styles/zxmall/images/add.gif" id="num_jia" style="cursor:pointer;" /></div></td>
            </tr>
            ...
            <tr>
              <td colspan="2"><div class="padtop20"><img alt="加入购物车" src="${ctx}/styles/zxmall/images/but_joinshop.gif" width="132" height="38" style="cursor:pointer;" id="add_shoping_car_btn" />&nbsp;&nbsp;<img alt="立即购买" src="${ctx}/styles/zxmall/images/but_buy2.gif" style="cursor:pointer;" id="buy_btn" /></a>&nbsp;&nbsp;<img alt="关注" style="cursor:pointer;" id="add_favotrites_btn" src="${ctx}/styles/zxmall/images/but_attention.gif" /></a>
              	<div class="bdsharebuttonbox" data-tag="share_1"><a class="bds_more" data-cmd="more">分享</a></div>
				<script>
					window._bd_share_config = {
						common : {
							bdText : '${konkaBcompPd.pd_sn}',bdDesc : '${konkaBcompPd.pd_name}',
							bdUrl : 'http://zx.konka.com/zxmall/PdShow.do?goods_id=${konkaBcompPd.id}', 	
							bdPic : <c:forEach items="${konkaBcompPd.picArray}" var="cur" varStatus="vs" begin="1" end="1">'http://zx.konka.com/${fn:substringBefore(cur, ".")}_800.jpg'</c:forEach>
						},
						share : [{"bdSize" : 16}]
					}
...
<div class="maincont"> 
        ...
        <ul class="listul4" style="padding:0;">
           <c:forEach items="${bcomp_pd_list_top_5}" var="cur" varStatus="vs">
	          ...
	          <li style="padding:0;" class="${li_class}">
	            <div class="listpic1"><table border="0" cellspacing="0" cellpadding="0" width="120"><tr><td height="110" style="overflow:hidden;" width="120" valign="middle" align="center"><a href="<c:url value='/zxmall/PdShow.do?goods_id=${cur.id}' />"><img src="${ctx}/${cur.main_pic}" style="max-height:100px;" width="100"/></a></td></tr></table></div>
	            <div class="listrxt1">
	              <h3><a href="<c:url value='/zxmall/PdShow.do?goods_id=${cur.id}' />">康佳&nbsp;<c:out value="${cur.pd_sn}" />&nbsp;<c:out value="${cur.pd_name}" /></a></h3>
  ...
  <!--right-->
  <div class="liscont3"> <c:if test="${not empty bcomp_pd_list_pj }">
  <div>
	<div class="right">
		<div id="recommend" class="m m1" data-widget="tabs" style="display: block;width:100%">
		<div class="mt">
		...
		<div id="newFittign-tab" data-widget="tabs">
		  ...
		  <div class="tab-cat-content">
		   <form action="<c:url value='/zxmall/ShoppingCar.do' />" id="form_pd_buy2" method="post">
		    <input type="hidden" name="method" value="buyByGoodsId" />
		    ...
		    <ul>
		      <li class="clearfix show" id="tab-cat-con1">  
		        ...
		        <div <c:if test="${vs.last ne true}">class="master"</c:if><c:if test="${ vs.last}">class="master1"</c:if>><s></s>
		          <div class="p-img"><a target="_blank" href="<c:url value='/zxmall/PdShow.do?goods_id=${cur.id}' />"><img width="93" src="${ctx}/${cur.main_pic}"/> </a> </div>
		          <div class="p-name"><a target="_blank" href="<c:url value='/zxmall/PdShow.do?goods_id=${cur.id}' />">${fnx:abbreviate(cur.pd_name, 2 * 16, '')}</a> </div>
		          <div class="choose">
		        ...
		        <div class="infos">
		          <div id="more-fitting-link"><a class="hl_link" target="_blank" href="${ctx}/zxmall/KonkaBcompPd.do?prod_type=10">进入配件选购中心&gt;</a><span> <b></b> </span> </div>
		          <s></s>
...
$(document).ready(function(){
	...
	$(document).delegate("#buy_btn", "click", function(){
		...
		if(parseInt(buy_num) <1){
 		...
 		$.ajax({
 			type: "POST",
 			url: "<c:url value='/zxmall/PdShow.do' />",
 			data: { "method":"ajaxGetSaleAreas", "goods_id":goods_id, "p_index":p_index, "timestamp":new Date().getTime() },
 			dataType: "json",
	...
	$(document).delegate(".pd_content", "click", function(){
		...
		$(".pd_content").each(function(){
		    ...
		    if($(this).attr("hover") == "hover"){
				 ...
				 // 如果是评论则加载Iframe
				 if("pdc3" == id){
					var url = "${ctx}/zxmall/EcPdEavl.do?goods_id=" + $("#goods_id").val() + "&timestamp=" + new Date().getTime();
