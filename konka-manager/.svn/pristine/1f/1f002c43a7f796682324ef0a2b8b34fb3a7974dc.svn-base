...
<div class="oarcont" align="center">
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="1" class="rtable3">
      ...
      <tr>
        <td width="15%" nowrap="nowrap" class="title_item" align="right">发布栏目：</td>
        <td width="85%"><c:forEach var="cur" items="${sysModuleWebList}">
        	<c:if test="${cur.c_index eq af.map.news_module}">${cur.c_name}</c:if>
        </c:forEach></td>
      </tr>
      <tr>
