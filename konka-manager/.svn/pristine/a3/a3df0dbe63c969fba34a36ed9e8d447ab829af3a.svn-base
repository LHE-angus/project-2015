...
<div class="maincont">
  ...
  <div class="member_right padbot45">
    <div class="position"><a href="${ctx }/member/Index.do">首页</a> &gt; <a href="${ctx }/member/center/Index.do">会员中心</a> &gt;我的佣金</div>
...
function encash(type,id){
	...
	if(is_ws == "0"){
		...
		$("#sbt_login").click(function(){ 
			...
			$.ajax({
				type: "POST",
				url:"${ctx}/member/center/EcRebates.do",
				...
				error: function (xhr, ajaxOptions, thrownError) {alert('网络异常，请稍后再试。。。');document.getElementById("sbt_login").value="提交";document.getElementById("sbt_login").disabled=false; 
					...
					if(result=='1'){ 
						...
						alert('银行卡信息维护成功');
						flag = true;
						location.href="${ctx}/member/center/EcRebates.do?method=save&type="+type+"&id="+id;  
					}else{ 
