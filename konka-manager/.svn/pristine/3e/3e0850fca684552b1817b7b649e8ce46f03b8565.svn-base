...
<table width="100%">
	<tr>
		<td width="50%" valign="top" ><!-- valign="top" style="padding:55px 0 0 0;"-->
			<table width="100%" border="0" cellpadding="0" cellspacing="1" class="tableClass">
				<tr>
					<th align="center" width="20%"><c:if test="${af.map.tj_type eq 1}">日期</c:if><c:if test="${af.map.tj_type eq 2}">月份</c:if></th>
				...
				<tr>
					<td align="center"><a href="JxcXsReport.do?method=list_xs&mod_id=${af.map.mod_id}&flag_type=${af.map.flag_type}&opr_date=${fn:substring(cur[0], 0, 10)}" style="color:#0072bc;">${fn:substring(cur[0], 0, 10)}</a></td>
					<td align="right">${cur[1]}</td>
					<td align="right"><span class="kz-price-12"><fmt:formatNumber value="${cur[2]}" type="currency" /></span></td>
				...
				<c:set var="total_num" value="${total_num+cur[1]}" />
				<c:set var="total_money" value="${total_money+cur[2]}" />
				<c:set var="total_cost" value="${total_cost+cur[3]}" />
				<c:set var="total_profit" value="${total_profit+cur[4]}" />
				</c:forEach>
				<tr>
...
if(${not empty entityList}){
   ...
   chart.setDataURL("${ctx}/customer/manager/JxcXsReport.do?method=xs_time_chart");		
