                ...
                <table width="100%" border="0" cellpadding="0" cellspacing="0" class="rtable2">
                  ...
                  <c:forEach var="cur" items="${entityList}" varStatus="vs">
                    <tr id="tr_${cur.shop_id}">
                      ...
                      <td nowrap="nowrap" align="left"><a href="<c:url value='/manager/admin/EntpShopSearch5W.do?method=view&shop_id=${cur.shop_id}&mod_id=${af.map.mod_id}'/>" style="text-decoration:none;"><font class="blue12px">${cur.shop_name}</font></a></td>
                      <td align="center">${cur.link_user}</td>
                      <td nowrap="nowrap" align="center">${cur.link_phone}</td>
                      <td align="center"><span style="cursor:pointer;"  class="fblue" onclick="sellAnalysisSubmit('bottomPageForm','page_id=EntpShopSearch5W&shop_id=${cur.shop_id}&type=1');">经营情况</span>
                      |
                      <c:if test="${empty cur.map.shop_develop_status}">
                      ...
                      <c:if test="${cur.map.shop_develop_status eq 1}">
                       ...
                       <!-- 
                       <span style="cursor:pointer;" onclick="doNeedMethod(null, 'EntpShopSearch5W.do', 'add','shop_id=${cur.shop_id}&'+$('#bottomPageForm').serialize())">开拓</span> |
                    ...
                    <form id="bottomPageForm" name="bottomPageForm" method="post" action="EntpShopSearch5W.do">
