	...
	<div class="topsearch">
		<form method="post" action="<c:url value='/webservice/wap/KonkaBcompPd.do' />" id="search_form_top">
		    <table style="width:100%">
			    <tr>
			    <td><div class="searcharea"><input name="pd_sn_or_pd_name_like" id="pd_sn_or_pd_name_like" placeholder="商品搜索" class="input_search" type="text" maxlength="20"  /></div></td>
			    <td class="padl5" width="60">
			    <a href="javascript:void(0);" id="search_btn_sub_top" class="new-tbl-cell" onclick="document.getElementById('search_form_top').submit();"><span class="icon2">搜索</span></a>
...
<form action="<c:url value='/webservice/wap/ShoppingCar.do' />" id="form_pd_buy" method="post">
...
<div class="mainbox">
    ...
    <div class="in" style="font-size:14px;">${konkaBcompPd.pd_name}</div>
    <div class="in" style="font-size:20px;"><strong style="color:red;" id="price_tag">￥<fmt:formatNumber value="${konkaBcompPd.ecGoodsPrice.price}" pattern="#,#00" /></strong><img id="price_tag_loading" style="display:none;" src="${ctx}/images/loading5.gif" /></div>
    <div class="in" style="color: #666666;text-decoration: line-through;font-size: 12px;">市 场 价：<font class="midtxt" id="original_price_tag">￥<fmt:formatNumber value="${konkaBcompPd.ecGoodsPrice.original_price}" pattern="#,#00" /></font><img id="original_price_tag_loading" style="display:none;" src="${ctx}/images/loading5.gif" />
    			<script>
					window._bd_share_config = {
						common : {
							bdText : '${konkaBcompPd.pd_sn}',bdDesc : '${konkaBcompPd.pd_name}',
							bdUrl : 'http://zx.konka.com/webservice/wap/PdShow.do?goods_id=${konkaBcompPd.id}', 	
   ...
   <div class="out2" style="margin-top:8px; background:#FFF;" onclick="location.href='${ctx}/webservice/wap/EcPdEavl.do?goods_id=${konkaBcompPd.id}';">
   ...
   <div class="out2" style="margin-top:8px; background:#FFF;" onclick="location.href='${ctx}/webservice/wap/PdContent.do?goods_id=${konkaBcompPd.id}&type=1';">
   ...
   <div class="out2" style="margin-top:8px; background:#FFF;" onclick="location.href='${ctx}/webservice/wap/PdContent.do?goods_id=${konkaBcompPd.id}&type=2';">
   ...
   </div><c:if test="${not empty konkaBcompPd.ecBindingPdListForService}">
   <div class="out2" style="margin-top:8px;margin-bottom:10px;background:#FFF;">
...
<div class="qb_flex" style="width:90%;position: fixed; left:5%;bottom: 0px; z-index: 10; display: table; text-align:center; "><input style="margin-right:6%;" type="button" id="add-cart" value="加入购物车" class="mod_btn btn_block btn_em flex_box" onclick="addShopingCar();"/><input type="button" id="buy-now" value="立即购买" class="mod_btn btn_block btn_strong flex_box" onclick="buy();" /></div> 
...
function getProudct(p_index,obj){
	...
	document.getElementById("original_price_tag").innerHTML=""; 
	showObj("price_tag_loading");
	showObj("original_price_tag_loading");
	//hideObj("stock");
	showObj("stocks_loading");   
	
	url="<c:url value='/webservice/wap/PdShow.do' />";
	     ...
	     if(xmlHttp.readyState == 4){
	         if(xmlHttp.status == 200){ 
	             ...
	             var data=eval("["+xmlHttp.responseText+"]");//获取json  
	        	   ...
	        	   document.getElementById("original_price_tag").innerHTML="￥"+data[0].original_price; 
	        	   ...
	        	   document.getElementById("had_stock_num").value = 0; 
	        		...
	        		hideObj("original_price_tag_loading");
...
function showObj(objId){$("#"+objId).show();//document.getElementById(objId).style.display="block";
}
function hideObj(objId){$("#"+objId).hide();//document.getElementById(objId).style.display="none";
}
