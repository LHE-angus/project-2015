...
public class QRCodeForPdServlet extends HttpServlet {

	protected static final Logger logger = LoggerFactory.getLogger(QRCodeForPdServlet.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException,
		...
		// 查询型号ID
		if (str.split(",,")[5].equals("") || str.split(",,")[5] == null) {
			String md_name = str.split(",,")[6];// 型号ID
			PePdModel pePdModel = new PePdModel();
			pePdModel.setMd_name(md_name);
			List<PePdModel> pePdModelList = facade.getPePdModelService().getPePdModelList(pePdModel);
			if (pePdModelList.size() == 1) {
				str = str.replace(",," + str.split(",,")[6], pePdModelList.get(0).getPd_id().toString() + ",,"
						+ str.split(",,")[6]);
			}
		}
