...
<body style="font-family:Microsoft Yahei;">
	...
	<div class="rtabcont1">
	  <table width="100%" border="0" cellpadding="0" cellspacing="1" class="tableClass">
	    ...
	    <c:forEach items="${entityList}" var="cur" varStatus="vs">
	      <tr>
	        ...
	        <td align="center">
	        	<c:if test="${cur.sell_type eq 1}">手机零售通</c:if>
	        	<c:if test="${cur.sell_type eq 2}">进销存销售</c:if>	
	        	<c:if test="${cur.sell_type eq 3}">专卖店</c:if>	
	        	<c:if test="${cur.sell_type eq 5}">盘亏</c:if>	
	        	<c:if test="${cur.sell_type eq 6}">进销存进货退货</c:if>	
	        	<c:if test="${cur.sell_type eq 7}">R3订单退货</c:if>	
	        </td>
	        <td align="left" nowrap="nowrap">${cur.md_name}</td>
	        <td align="right" nowrap="nowrap"><fmt:formatNumber value="${cur.sell_price}" pattern="0.00" /></td>
	        ...
	        <td align="center"><fmt:formatDate value="${cur.sell_date}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
	      </tr>
	      <c:if test="${vs.last}">
	  ...
	  <c:if test="${not empty entityList}">
	    <div class="rtabcont3">
	      <form id="bottomPageForm" name="bottomPageForm" method="post" action="JsSells.do">
