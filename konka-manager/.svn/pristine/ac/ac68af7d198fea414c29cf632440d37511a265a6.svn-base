        ...
        <tr>
          <td height="36" align="left" style="padding-left:5px;">
           &nbsp;<strong class="fb">年月：</strong>
            ...
            &nbsp;&nbsp;&nbsp;
            <input name="button" type="button" class="bgSearch" id="sear_btn" value="搜 索" />
            &nbsp;<input name="but_exl" type="button" class="but_excel" id="but_exl" value="导出" /></td>
        </tr>
      </table>
    </div>
  </html-el:form>
  <h1 align="center" style="padding-top: 20px;font-size: 22px;">${af.map.year}年${af.map.month}月份各分公司平板销售评估表</h1>
  ...
  <div class="rtabcont1" style="overflow-x: auto;">
    <table class="rtable6 datatable" width="100%" cellspacing="1" cellpadding="0" border="0">
      ...
      <c:forEach var="cur" items="${entityList}" varStatus="vs">
        <tr>
          <td nowrap="nowrap" align="left"><a href="${ctx}/manager/admin/KonkaR3DeptStockInfoToFgsTop.do?method=detail&v_class=${cur.class1}&year=${af.map.year}&month=${af.map.month}"><span style="color: blue;">${cur.class1}</span></a></td>
        ...
        <tr>
          <td nowrap="nowrap" colspan="1" align="left">合计</td>
          
          <td nowrap="nowrap" align="right"><fmt:formatNumber value="${v_pdlabst}" pattern="###,##0" /></td>
          <td nowrap="nowrap" align="right"><fmt:formatNumber value="${v_pclabst}" pattern="###,##0" /></td>
          <td nowrap="nowrap" align="right"><fmt:formatNumber value="${v_zzlabst}" pattern="###,##0" /></td>
          <td nowrap="nowrap" align="right"><fmt:formatNumber value="${v_bclabst}" pattern="###,##0" /></td>
          <td nowrap="nowrap" align="right"><fmt:formatNumber value="${v_jylabst}" pattern="###,##0" /></td>
          
          <td nowrap="nowrap" align="right"><fmt:formatNumber value="${v_yjlabst}" pattern="###,##0" /></td>
          <td nowrap="nowrap" align="right"><fmt:formatNumber value="${v_cllabst}" pattern="###,##0" /></td>
          <td nowrap="nowrap" align="right"><fmt:formatNumber value="${v_tjlabst}" pattern="###,##0" /></td>
          <td nowrap="nowrap" align="right"><fmt:formatNumber value="${v_zclabst}" pattern="###,##0" /></td>
          
          <td nowrap="nowrap" align="right"><fmt:formatNumber value="${v_dzlabst}" pattern="###,##0" /></td>
          <td nowrap="nowrap" align="right"><fmt:formatNumber value="${v_mzt}" pattern="###,##0" /></td>
          <td nowrap="nowrap" align="right"><fmt:formatNumber value="${v_mwf}" pattern="###,##0" /></td>
          <td nowrap="nowrap" align="right"><fmt:formatNumber value="${v_sum}" pattern="###,##0" /></td>
          
          <td nowrap="nowrap" align="right"><fmt:formatNumber value="${v_lfimg}" pattern="###,##0" /></td>
          <td nowrap="nowrap" align="right"><fmt:formatNumber value="${v_lfimg1}" pattern="###,##0" /></td>
          <td nowrap="nowrap" align="right" class="bg_y"><fmt:formatNumber value="${v_zzl}" pattern="###,##0.00" /></td>
        </tr>
    </table>
  </div>
</div>
...
$(document).ready(function(){
	$("#but_exl").click(function(){
		$("#mod_id").append("<input type='hidden' name='excel_all' value='1' id='excel_all' />");
		this.form.submit();
	});
