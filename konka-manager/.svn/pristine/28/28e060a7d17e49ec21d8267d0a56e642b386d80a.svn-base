...
<div style="width:100%">
  ...
  <div class="rtabcont1">
    <form action="<c:url value='/jxcnokey/JxcJnhmSellBill.do'/>" method="post">
      ...
      <c:forEach items="${sellBillList}" var="cur" varStatus="status">
        <tr>
          ...
          <td align="center" ><c:choose>
          	<c:when test="${cur.map.states ne 1}">待审核</c:when>
          	<c:when test="${cur.map.states eq 1}">审核通过</c:when>
          </c:choose></td>
          <td align="left" >${fn:escapeXml(cur.map.audit_remarks)}</td>
          <!-- <td align="center" >${fn:escapeXml(cur.remarks)}</td> -->
          <td align="center" >
            <c:if test="${cur.map.states ne 1}" >
            	<a href="#" onclick="go('${ctx}/jxcnokey/JxcJnhmSellBill.do?method=edit&sell_bill_id=${cur.id}')"><span style="color:#060;">修改订单</span></a>
            	<span style="color:#ccc;">打印补贴确认单</span>
            </c:if>
          	<c:if test="${cur.map.states eq 1}" >
          		<span style="color:#ccc;">修改订单</span>
