(function($) {
	...
	$.fn.tab.prototype = {
			...
			if ($(_this.opts.tabList).length > 0) {
				$(_this.opts.tabList)
						.each(
								function(index) {
									$(this)
											.bind(_this.opts.eventType,
													function() { //判断是否禁用，是否效果还在执行中，是否在当前选中的按钮上                                                         
														if ($
																.inArray(
																		index,
																		_this.disableArr) == -1
																...
																&& $(this)
																		.attr(
																				"class")
																		.indexOf(
																				_this.opts.tabActiveClass) == -1) { //callback                                                                 
															if (_this.opts.callBackStartEvent) {
																_this.opts
																		.callBackStartEvent(index);
															}
															$(
																	_this.opts.tabList)
																	.removeClass(
																			_this.opts.tabActiveClass);
															$(this)
																	.addClass(
																			_this.opts.tabActiveClass);
															_this
		...
		_showContent : function(index) {
			...
			case "show":
				$(_this.opts.contentList + ":visible").hide(); //callback                                         
	...
	case "fade":
		$(_this.opts.contentList + ":visible").fadeOut(_this.opts.showSpeed,
				function() { //callback
					...
					$(_this.opts.contentList).eq(index).fadeIn(function() { //callback   
								if (_this.opts.callBackShowEvent) {
									_this.opts.callBackShowEvent(index);
								}
								isShow = false;
	...
	case "slide":
		$(_this.opts.contentList + ":visible").slideUp(_this.opts.showSpeed,
				function() { //callback 
					...
					$(_this.opts.contentList).eq(index).slideDown(function() { //callback 
								if (_this.opts.callBackShowEvent) {
									_this.opts.callBackShowEvent(index);
								}
								isShow = false;
...
setDisable : function(index) { //如果不存在
			var _this = this;
			if ($.inArray(index, this.disableArr) == -1) {
				this.disableArr.push(index);
				$(_this.opts.tabList).eq(index).addClass(
						_this.opts.tabDisableClass);
			}
		},
		setEnable : function(index) { //如果不存在
			var _this = this;
			var i = $.inArray(index, this.disableArr);
			if (i > -1) {
				this.disableArr.splice(i, 1);
				$(_this.opts.tabList).eq(index).removeClass(
						_this.opts.tabDisableClass);
			}
		},
