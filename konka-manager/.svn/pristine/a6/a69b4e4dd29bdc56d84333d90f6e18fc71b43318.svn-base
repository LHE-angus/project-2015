...
<!-- top end --> 
<jsp:include page="/touch/__inc/slider.jsp" flush="true" /> 
<!-- first end -->
<!-- second start -->
  ...
  <div class="listleft">
        ...
        <ul class="listul4">
          <c:forEach items="${bcomp_pd_list_top_5}" var="cur" varStatus="vs">
            ...
            <li style="padding:0;" class="${li_class}">
              <div class="listpic1"><table border="0" cellspacing="0" cellpadding="0" width="120"><tr><td height="110" style="overflow:hidden;" width="120" valign="middle" align="center"><a href="<c:url value='/touch/PdShow.do?goods_id=${cur.id}' />"><img src="${ctx}/${fn:substringBefore(cur.main_pic, '.')}_120.jpg" style="max-height:100px;" /></a></td></tr></table></div>
              <div class="listrxt1">
                <h3><a href="<c:url value='/touch/PdShow.do?goods_id=${cur.id}' />">康佳&nbsp;${fnx:abbreviate(cur.pd_sn, 2 * 7, '.')}&nbsp;${fn:escapeXml(cur.pd_size)}英寸</a></h3>
                <h4>价格：￥
    ...
    <c:if test="${not empty ecUserFavotriesList}">
     <div class="liscont1 margintop10">
            ...
            <li><div class="listpic1"><a href="${ctx }/touch/PdShow.do?goods_id=${cur.goods_id }"><img src="${ctx}/${cur.img_url }" width="118"  height="81"/></a></div><div class="listrxt1"><h3><a href="${ctx }/touch/PdShow.do?goods_id=${cur.goods_id }" ><c:out value="${cur.md_name }"/></a></h3><h4>关注价格：￥${cur.price }</h4></div><div class="clear"></div></li>
             </c:forEach>
            </ul>
            </div>
     </div></c:if>
   ...
   <!--right-->
    <div class="listright"><div class="position"><a href="${ctx }/memer/Index.do">首页</a> &gt; <a href="${ctx }/touch/ArticleInfo.do">内购咨询</a> </div>
      <div class="news-box">
        <div class="box_list">
        	<ul>
	          <c:forEach items="${entityList}" var="cur"> 
	          <li class="border"><span class="r grayfont"><fmt:formatDate value="${cur.pub_date}" pattern="yyyy/MM/dd" /></span><a href="${ctx }/touch/ArticleInfo.do?method=view&id=${cur.id}" target="_blank" title="${cur.title}">${fnx:abbreviate(cur.title, 2 * 45, '...')}</a></li>
	          </c:forEach>
         	</ul>
