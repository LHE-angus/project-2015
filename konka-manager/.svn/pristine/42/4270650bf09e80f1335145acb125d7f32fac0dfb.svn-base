  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="1" class="tableClass">
    ...
    <c:forEach items="${konkaOrderInfoList}" var="cur" varStatus="status">
      <tr>
        ...
        <td align="center"><span style="cursor:pointer;"  class="fblue"  onclick="doNeedMethod(null, 'JxcKonkaOrderRegister.do', 'view','&keySeq=${af.map.keySeq}&order_id=${cur.id}&'+$('#bottomPageForm').serialize())">查看明细</span>
...
<script type="text/javascript">//<![CDATA[
	$(document).ready(function(){
		$("#gotoAdd").click(function(){
			window.location.href = "${ctx}/jxcnokey/JxcKonkaOrderRegister.do?method=add&keySeq=${af.map.keySeq}";
		})
