 ...
 *         epp开心猫filter
 */
public class SimpleAuthenticateFilter4Member extends GenericFilterBean {
	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain chain)
		...
		if (null != acSession) {
			EcUser eu = (EcUser) session.getAttribute(Constants.EPP_USER);
			EcUser ac_eu = (EcUser) acSession.getAttribute(Constants.EPP_USER);
		...
		if(ecUser.getUser_type().intValue()==1&&ecUser.getIs_act()!=null&&ecUser.getIs_act().intValue()!=0){  
			if(requestUrl.indexOf("RegUser.do")==-1&&requestUrl.indexOf("KonkaGroupPeArticleInfo")==-1){
				if(requestUrl.indexOf("/member/")!=-1){
					response.sendRedirect(contextPath + "/member/center/RegUser.do?"); 
					return;
				}else if(requestUrl.indexOf("/epp/mobile/")!=-1){
					response.sendRedirect(contextPath + "/epp/mobile/center/RegUser.do?"); 
					return;
				}else if(requestUrl.indexOf("/wap/")!=-1){
					response.sendRedirect(contextPath + "/wap/center/RegUser.do?"); 
					return;
				}
			} 
		}
