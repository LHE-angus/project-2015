...
<jsp:include page="/sfmall/__inc/_mleft.jsp" flush="true" />
    ...
    <div class="sfmall_right padbot45">
   	  <div class="position"><a href="${ctx }/sfmall/Index.do">首页</a> &gt; 会员中心</div>
        <div class="sfmalltab2">
        	<h3>亲爱的<span style="font-size:20px;color: red;"> ${ecUser.real_name }</span> 欢迎您来到康佳&顺丰直销商城！上一次登录时间：<fmt:formatDate value="${ecUser.last_login_time}" pattern="yyyy-MM-dd HH:mm:ss" /></h3>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="sfmall_table1">
        <tbody>
        <tr>
        	<td width="34%"><h3 class="sfmall_tdh3">订单提醒：</h3><h4 class="sfmall_tdh4"><a href="Orders.do">待处理订单(${pShowOrderNum})</a></h4></td> 
         	<td><c:if test="${sfmall eq 1 }"><h3 class="sfmall_tdh3">我的积分：</h3><h4 class="sfmall_tdh4"><a href="${ctx }/sfmall/EcGift.do"> ${totalScore}分</a></h4></c:if></td>
        </tr>
        </tbody>
        </table>
        </div>
        <ul class="sfmalltit3"><li class="curli" id="lia1"><a href="JavaScript:void(0);">限时抢购</a></li><li id="lia2"><a  href="JavaScript:void(0);">您关注的商品</a></li></ul>
        <div class="sfmalltab4">
        <ul class="sfmallul2" id="ula1">
        <c:forEach items="${bcomp_pd_list_5}" var="cur" varStatus="vs">
	        <li>
		        <div class="sfmalllipic">
		        	<a href="<c:url value='/sfmall/PshowOrderPanicBuying.do?method=add&goods_id=${cur.id}' />"><img alt="${cur.pd_name}" src="${ctx}/${fn:substringBefore(cur.main_pic, '.')}_095.jpg" /></a>
		        </div>
		        <h3 class="sfmalllitit"><a href="<c:url value='/sfmall/PshowOrderPanicBuying.do?method=add&goods_id=${cur.id}' />" >${cur.pd_sn} </a> </h3>
		        <h3 class="sfmalllitit">原 价：<font class="midtxt">￥<fmt:formatNumber value="${cur.ecGoodsPrice.original_price}" pattern="0" /></font> </h3>
		        <h3 class="sfmalllitit">抢购价：<a class="fontblue14" href="<c:url value='/sfmall/PshowOrderPanicBuying.do?goods_id=${cur.id}' />" >￥<fmt:formatNumber value="${cur.ecGoodsPrice.price}" pattern="0" /></a></h3>
	        </li>
        </c:forEach>
        </ul>
        <ul class="sfmallul2" id="ula2" style="display:none;">
         <c:forEach var="cur" items="${ecUserFavotriesList}" varStatus="vs"> 
	        <li><div class="sfmalllipic"><a href="${ctx }/sfmall/PdShow.do?goods_id=${cur.goods_id }">
	         <img src="${ctx}/${fn:substringBefore(cur.img_url, '.')}_095.jpg" /></a></div>
	         <h3 class="sfmalllitit"><a href="${ctx }/sfmall/PdShow.do?goods_id=${cur.goods_id }"><c:out value="${cur.md_name }" /></a></h3>
	         <h3 class="sfmalllitit">关注价格：<font class="fontblue14">￥${cur.price }</font></h3> 
	        </li>
         </c:forEach>
         <c:if test="${empty ecUserFavotriesList }">
         <li style="width:100%;font-size:14px;height:40px;text-align:center"> 您还没有关注任何商品,点击<a href="${ctx }/sfmall/Index.do"><font color="#2233ff">这里</font></a>去关注您喜爱的商品吧 </li>
         </c:if>
        </ul>
        </div>
        <ul class="sfmalltit3"><li class="curli" id="lib1"><a class="redfont" href="JavaScript:void(0);">猜你喜欢</a></li><li id="lib2"><a href="JavaScript:void(0);">近期订单</a></li></ul>
        <div id="ulb1">
        <div class="sfmalltab4" >
        <i class="to_left">
			<a id="butPrevGroup" href="javascript:void(0);"><img src="${ctx }/styles/sfmall/images/arrow_left.gif" /></a>
		</i>
        <div class="sfmallulbox3" >
        <ul class="sfmallul3" id="hotPd" >            
	        <c:forEach items="${bcomp_pd_list_2}" var="cur" varStatus="vs">
	        <li id="hotPd${vs.count}" <c:if test="${vs.count>6 }">style="display:none"</c:if>>
		        <div class="sfmalllipic">
		        	<a href="<c:url value='/sfmall/PdShow.do?goods_id=${cur.id}' />" title="康佳${cur.pd_sn}&nbsp;${cur.pd_name}"> <img alt="${cur.pd_name}" src="${ctx}/${fn:substringBefore(cur.main_pic, '.')}_095.jpg" align="absmiddle" /> </a>
		        </div>
		        <h3 class="sfmalllitit"><a href="<c:url value='/sfmall/PdShow.do?goods_id=${cur.id}' />" title="${cur.pd_name}" >${fnx:abbreviate_si(fn:escapeXml(cur.pd_name), 24)}</a></h3>
		        <h3 class="sfmalllitit">会员价：<a class="fontblue14" href="<c:url value='/sfmall/PdShow.do?goods_id=${cur.id}' />">￥<c:if test="${ecUser.user_type eq 1 }"><fmt:formatNumber value="${cur.ecGoodsPrice.price}" pattern="0" /></c:if>
        ...
        <i class="to_right">
			<a id="butNextGroup" href="javascript:void(0);"><img src="${ctx }/styles/sfmall/images/arrow_right.gif" /></a>
		</i> 
        </div>
        </div>
        <div id="ulb2" style="display:none;"><br/>
        <table width="99%" border="0" cellspacing="0" cellpadding="0" class="sfmall_form_table2">
	        <tr class="tr1">
        ...
        <c:forEach var="cur" items="${pShowOrderList}" varStatus="vs">
            <tr height="60">
              ...
              <td align="center" nowrap="nowrap"><c:forEach items="${cur.pshowOrdeDetailsList}" var="cur2">
                  ...
                  <a href="${ctx }/sfmall/PdShow.do?goods_id=${cur2.pd_id}"><c:forEach items="${imgs }" var="img1" begin="0" end="0"><img src="${ctx }/${img1}" alt="${cur2.map.pd_name }" width="51" height="40"/></c:forEach></a>
                  <br/>
                </c:forEach>
              </td>
              ...
              <td align="center" nowrap="nowrap" style="line-height:23px;">
                ...
                <c:if test="${cur.state eq 0 }"><a class="btn btn-4" href="<c:url value='/sfmall/Payment.do?trade_index=${cur.trade_index}' />"><s></s>付款</a><br/> <a href="#" id="t${cur.id }" onclick="deleteInfo(this);"><font color="#0022ff">取消订单</font></a></c:if>
                <c:if test="${cur.state eq 10 }"><a href="Orders.do?method=view&id=${cur.id}"><font color="#0022ff">订单详情</font></a> </c:if>
                <c:if test="${cur.state eq 20 }"><a href="Orders.do?method=view&id=${cur.id}"><font color="#0022ff">订单详情</font></a> </c:if>
                <c:if test="${cur.state eq 30 }"><a href="Orders.do?method=view&id=${cur.id}"><font color="#0022ff">订单详情</font></a></c:if>
