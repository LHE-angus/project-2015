...
<br/>
  <div class="position"><a href="<c:url value='/member/Index.do' />">首页</a> &gt; <c:if test="${fn:contains(konkaBcompPd.pd_name, 'HYUNDAI')}">HYUNDAI</c:if><c:if test="${fn:contains(konkaBcompPd.pd_name, '戴尔')}">戴尔</c:if><c:if test="${not fn:contains(konkaBcompPd.pd_name, 'HYUNDAI') and not fn:contains(konkaBcompPd.pd_name, '戴尔') and konkaBcompPd.prod_type eq 0}">康佳</c:if>&nbsp;<c:out value="${konkaBcompPd.pd_sn}" /></div>
  <div class="viewbox1" style="min-height:450px;">
    ...
    <div class="viewright">
      <div class="viewtit1"><h3> 
      			<c:if test="${fn:contains(konkaBcompPd.pd_name, 'HYUNDAI')}">韩国现代 HYUNDAI</c:if>   
      			<c:if test="${fn:contains(konkaBcompPd.pd_name, '戴尔')}">戴尔</c:if> 
                <c:if test="${not fn:contains(konkaBcompPd.pd_name, 'HYUNDAI') and not fn:contains(konkaBcompPd.pd_name, '戴尔') and konkaBcompPd.prod_type eq 0}">康佳</c:if>&nbsp;<c:out value="${konkaBcompPd.pd_sn}" /> &nbsp;<span style="color: #d40207"><c:if test="${konkaBcompPd.label_of_cate eq 0}">新品</c:if><c:if test="${konkaBcompPd.label_of_cate eq 2}">热销</c:if><c:if test="${konkaBcompPd.label_of_cate eq 3}">特惠</c:if><c:if test="${konkaBcompPd.label_of_cate eq 7}">精品</c:if></span></h3>
      ...
      <div class="viewtab1">
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="proxltab">
        	<c:if test="${ecUser.user_type eq 1 }">
             <tr>
              <td colspan="3" height="22">会 员 价：<strong class="yel16b redfont" id="price_tag">￥
            ...
            </tr> </c:if><c:if test="${ecUser.user_type eq 2 }">
            <tr>
              <td colspan="3" height="22">市场终端价：<font class="yel16b redfont" id="original_price_tag">￥
              <c:if test="${konkaBcompPd.ecGoodsPrice.original_price gt 0}">
              <fmt:formatNumber value="${konkaBcompPd.ecGoodsPrice.original_price}" pattern="#,##0" />
              </c:if>
              <c:if test="${konkaBcompPd.ecGoodsPrice.original_price lt 0.01}">
              <fmt:formatNumber value="${konkaBcompPd.ecGoodsPrice.original_price}" pattern="#0" /> 
              </c:if>
              </font><img id="original_price_tag_loading" style="display:none;" src="${ctx}/images/loading5.gif" /></td>
            </tr>
            </c:if>
            <c:if test="${ecUser.user_type eq 1 and touch eq 1 and konkaBcompPd.prod_type ne 9 and konkaBcompPd.prod_type ne 22}">  
            <tr><c:if test="${not empty konkaBcompPd.integral and konkaBcompPd.integral >1}">
              ...
              <td colspan="3" height="22">积 　  分：付款积分
              <c:if test="${konkaBcompPd.ecGoodsPrice.price gt 0}">
              <fmt:formatNumber value="${konkaBcompPd.ecGoodsPrice.price}" pattern="#,##0" />
              </c:if>
              <c:if test="${konkaBcompPd.ecGoodsPrice.price lt 0.001}">
              <fmt:formatNumber value="${konkaBcompPd.ecGoodsPrice.price}" pattern="#0" />
               ...
               , 奖励积分
               <c:if test="${integral gt 0}">
               <fmt:formatNumber value="${integral}" pattern="#,##0" />
               </c:if>
               <c:if test="${integral lt 0.001}">   
               <fmt:formatNumber value="${integral}" pattern="#0" />   
               </c:if>   
               </td>
            </tr>
            ...
            </tr></c:if><c:if test="${ecUser.user_type eq 1 and touch eq 1 and konkaBcompPd.prod_type eq 9 and konkaBcompPd.prod_type eq 22}">
            <tr>
              	<td colspan="3" height="22">积 　  分：付款积分 0, 奖励积分 0  </td>
            </tr>
            <tr>
              	<td colspan="3" height="22">佣 　  金：￥0  </td>
            </tr></c:if>
            <tr>
      ...
      <div class="viewtab2">
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="proxltab">
          <tbody>
            ...
            <tr>
              <td colspan="2"><div class="padtop20"><img alt="加入购物车" src="${ctx}/styles/member/images/but_joinshop.gif" width="132" height="38" style="cursor:pointer;" id="add_shoping_car_btn" />&nbsp;&nbsp;<img alt="立即购买" src="${ctx}/styles/member/images/but_buy2.gif" style="cursor:pointer;" id="buy_btn" /></a>&nbsp;&nbsp;<img alt="关注" style="cursor:pointer;" id="add_favotrites_btn" src="${ctx}/styles/member/images/but_attention.gif" /></a></div></td>
            </tr>
          </tbody>
        </table>
      </div>
	...
	<div class="right">
		...
		<div id="newFittign-tab" data-widget="tabs">
		  ...
		  <div class="tab-cat-content">
		   <form action="<c:url value='/member/ShoppingCar.do' />" id="form_pd_buy2" method="post">
		    <input type="hidden" name="method" value="buyByGoodsId" />
