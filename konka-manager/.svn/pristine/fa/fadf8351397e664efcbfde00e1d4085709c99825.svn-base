...
public class ShoppingCarAction extends BaseMemberAction {
	...
	public ActionForward list(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		return new ActionForward("/../member/ShoppingCar/step_one.jsp");
	}
	...
	public ActionForward buyByGoodsId(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		String tuangou = (String) dynaBean.get("tuangou");
		...
		if (tuangou != null && "6".equals(tuangou)) {
			// 取上下架时间
			...
			if (null != ecList && ecList.size() > 0) {
				...
				if (ecg.getStart_time().getTime() > now_date) {
					super.renderJavaScript(response, "window.onload=function(){alert('对不起，还没到团购时间！');location.href='"
					        + super.getCtxPath(request) + "/member/PdShowYd.do?goods_id=" + goods_id + "';}");
					return null;
				}
		...
		if (null != ecgList && ecgList.size() > 0) {
			...
			if (null != egList && egList.size() > 0) {// 根据当前用户的pindex找到所在的省份
				BaseProvinceListFour bf = new BaseProvinceListFour();
				bf.setP_level(1);
				bf.getMap().put("pindex", p_index);
				List<BaseProvinceListFour> bfList = super.getFacade().getBaseProvinceListFourService()
		...
		if (tuangou != null && "6".equals(tuangou)) {// 团购商品不加入购物车
	...
	public ActionForward stepTwo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		return new ActionForward("/../member/ShoppingCar/step_two.jsp");
	}
	...
	public ActionForward createOrder(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if ("6".equals(tuangou)) {// 团购订单
			pshowOrder.setOrder_type(6);
		}
	...
	public ActionForward checkPayPwd(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if ("1".equals(show)) {
			...
			return new ActionForward("/../member/ShoppingCar/checkpwd.jsp");
		}
