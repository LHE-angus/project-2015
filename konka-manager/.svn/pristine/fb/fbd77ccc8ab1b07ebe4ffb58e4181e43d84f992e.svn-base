...
<div class="maincont margintop10">
  ...
  <div class="listleft">
   ...
   <!--right-->
    <div class="listright"><div class="position"><a href="${ctx }/memer/Index.do">首页</a> &gt; <a href="${ctx }/zxmall/ArticleInfo.do">内购咨询</a> </div>
      <div class="news-box">
        <div class="box_list">
        	<ul>
	          <c:forEach items="${entityList}" var="cur"> 
	          <li class="border"><span class="r grayfont"><fmt:formatDate value="${cur.pub_date}" pattern="yyyy/MM/dd" /></span><a href="${ctx }/zxmall/ArticleInfo.do?method=view&id=${cur.id}" target="_blank" title="${cur.title}">${fnx:abbreviate(cur.title, 2 * 45, '...')}</a></li>
	          </c:forEach>
         	</ul>
