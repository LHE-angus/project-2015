...
<link rel="stylesheet" type="text/css" href="${ctx}/styles/wap/css/shoping.css" /> 
<script type="text/javascript" src="${ctx}/commons/scripts/jquery-1.6.4.min.js"></script>  
</head>
<body onload="checkSelect();"> 
	<div class="mainbox">
		<div class="maincont"><c:if test="${not empty konkaBcompPdList }">
		<form action="<c:url value='/wap/ShoppingCar.do' />" method="post" id="shopping_car_form">
		...
		<div class="shoppingcart"  id="product_${cur.id}">
		    ...
		    <div class="shoppingcart_r">
		      <div class="shoppingcart_name">康佳 ${fnx:abbreviate(cur.pd_name, 2 * 34, '...')} </div>
		...
		</form></c:if><c:if test="${empty konkaBcompPdList }">
		<table><tr><td align="center"><img src="${ctx}/styles/wap/images/cart_qugouwu.png" width="330" /></td></tr></table>
		</c:if>
		</div> 
	<div class="clear"></div>
	</div> <c:if test="${not empty konkaBcompPdList }">
...
</c:if><script type="text/javascript">//<![CDATA[
var xmlHttp; 
var url;
