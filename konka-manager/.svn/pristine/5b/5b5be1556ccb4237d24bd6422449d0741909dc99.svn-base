...
public class EcBackPasswordAction extends BaseAction {
	...
	public ActionForward nextStep(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (StringUtils.isBlank(link_phone) || StringUtils.isBlank(user_name) || StringUtils.isBlank(email)) {
		...
		if (null == entityList || entityList.size() == 0) {
			...
			String key = md5.md5(ss.toString() + user_name);
			...
			// mailSender, mailForm,
			// senderName, request);
			Long id = super.getFacade().getEcUserService().createEcUserAndSendEmail4(entity, super.getMailSender(),
