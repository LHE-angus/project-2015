...
<div class="maincont">
  ...
  <div class="zxmall_right padbot45">
    <div class="position"><a href="${ctx }/zxmall/Index.do">首页</a> &gt; <a href="${ctx }/zxmall/center/Index.do">会员中心</a> &gt; 我的下级订单</div>
    <div class="zxmalltab3">
      <html-el:form action="/center/OrdersForSub">
        <html-el:hidden property="method" styleId="method" value="list" />
        <html-el:hidden property="OrdersForSubtate" />
        ...
        <ul class="zxmalltit3">
          <c:if test="${touch eq 2 }">
          <li 
          <c:if test="${af.map.orderState eq '1' }">class="curli"</c:if> ><a href="OrdersForSub.do?method=list&orderState=1&ts=<c:out value='${af.map.ts}'/>">进行中</a>
          </li>
          </c:if>
          <li 
          <c:if test="${af.map.orderState eq '2' }">class="curli"</c:if> ><a href="OrdersForSub.do?method=list&orderState=2&ts=<c:out value='${af.map.ts}'/>">已完成</a>
          </li>
           <c:if test="${touch eq 2 }">
          <li 
          <c:if test="${af.map.orderState eq '3' }">class="curli"</c:if> ><a href="OrdersForSub.do?method=list&orderState=3&ts=<c:out value='${af.map.ts}'/>">已取消</a>
          </li>
          </c:if>
        </ul>
      </html-el:form>
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="zxmall_form_table2">
        ...
        <tbody>
          <c:forEach var="cur" items="${entityList}" varStatus="vs">
            <tr height="60">
              <td align="center" nowrap="nowrap"><a href="OrdersForSub.do?method=view&id=${cur.id}"><c:out value="${cur.trade_index}"/></a></td>
              ...
              <td align="center" nowrap="nowrap"><a href="OrdersForSub.do?method=view&id=${cur.id}"><c:if test="${cur.state eq -30 }">退货成功</c:if>
              ...
              <td align="center" nowrap="nowrap" style="line-height:23px;">
              	<c:if test="${cur.state eq -30 }"><a href="OrdersForSub.do?method=view&id=${cur.id}"><font color="#0022ff">订单详情</font></a></c:if>
                <c:if test="${cur.state eq -20 }"><a href="OrdersForSub.do?method=view&id=${cur.id}"><font color="#0022ff">订单详情</font></a></c:if>
                <c:if test="${cur.state eq -10 }"><a href="OrdersForSub.do?method=view&id=${cur.id}"><font color="#0022ff">订单详情</font></a></c:if>
                <c:if test="${cur.state eq 0 }"><a href="OrdersForSub.do?method=view&id=${cur.id}"><font color="#0022ff">订单详情</font></a><br/>
                <c:if test="${cur.pay_way ne 0 and cur.pay_way ne 1}"> <a class="btn btn-4" href="<c:url value='/zxmall/Payment.do?trade_index=${cur.trade_index}' />"><s></s>付款</a><br/></c:if>
                 <a href="#" id="t${cur.id }" onclick="deleteInfo(this);" ><font color="#0022ff">取消订单</font></a></c:if>
          		<c:if test="${cur.state eq 5 }"><a href="OrdersForSub.do?method=view&id=${cur.id}"><font color="#0022ff">订单详情</font></a> </c:if>
                <c:if test="${cur.state eq 10 }"><a href="OrdersForSub.do?method=view&id=${cur.id}"><font color="#0022ff">订单详情</font></a> </c:if>
                <c:if test="${cur.state eq 20 }"><a href="OrdersForSub.do?method=view&id=${cur.id}"><font color="#0022ff">订单详情</font></a> </c:if>
                <c:if test="${cur.state eq 30 }"><a href="OrdersForSub.do?method=view&id=${cur.id}"><font color="#0022ff">订单详情</font></a></c:if>
                <c:if test="${cur.state eq 40 }"><a href="OrdersForSub.do?method=view&id=${cur.id}"><font color="#0022ff">订单详情</font></a></c:if>
                <c:if test="${cur.state eq 50 }"><a href="OrdersForSub.do?method=view&id=${cur.id}"><font color="#0022ff">订单详情</font></a></c:if>
                <c:if test="${cur.state eq 60 }"><a href="OrdersForSub.do?method=view&id=${cur.id}"><font color="#0022ff">订单详情</font></a><br />
...
function deleteInfo(obj){
	...
	if(confirm("您确定取消订单吗？")){
		location.href="${ctx}/zxmall/center/OrdersForSub.do?method=delete"+id; 
	}
}

function getState(id){
	var returnValue = window.showModalDialog("OrdersForSub.do?method=sfList2&id="+id+"&azaz=" + Math.random(),window,"dialogWidth:700px;status:no;dialogHeight:300px");
