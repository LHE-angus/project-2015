 ...
 */
jQuery.fn.extend({
	pager2 : function(url, data, options, callback) {
		...
		var $this = this.extend(jQuery.extend({
			...
			pageSize2 : 10, // displayed data item number per page.
			...
			requestPageSizeAttr : 'pageSize2',
			requestPageFirstRowAttr : 'firstRow2',
		...
		$this.extend({
			...
			loadData : function() {
				if (null != url) {
					...
					requestUrl += $this.requestPageSizeAttr + "=" + $this.pageSize2 + "&";
					requestUrl += $this.requestPageFirstRowAttr + "=" + $this.firstRow2 + "&";
					...
					jQuery.ajax({
						...
						success: function(json) {
							...
							// callback
							callback(json, $this.firstRow2, $this.pageSize2);
						}
					});
				}
				return $this;
			...
			setCounts : function(counts) {
				...
				$this.pagesCount = $this.lastPage = Math.floor($this.counts / $this.pageSize2) + 1;
				return $this;
			},
			setCurrentPage : function(pn) {
				...
				$this.firstRow2 = $this.pageSize2 * ($this.currentPage - 1) + 1;
				return $this;
			},
