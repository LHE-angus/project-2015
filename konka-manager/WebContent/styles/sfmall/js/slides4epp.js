(function(d) {
	d.flexslider = function(i, k) {
		...
		e = c.namespace,
		p = "onsfmallstart" in window || window.DocumentTouch && document instanceof DocumentTouch,
		t = p ? "sfmallend": "click",
		...
		f = {
			init: function() {
				...
				p && c.sfmall && f.sfmall(); (!r || r && c.smoothHeight) && d(window).bind("resize focus", f.resize);
				setTimeout(function() {
					c.start(a)
				},
				200)
			...
			controlNav: {
				...
				setupPaging: function() {
					...
					p && a.controlNavScaffold.delegate("a", "click sfmallstart",
					function(a) {
						a.preventDefault()
					})
				},
				setupManual: function() {
					...
					p && a.controlNav.live("click sfmallstart",
					function(a) {
						a.preventDefault()
					})
				},
			...
			directionNav: {
				setup: function() {
					...
					p && a.directionNav.bind("click sfmallstart",
					function(a) {
						a.preventDefault()
					})
				},
			...
			pausePlay: {
				setup: function() {
					...
					p && a.pausePlay.bind("click sfmallstart",
					function(a) {
						a.preventDefault()
					})
				},
			...
			sfmall: function() {
				function b(b) {
					j = l ? d - b.sfmalles[0].pageY: d - b.sfmalles[0].pageX;
					p = l ? Math.abs(j) < Math.abs(b.sfmalles[0].pageX - e) : Math.abs(j) < Math.abs(b.sfmalles[0].pageY - e);
				...
				function g() {
					i.removeEventListener("sfmallmove", b, !1);
					if (a.animatingTo === a.currentSlide && !p && null !== j) {
						var h = m ? -j: j,
						l = 0 < h ? a.getTarget("next") : a.getTarget("prev");
						a.canAdvance(l) && (550 > Number(new Date) - k && 50 < Math.abs(h) || Math.abs(h) > q / 2) ? a.flexAnimate(l, c.pauseOnAction) : r || a.flexAnimate(a.currentSlide, c.pauseOnAction, !0)
					}
					i.removeEventListener("sfmallend", g, !1);
					f = j = e = d = null
				}
				var d, e, f, q, j, k, p = !1;
				i.addEventListener("sfmallstart",
				function(j) {
					a.animating ? j.preventDefault() : 1 === j.sfmalles.length && (a.pause(), q = l ? a.h: a.w, k = Number(new Date), f = h && m && a.animatingTo === a.last ? 0 : h && m ? a.limit - (a.itemW + c.itemMargin) * a.move * a.animatingTo: h && a.currentSlide === a.last ? a.limit: h ? (a.itemW + c.itemMargin) * a.move * a.currentSlide: m ? (a.last - a.currentSlide + a.cloneOffset) * q: (a.currentSlide + a.cloneOffset) * q, d = l ? j.sfmalles[0].pageY: j.sfmalles[0].pageX, e = l ? j.sfmalles[0].pageX: j.sfmalles[0].pageY, i.addEventListener("sfmallmove", b, !1), i.addEventListener("sfmallend", g, !1))
				},
				!1)
			},
