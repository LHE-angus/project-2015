...
<body>
<div class="top_class"><span class="lspan"><a href="<c:url value='/epp/mobile/center/Index.do?'/>"><img src="${ctx}/styles/epp/mobile/images/return.gif" /></a></span><h3>我的订单</h3></div><div class="maincont"><!--right-->
<div class="member_right padbot45">
<div class="membertab3"><html-el:form action="/mobile/center/Orders"><html-el:hidden property="method" styleId="method" value="list" /><html-el:hidden property="orderState" />
	<table style="width:100%;">
		<tr height="40">
			<td width="30%"><font style="margin-left: 30px;">下单时间：</font></td>
		...
		<tr height="40">
			<td width="30%"><font style="margin-left: 30px;">订单编号：</font></td><td><html-el:text property="trade_index" maxlength="20" styleClass="input_txt" style="width:160px;"></html-el:text> <input class="inputbtn" type="submit" name="Submit" value="查询" /></td>
		</tr> 
	...
	<div class="in">
		<table style="width:100%;">
			<tr>
				...
				<td rowspan="1" width="34%">
					<a href="javascript:void(0);" onclick="view(${cur.id});"><font color="#0022ff">订单详情</font></a>
					<c:if test="${cur.state eq 0 }"><br/><c:if test="${cur.pay_way ne 9 and cur.pay_way ne 0 and cur.pay_way ne 1}"> <a class="btn btn-4" href="<c:url value='/epp/mobile/Payment.do?trade_index=${cur.trade_index}' />"><s></s>付款</a><br/></c:if> <a href="#" id="t${cur.id }" onclick="deleteInfo(this);" ><font color="#0022ff">取消订单</font></a> </c:if>  
				</td>
			</tr>
			<tr><td colspan="5"><div class="divcss5-4"></div></td></tr><c:forEach items="${cur.pshowOrdeDetailsList}" var="cur2">
			<tr height="60">
				...
				<td colspan="3">${cur2.map.pd_name } <p>单价：<font style="color: red;">￥${cur2.price}</font></p><p>数量：${cur2.num}</p></td>
			</tr></c:forEach>
		</table>
	</div>
</div>
</c:forEach><c:if test="${empty entityList }">
...
function deleteInfo(obj){
	...
	if(confirm("您确定取消订单吗？")){
		location.href="${ctx}/epp/mobile/center/Orders.do?method=delete"+id; 
	}
}
...
function view(id){
	...
	location.href="${ctx}/epp/mobile/center/Orders.do?method=view"+id;  
}
