   ...
   <c:if test="${ecUser.is_act ne 0 or ecUser.login_count <20 }">
    ...
    <div class="membertab3" id="div_info">
     <p style="margin-left:20px;margin-top:15px;font-size:16px;line-height:36px;"> 
     	${ecUser.real_name }您好，
     	<c:if test="${is_act eq 0 }"> 您的资料已经填写完成，审核通过,欢迎 <a href="${ctx }/member/Index.do">购物</a>,建议您先进入会员中心 查看您的会员权益!</c:if>  
     	<c:if test="${is_act eq 1 }"> 您的资料没有填写完整 ，请完善 >> <a href="${ctx }/member/center/RegUser.do?"><font style="font-size: 20px;color: red;">用户资料</font></a> </c:if>
     	<c:if test="${is_act eq 2 }"> 您的资料已经填写完成，等待审核。。。审核结果稍后将发送到您登记的邮箱<br/></c:if>
     	<c:if test="${is_act eq 3 }"> 您的资料审核不通过，请重新完善 >> <a href="${ctx }/member/center/RegUser.do?method=edit"><font style="font-size: 20px;color: red;">用户资料</font></a><br/>如有疑问，请联系开心猫客服  0775-61368827（直线）0775-26608866 转  6346（内线）  </c:if>
     </p> 
    </div>
  </div>
</div>
