...
<div class="card_center">
  ...
  <div class="card_box">
    <ul>
      ...
      <li class="mc">商品名称</li>
      <li class="tp">商品图片</li>
      <li class="jg">数量</li><c:if test="${ecUser.user_type eq 1 }">
      <li class="sl">内购价</li></c:if><c:if test="${ecUser.user_type eq 2 }">
  ...
  <form action="<c:url value='/member/ShoppingCar.do' />" method="post" id="shopping_car_form">
    ...
    <c:forEach items="${konkaBcompPdList}" var="cur" varStatus="vs">
	  <div class="card_content" id="div_${cur.id}">
	    ...
	    <ul>
	      ...
	      <li class="c_two">
	       <a href="<c:url value='/member/PdShow.do?goods_id=${cur.id}' />" target="_blank" title="${cur.pd_name}">${fnx:abbreviate(cur.pd_name, 2 * 16, '...')}</a>
	      </li>
	      <li class="c_three"><table border="0" cellspacing="0" cellpadding="0"><tr><td height="70" width="100" style="overflow:hidden;" valign="middle" align="center"><a href="<c:url value='/member/PdShow.do?goods_id=${cur.id}' />"><img alt="${cur.pd_name}" src="${ctx}/${fn:substringBefore(cur.main_pic, '.')}_060.jpg" style="max-width:60px;max-height:60px;" /></a></td></tr></table></li>
	      <li class="c_four">
	        <div class="fout_left"><img src="${ctx}/styles/member/images/jian_1.jpg" width="11" height="11" class="num_jian_class" data="${cur.id}" style="cursor:pointer;" /></div>
	        ...
	        <div class="fout_right"><img src="${ctx}/styles/member/images/jia_1.jpg" width="11" height="11" class="num_jia_class" data="${cur.id}" style="cursor:pointer;" /></div>
	      </li><c:if test="${ecUser.user_type eq 1 }">
	      ...
	      <li class="c_kucu" id="stocks_flag_${cur.id}">
	      	<c:if test="${not empty cur.ecStocks.stocks and cur.ecStocks.stocks ge cur.ecShoppingCart.pd_num}"><font style="margin-left:10px;line-height:70px;" class="stocks_flag">现货，下单后发货！</font></c:if> 
	      	<c:if test="${empty cur.ecStocks.stocks or cur.ecStocks.stocks lt cur.ecShoppingCart.pd_num}"><font color="red" style="margin-left:10px;line-height:70px;">抱歉，暂时缺货！</font></c:if>  
	      </li>
  ...
  <div class="but_card" style="font-size:14px;font-weight:bold;">
    <ul>
      <li><a href="<c:url value='/member/Index.do' />" id="step_one_go_shopping">&lt;&lt; 继续购物</a></li>
...
$(document).ready(function(){
	...
	$(".buy_num_class").bind("input propertychange", function() {
		...
		if(buy_num > had_stock_num){
			$("#stocks_flag_" + goods_id).empty().html('<font color="red" style="margin-left:10px;line-height:70px;">抱歉，暂时缺货！</font>');
		} else {
			$("#stocks_flag_" + goods_id).empty().html('<font style="margin-left:10px;line-height:70px;" class="stocks_flag">现货，下单后发货！</font>');
		}
	});
