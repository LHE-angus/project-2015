...
<!-- second start -->
<div class="maincont margintop10" style="padding-top:8px;">
  <div class="position"><a href="<c:url value='/member/Index.do' />">首页</a> &gt; <a href="<c:url value='/member/EcGift.do' />">积分商城</a> &gt;<c:out value="${ecGift.pd_sn}" /></div>
  <div class="viewbox1">
    <jsp:include page="../__inc/ec_gift_images.jsp" flush="true" />
    <!-- 表单开始 -->
    <form action="<c:url value='/member/EcGift.do' />" id="form_pd_buy" method="post">
    ...
    <div class="viewright">
      <div class="viewtit1"><h3><c:out value="${ecGift.pd_sn}" /></h3><h4><c:out value="${ecGift.pd_name}" /></h4><div class="clear"></div>
      </div>
      <div class="viewtab1">
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="proxltab">
          <tbody>
          <tr>
              <td colspan="3">市场价：<strong class="yel16b"><fmt:formatNumber value="${ecGift.original_price}" pattern="0.00" /></strong></td>
            </tr>
            <tr>
              <td colspan="3">所需积分：<strong class="yel16b redfont"><fmt:formatNumber value="${ecGift.need_integral}" pattern="0" /></strong></td>
            </tr>
            <c:if test="${not empty ecGift.ecGiftComm and not empty ecGift.ecGiftComm.price and not empty ecGift.ecGiftComm.goods_url }">
             <tr>
              <td colspan="3">商品价格：<strong class="yel16b redfont">${fn:escapeXml(ecGift.ecGiftComm.price)}元</strong></td>
            </tr>
            </c:if>
            <tr>
      ...
      <div class="viewtab2">
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="proxltab">
          <tbody>
            <tr>
              <td><div class="padtop20">&nbsp;&nbsp;<img alt="立即兑换" src="${ctx}/styles/member/images/but_jifen.jpg" style="cursor:pointer;" id="buy_btn" />  &nbsp;积分不足去[<a href="${ctx }/member/EcGiftJf.do" target="_blank"><font color="blue">充值</font></a>]
               <c:if test="${not empty ecGift.ecGiftComm and not empty ecGift.ecGiftComm.price and not empty ecGift.ecGiftComm.goods_url}"> &nbsp;&nbsp;<a href="${ecGift.ecGiftComm.goods_url}"><img alt="立即购买" src="${ctx}/styles/member/images/but_comm.jpg" style="cursor:pointer;" id="buy_btn2" /></a></c:if>
              </div></td>
            </tr>
          </tbody>
        </table>
        ...
        <ul class="listul4" style="padding:0;">
           <c:forEach items="${bcomp_pd_list_top_5}" var="cur" varStatus="vs">
	          ...
	          <li style="padding:0;" class="${li_class}">
	            <div class="listpic1"><table border="0" cellspacing="0" cellpadding="0" width="120"><tr><td height="110" style="overflow:hidden;" width="120" valign="middle" align="center"><a href="<c:url value='/member/PdShow.do?goods_id=${cur.id}' />"><img src="${ctx}/${fn:substringBefore(cur.main_pic, '.')}_120.jpg" style="max-height:100px;" /></a></td></tr></table></div>
	            <div class="listrxt1">
