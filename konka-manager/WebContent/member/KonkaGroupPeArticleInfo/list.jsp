...
<link rel=stylesheet type=text/css  href="${ctx}/styles/member/css/news.css" />
...
<div class="maincont margintop10">
      ...
      <div class="larticle_category">
        <ul>
          <li><a class="a_cat_a" href="javascript:void(0);">资讯动态</a>
	          <a class="a_cat_b" href="<c:url value='/member/KonkaGroupPeArticleInfo.do?article_type_id=2000' />">康佳资讯</a>
		  </li>
          <li><a class="a_cat_a" href="javascript:void(0);">帮助中心</a>
	          <a class="a_cat_b" href="<c:url value='/member/KonkaGroupPeArticleInfo.do?article_type_id=2100' />">新手上路</a>
	          <a class="a_cat_b" href="<c:url value='/member/KonkaGroupPeArticleInfo.do?article_type_id=2200' />">支付方式</a>
	          <a class="a_cat_b" href="<c:url value='/member/KonkaGroupPeArticleInfo.do?article_type_id=2300' />">物流配送</a>
	          <a class="a_cat_b" href="<c:url value='/member/KonkaGroupPeArticleInfo.do?article_type_id=2400' />">售后服务</a>
	          <a class="a_cat_b" href="<c:url value='/member/KonkaGroupPeArticleInfo.do?article_type_id=2500' />">特色服务</a>
	          <a class="a_cat_b" href="<c:url value='/member/KonkaGroupPeArticleInfo.do?article_type_id=2600' />">走进康佳</a>
	          <!-- <a class="a_cat_b" href="<c:url value='/member/KonkaGroupPeArticleInfo.do?article_type_id=2800' />">球队介绍</a>  -->
	          </li>
        </ul>
      </div>
    </div>
  ...
  <div class="listright-news">
    <div class="position"><a href="<c:url value='/member/Index.do' />">首页</a> <c:if test="${not empty af.map.article_type_id}">&gt; <a href="<c:url value='/member/KonkaGroupPeArticleInfo.do' />?article_type_id=${af.map.article_type_id}"> <c:if test="${af.map.article_type_id eq 2000}">康佳资讯</c:if>
      ...
      <div class="box_list">
        <ul>
        	<c:forEach items="${entityList}" var="cur" varStatus="vs">
        		...
        		<li class="border"><span class="r grayfont"><fmt:formatDate value="${cur.pub_date}" pattern="yyyy/MM/dd" /></span><a href="<c:url value='/member/KonkaGroupPeArticleInfo.do?method=view&id=${cur.id}' />">${fn:escapeXml(cur.title)}</a></li>
        		</c:if>
