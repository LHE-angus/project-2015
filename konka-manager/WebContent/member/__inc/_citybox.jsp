...
<tbody>
  <tr>
    ...
    <td width="63%" height="22">
    	<span id="had_stock" style="display:${konkaBcompPd.ecStocks.stocks > 0 ? '' : 'none'};"><strong class="f14b">现货</strong>&nbsp;<c:if test="${empty konkaBcompPd.ecBaseExpressReachDay and tuangou ne 6 }">付款后立即发货！</c:if><c:if test="${not empty konkaBcompPd.ecBaseExpressReachDay}">付款后预计${konkaBcompPd.ecBaseExpressReachDay.max_reach_day}天送达</c:if></span>
			...
			success: function(data) {
				...
				if(data.stocks > 0){
					...
					if(data.max_reach_day > 0){
						...
						$("#had_stock").empty().html('<strong class="f14b">现货</strong>&nbsp;<c:if test="${tuangou ne 6 }">付款后立即发货</c:if>').show();
					}
					$("#no_stock").hide();
				} else {
					$("#had_stock").hide();
