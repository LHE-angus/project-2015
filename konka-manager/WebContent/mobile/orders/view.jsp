    ...
    <div class="membertab3">
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="member_form_table0"  style="border:1px solid #e3e3e3;">
        ...
        <tr>
          <td colspan="2">订单状态：
            ...
            <c:if test="${af.map.state eq 0 }">待付款</c:if>
      ...
      <table id="tbl2" width="100%" border="0" cellpadding="0" cellspacing="0" class="member_form_table0" style="border:1px solid #e3e3e3;display:none;" >
        <tr>
          <td align="left">付款方式：
            ...
            <c:if test="${af.map.state ne -10 && af.map.state ne 0 }">已付款 </c:if>
            </c:if>
          </td>
        </tr>
      </table>
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="member_form_table0" style="border:1px solid #e3e3e3;" >
        ...
        <tr >
          <td align="left" style="line-height:22px;border-bottom:1px solid #e3e3e3;"><p style="margin-top:1px;font-size:14px;color:#ff0000;font-weight:bold;">支付及配送方式</p>
            支付方式： <c:if test="${af.map.state gt 0 }">
            <c:if test="${af.map.pay_way eq 1}">在线支付</c:if>
            <c:if test="${af.map.pay_way eq 2}">支付宝</c:if>
            <c:if test="${af.map.pay_way eq 3}">银联</c:if>
             <c:if test="${af.map.pay_way eq 5}">民生e支付</c:if></c:if>
            <br/>
            配送方式：
        ...
        <tr >
          <td align="left" style="line-height:22px;border-bottom:1px solid #e3e3e3;"><p style="margin-top:1px;font-size:14px;color:#ff0000;font-weight:bold;">发票信息</p>
            ...
            <c:if test="${af.map.bill_is_add eq 1}">是 <br/>
             ...
             <!-- 发票内容：${fn:escapeXml(af.map.bill_content)} <br/> 
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="member_form_table0" style="border:1px solid #e3e3e3;" >
        ...
        <tr style="height:60px;">
          ...
          <c:forEach items="${pshowOrdeDetails}" var="cur">${fn:escapeXml(cur.pd_name)}  
	         <font color="#ff5300">
...
$(document).ready(function(){    
	var id = "${af.map.id}";
	  ...
	  $("#syncBtn").click(function(){
			...
			$.ajax({
				type: "post",
				url: "${ctx}/epp/mobile/center/Orders.do?method=sfList",
				data: {"id" : id},
				dataType: "json",
