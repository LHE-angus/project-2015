...
<div class="mainbox">
	<div class="maincont">   
<!-- buy start -->
<html-el:form action="/wap/ShoppingCar">
...
<input type="hidden" name="pay_way" value="9" id="pay_way" />
   ...
   <div style="margin-top:5px;width:100%;background-color:#ffffff;"> 
    ...
    <div style="margin-left:3px;padding: 3px;border: solid 1px #E6E6E6;">
      <table style="width:100%;"> <c:if test="${ecUser.map.web_type ne 2 }">
          ...
          <tr height="55">
            <td align="center"><font color="red">*</font>银行转账： </td>
            <td><table style="width:100%;"><tr height="45">
            <td align="center" valign="middle" width="22"><input type="radio" name="pay_way" value="1" id="pay_way_1" /></td> 
            <td align="left" valign="middle" colspan="5"><label for="pay_way_1" style="cursor:pointer;">户名：康佳集团股份有限公司
						帐号：744557963343
						开户行：中国银行深圳沙河支行   （银行转账时请备注：康佳epp内购平台购机款 ,收款人手机:18664553495）</label></td></tr></table> </td>
          </tr>
          <c:if test="${ecUser.user_type eq 1 && is_hdfk eq 0 }">
          <tr height="55">
    ...
    <div style="margin-left:3px;padding: 3px;border: solid 1px #E6E6E6;">
      <table style="width:100%;border:1px;"> 
        <c:forEach items="${konkaBcompPdList}" var="cur">
	        <tr >
	          <td style="border-bottom:solid 1px #E6E6E6;" width="18%"><a href="<c:url value='/webservice/wap/PdShow.do?goods_id=${cur.id}' />" target="_blank">康佳${cur.pd_sn}</a></td>
  ...
  <div class="but_card">
    <ul><li><a href="<c:url value='/webservice/wap/ShoppingCar.do' />" id="step_one_go_shopping">返回购物车</a></li><li><a style="cursor:pointer;" id="step_two_btn">提交订单</a></li> </ul>
  </div> 
</html-el:form>
</div> 
</div>
...
function showDialog(){   
	 if(is_v==0){
		...
		$("#val_div").html("<font color='red'>验证支付密码...</font>");
	  	var url="<c:url value='/webservice/wap/ShoppingCar.do' />?method=checkPayPwd&time="+Math.random();  
