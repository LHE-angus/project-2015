    ...
    <tr>
        ...
        <td width="60%" valign="top" align="center"> 
	        ...
	        <tr style="background-color: #072B31;color:#ffffff;">
	        	<td align="center" width="60">渠道</td><td  align="center">目标销售额</td><td  align="center">实际销售额</td><td  align="center">同期销售额</td><td  align="center">完成率</td><td align="center">增长率</td><td align="center">实际销售额占比</td><td align="center">累计目标额</td><td align="center">累计销售额</td><td align="center">累计完成率</td>
	        </tr>
	        ...
	        <tr style="background-color: #E1F3EB;">
		        ...
		        <td align="center">
		        	<c:if test="${cur.month_customer_sale gt 0 or cur.month_customer_sale lt 0 }">
		        		<fmt:formatNumber value="${(cur.month_settle_money-cur.month_customer_sale)*100/cur.month_customer_sale}" pattern="0.00"/>%
		        	</c:if>
		        	<c:if test="${!(cur.month_customer_sale gt 0 or cur.month_customer_sale lt 0) }">
		        		-
		        	</c:if>
		        ...
		        <td align="right">
		        	<c:if test="${s_fgs_month_settle_money gt 0 or s_fgs_month_settle_money lt 0 }">
		        		<fmt:formatNumber value="${(cur.month_settle_money-s_fgs_month_settle_money)*100/s_fgs_month_settle_money}" pattern="0.00"/>%
		        	</c:if>
		        	<c:if test="${!(s_fgs_month_settle_money gt 0 or s_fgs_month_settle_money lt 0) }">
		        		-
		        	</c:if>
		        ...
		        <td align="center">
		        	<c:if test="${cur.map.customer_sale_lj gt 0 or cur.map.customer_sale_lj lt 0 }">
		        		<fmt:formatNumber value="${(cur.map.settle_money_lj-cur.map.customer_sale_lj)*100/cur.map.customer_sale_lj}" pattern="0.00"/>%
		        	</c:if>
		        	<c:if test="${!(cur.map.customer_sale_lj gt 0 or cur.map.customer_sale_lj lt 0) }">
		        		-
		        	</c:if>
	        ...
	        <tr style="background-color: #5BCAC6;">
		        ...
		        <td align="center">
		        	<c:if test="${s_month_customer_sale gt 0 or s_month_customer_sale lt 0 }">
		        		<fmt:formatNumber value="${(s_month_settle_money-s_month_customer_sale)*100/s_month_customer_sale}" pattern="0.00"/>%
		        	</c:if>
		        	<c:if test="${!(s_month_customer_sale gt 0 or s_month_customer_sale lt 0) }">
		        		-
		        	</c:if>
		        ...
		        <td align="center">
		        	<c:if test="${s_customer_sale_lj gt 0 or s_customer_sale_lj lt 0 }">
		        		<fmt:formatNumber value="${(s_settle_money_lj-s_customer_sale_lj)*100/s_customer_sale_lj}" pattern="0.00"/>%
		        	</c:if>
		        	<c:if test="${!(s_customer_sale_lj gt 0 or s_customer_sale_lj lt 0) }">
		        		-
		        	</c:if>
...
<table id="table5" style="width:100%;">
    <tr>
        <td width="100%" valign="top" align="center"> 
	        ...
	        <c:forEach items="${fgsList}" var="cur" varStatus="vs">
	        <c:if test="${not empty cur.map.name}">
	          <c:set var="s_month_customer_sale" value="${s_month_customer_sale + cur.month_customer_sale}"></c:set>
	          <c:set var="s_month_settle_money" value="${s_month_settle_money + cur.month_settle_money}"></c:set>
	          <c:set var="s_pre_year_settle_money" value="${s_pre_year_settle_money + cur.pre_year_settle_money}"></c:set>
	          <c:set var="s_month_settle_money_bz" value="${s_month_settle_money_bz + cur.month_settle_money_bz}"></c:set>
...
function myChart1(year,month,fgs_id){
	...
	$.ajax({
		type: "POST",
		url: "<c:url value='/webservice/JcfxReportXswcFgs.do' />",
