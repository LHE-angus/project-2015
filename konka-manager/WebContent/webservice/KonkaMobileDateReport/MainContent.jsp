...
<!-- 		<li>按客户</li> -->
    ...
    <div class="sjfx-cont qhContList">
    	<iframe id="mainFrame" src="${ctx}/webservice/KonkaMobileDateReport.do?method=list&type=0&start_time=${start_time}&end_time=${end_time}&user_id=${user_id}&userpass=${userpass}&dept_id=${dept_id}&data_type=${data_type}" width="100%"></iframe>
    </div>
</article>
...
$(".qhNavList li").click(function(){
	...
	//alert($(this).index());
	if($(this).index() != 3){
	$("#mainFrame").attr("src","${ctx}/webservice/KonkaMobileDateReport.do?method=list&type="+$(this).index()+"&start_time=${start_time}&end_time=${end_time}&user_id=${user_id}&userpass=${userpass}&dept_id=${dept_id}&data_type=${data_type}");
	}
