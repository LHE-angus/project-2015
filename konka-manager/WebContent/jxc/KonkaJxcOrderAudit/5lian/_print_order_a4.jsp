...
<body>
	...
	<table width="100%" border="0" >
		...
		<tr>
			<td colspan="1" align="right" style="font-size: 12px;" >客户名称:</td>
			<td colspan="2" align="left" style="font-size: 12px;">${fn:escapeXml(orderinfo.ag)}&nbsp;${fn:escapeXml(orderinfo.user_shop_name)}&nbsp;[&nbsp;${khxd.CTLPC }&nbsp;]</td>
		...
		<tr>
			<td colspan="1" align="right" style="font-size: 12px;" >送达方:</td>
		    <td colspan="2" align="left" style="font-size: 12px;">${orderinfo.we }&nbsp;${konkaR3Shop.customer_name }</td>
		    <td colspan="1" align="right" style="font-size: 12px;">出库仓位:</td>
		    <td colspan="1">${storeKey}</td>
		    <td colspan="1" >&nbsp;</td>
	 	</tr>
		<tr>
			<td colspan="1" align="right" style="font-size: 12px;" >送货地址(电话):</td>
		    <td colspan="2" align="left" style="font-size: 12px;">${orderinfo.user_addr }&nbsp;${orderinfo.user_tel }&nbsp;${orderinfo.user_phone }</td>
		    <td colspan="1" align="right" style="font-size: 12px;"></td>
		    <td colspan="1"></td>
	 	</tr>
		    ...
		    <table width="100%" border="1" frame="void">
		     ...
		     <c:forEach items="${konkaOrderInfoDetailsList}" var="cur" varStatus="vs" >
              <c:if test="${ cur.good_count gt 0 }">
          	  ...
          	  <c:if test="${ cur.good_count le 0 }">
              	<c:set scope="request" var="recordCount" value="${recordCount-1}"></c:set>
              </c:if>
            </c:forEach>
            <!-- 不足七行补7行,超过一直往下加 -->
		   <c:if test="${recordCount le 7}" >
	    ...
	    <td colspan="2" >
		    <table width="100%"  border="1" frame="void" >
		      ...
		      <c:if test="${totalCount gt 7}">
			      <c:forEach begin="8" end="${totalCount}">
			      	 <tr>
				        <td width="30%">&nbsp;</td>
				        <td width="35%">&nbsp;</td>
				        <td width="35%">&nbsp;</td>
	...
	<table width="100%" border="1" frame="border" >
		...
		<c:if test="${!empty konkaOrderInfoAuditList}">
			<tr> 
				<tr>
