    ...
    <html-el:form action="/JxcKonkaOrderRegister.do">
      <table width="100%" border="0" cellpadding="0" cellspacing="1" class="tableClass">
        ...
        <tr>
          ...
          <td colspan="6"><font color="red">${fn:escapeXml(af.map.trade_index)}</font></span></td>
        </tr>
        <tr>
          <td class="title_item" width="15%">配送方式：</td>
          <td colspan="6"><c:if test="${af.map.send_type eq 1}">自提</c:if>
            <c:if test="${af.map.send_type eq 2}">配送</c:if></td>
        </tr>
        <tr>
          <td class="title_item" width="15%">货款支付方式：</td>
          <td colspan="6"><c:if test="${af.map.pay_type eq 4}">现款</c:if>
            <c:if test="${af.map.pay_type eq 5}">担保</c:if></td>
        </tr>
        <tr>
          <td class="title_item" width="15%">运费：</td>
          <td colspan="6"> ${fn:escapeXml(af.map.freight)} </td>
        </tr>
        <tr>
          <td class="title_item" width="15%">备注：</td>
          <td colspan="6">${fn:escapeXml(af.map.remark)}</td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td colspan="4">${fn:escapeXml(af.map.order_num)}</td>
        </tr>
        <tr>
          ...
          <td colspan="4"><fmt:formatNumber value="${fn:escapeXml(af.map.good_discount_price)}" pattern="0.00" />
            (元)</td>
        </tr>
        ...
        <tr>
          <td colspan="7" align="center"><html-el:button styleClass="bgButtonPrint" property="" styleId="btn_print" value="打印"/>
            &nbsp;&nbsp;
