  ...
  <div class="shoppingcart_c">
    ...
    <h3 class="TitleName"><b>订单状态</b></h3>
      ...
      <div style="font-size:14px; padding:10px 0px;">
         ...
         <c:if test="${cur1.state eq 0 }"> 
         <c:if test="${cur1.pay_way eq 0 or cur1.pay_way eq 1}">待审核  </c:if> <c:if test="${cur1.pay_way ne 9 and cur1.pay_way ne 0 and cur1.pay_way ne 1}">待付款  </c:if>
         </c:if>
         ...
         <c:if test="${cur1.pay_way eq 9}"> 线下处理  </c:if> 
      </div>
  	</div>
      ...
      <tbody>
        ...
        <c:forEach items="${cur1.pshowOrdeDetailsList}" var="cur2">
	        <tr>
	          ...
	          <td style="border-bottom:solid 1px #E6E6E6;"><a href="<c:url value='/touch/PdShow.do?goods_id=${cur2.pd_id}' />" target="_blank">${cur2.pd_name}</a></td>
	          ...
	          <td valign="middle" style="text-align:left;border-bottom:solid 1px #E6E6E6;">
	           ...
	           <td style="border-bottom:solid 1px #E6E6E6;">￥<fmt:formatNumber value="${cur2.total_price}" pattern="#,#00" /></td>
	        </tr>
        </c:forEach>
      </tbody>
    </table>
