...
<div class="oarcont" id="body_oarcont">
  ...
  <div class="rtabcont1">
    <html-el:form action="/admin/KonkaSellUnReport">
	    ...
	    <table width="100%" border="0" cellpadding="0" cellspacing="1" class="rtable2">
	      <tr class="tabtt1">
	        ...
	        <td width="10%" nowrap="nowrap">门店名称</td>
	         <td width="10%" nowrap="nowrap">促销员</td>
	        <td width="5%" nowrap="nowrap">是否申报</td>
	        <td width="5%" nowrap="nowrap">申报次数</td>
	        <td width="5%" nowrap="nowrap">申报金额</td>
	        <td width="5%" nowrap="nowrap">申报数量</td>
	        <td width="5%" nowrap="nowrap">日期区间</td>
	        <td width="5%" nowrap="nowrap">门店状态</td>
	        <td width="5%" nowrap="nowrap">停用时间</td>
	      </tr>
	      ...
	      <c:forEach items="${entityList}" var="cur" varStatus="vs">
	        <tr>
	         ...
	         <td align="left" nowrap="nowrap">${cur[24]}</td>
	          <td align="center" nowrap="nowrap">
			  	<c:choose>
	          		<c:when test="${cur[17] eq 0}"><font color="#B452CD">未申报</font></c:when>
	          		<c:when test="${cur[17] eq 1}">已申报</c:when>
	          ...
	          ${cur[19]}
	      <%--   <span class="kz-price-12"><fmt:formatNumber value="${cur[19]}" type="currency" /></span>--%>  
	          </td>
	          ...
	          <td align="center" nowrap="nowrap">
	           ...
	           <td align="center" nowrap="nowrap">
	           <c:choose>
	          		<c:when test="${ cur[15] eq 0}"><font color="#9BCD9B">未停用</font></c:when>
	          		<c:when test="${cur[15] eq 1}"><font color="#4876FF">已停用</font></c:when>
	          		<c:otherwise>状态错误</c:otherwise>
	          	</c:choose>
	           </td>
	            <td align="center" nowrap="nowrap">
	          <c:choose>
	          		<c:when test="${ cur[15] eq 0}">/</c:when>
	          		<c:when test="${cur[15] eq 1}">${cur[16]}</c:when>
	          		<c:otherwise>状态错误</c:otherwise>
	          	</c:choose>
	           </td>
	        </tr>
...
function exportExcel(){
	//CNZZ统计代码
	//_czc.push(["_trackEvent", "零售申报统计下载", "导出", "Excel", 0]);
