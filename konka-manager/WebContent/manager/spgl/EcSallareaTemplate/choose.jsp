...
<body>
 <form name="storeSalesAreaForm" method="post" action="${ctx}/manager/spgl/EcSallareaTemplate.do"> 
...
<div class="store-right" style="min-height:0;overflow:hidden;"> 
		...
		<div class="store-path">销售区域管理 &gt; 销售区域<c:if test="${empty af.map.id}">增加</c:if><c:if test="${not empty af.map.id}">编辑</c:if>&nbsp;&nbsp;<c:if test="${not empty af.map.city}">当前选择：${af.map.city}</c:if>  
		  ...
		  <div class="table_right" style="height:75px">	
		  	<table width="99%" cellspacing="0" cellpadding="0">
					<tbody>
						<tr>
						    <td colspan="3" align="center">模板名称：<input type="text" name="title" value="${af.map.title}" class="input-text" id="title" maxlength="30"/> 
						    </td>
