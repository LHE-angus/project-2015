...
<div class="oarcont" align="center">
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="1" class="rtable3">
      ...
      <c:if test="${not empty af.map.logo_url}">
        <tr>
          <td nowrap="nowrap" class="title_item" align="right">主图：</td>
          <td><img src="${fn:substringBefore(af.map.logo_url, '.')}_400.jpg" title="${af.map.remark}" /></td>
        </tr>
      </c:if>
      <tr>
      ...
      <tr>
       ...
       <tr>
        <td nowrap="nowrap" class="title_item" align="right">LOGO类型：</td>
        <td>
      ...
      <tr>
        ...
        <td><c:choose>
            <c:when test="${af.map.del_mark eq 0}">发布中</c:when>
            <c:when test="${af.map.del_mark eq 1}">关闭</c:when>
          </c:choose></td>
      </tr>
      <tr>
        <td>&nbsp;</td>
