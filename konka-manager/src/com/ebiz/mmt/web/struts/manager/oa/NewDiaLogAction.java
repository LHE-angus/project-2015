...
public class NewDiaLogAction extends BaseMmtoaAction {
	...
	public ActionForward selectUser(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		String num = (String) dynaBean.get("num");//表示哪个审核人，打开对话框列出相应的人员列表
		...
		//根据选择的角色查询用户列表 1:分公司人员   2：分公司人员    3:总部    4：总部及分公司
		...
		if (StringUtils.isNotBlank(num)) {
			...
			if("3".equals(num)){
				peProdUser.getMap().put("dept_id_zong", 0);
			}
			if("4".equals(num)){
				if (null != kd) {
					peProdUser.getMap().put("dept_id_in", kd.getDept_id());
					peProdUser.getMap().put("dept_id_aplication", true);
				}
			}
		}
...
//		peProdUser.getMap().put("user_id", _peProdUser.getId());
		...
		String address = "/oa/Dialog_new/selectUser.jsp?1=1&";
		if (selectype != null){
