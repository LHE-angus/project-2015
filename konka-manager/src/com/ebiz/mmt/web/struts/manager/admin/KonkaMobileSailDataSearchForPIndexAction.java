 ...
 * @version 2013-09-22
 */
public class KonkaMobileSailDataSearchForPIndexAction extends BaseAction {
	...
	public ActionForward list(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (StringUtils.isNotBlank(label_ytv)) {
			...
			entity.getMap().put("md_serise_ytv", "'1800','9800','6680','8800'");
		}
	...
	public List<?> getKonkaMobileSailDataForSum(VADetailsOfSalesData entity) {
		...
		String gdp_pindexfieldlist = " ,sum(t3.gdp),sum(t3.mianji),sum(t3.jdkhsl) ";
		if (entity.getMap().get("group_p_index") != null
			...
			if ("1".equals(entity.getMap().get("group_p_index").toString())) {
				sql_p_name += " ,(select p.p_name from KONKA_BASE_PROVINCE_LIST_FOUR p where p.p_index = t1.p_index),'','','' ";
				gdp_pindex = " p_area";
				gdp_pindexfield = " p_area as p_index";
			} else if ("2".equals(entity.getMap().get("group_p_index").toString())) {
				sql_p_name += " ,(select p.p_name from KONKA_BASE_PROVINCE_LIST_FOUR p where p.p_index = substr(t1.p_index,1,2)||'0000') ";
				sql_p_name += " ,(select p.p_name from KONKA_BASE_PROVINCE_LIST_FOUR p where p.p_index = t1.p_index),'','' ";
				gdp_pindex = " substr(p_area,1,2)||'0000' ";
				gdp_pindexfield = " substr(p_area,1,2)||'0000' as p_index";
			} else if ("3".equals(entity.getMap().get("group_p_index").toString())) {
				sql_p_name += " ,(select p.p_name from KONKA_BASE_PROVINCE_LIST_FOUR p where p.p_index = substr(t1.p_index,1,2)||'0000') ";
				sql_p_name += " ,(select p.p_name from KONKA_BASE_PROVINCE_LIST_FOUR p where p.p_index = substr(t1.p_index,1,4)||'00') ";
				sql_p_name += " ,(select p.p_name from KONKA_BASE_PROVINCE_LIST_FOUR p where p.p_index = t1.p_index),'' ";
				gdp_pindex = " substr(p_area,1,4)||'00' ";
				gdp_pindexfield = " substr(p_area,1,4)||'00' as p_index";
			} else if ("4".equals(entity.getMap().get("group_p_index").toString())) {
				sql_p_name += " ,(select p.p_name from KONKA_BASE_PROVINCE_LIST_FOUR p where p.p_index = substr(t1.p_index,1,2)||'0000') ";
				sql_p_name += " ,(select p.p_name from KONKA_BASE_PROVINCE_LIST_FOUR p where p.p_index = substr(t1.p_index,1,4)||'00') ";
				sql_p_name += " ,(select p.p_name from KONKA_BASE_PROVINCE_LIST_FOUR p where p.p_index = substr(t1.p_index,1,6)) ";
				gdp_pindex = " substr(p_area,1,6) ";
				gdp_pindexfield = " substr(p_area,1,6) as p_index";
				sql_p_name += " ,(select p.p_name from KONKA_BASE_PROVINCE_LIST_FOUR p where p.p_index = decode(length(trim(t1.p_index)),6,t1.p_index||'00',t1.p_index)) ";
			} else {
				// 行政区域分组未选
				return null;
			}
		...
		if (entity.getMap().get("group_time") != null
				&& StringUtils.isNotBlank(entity.getMap().get("group_time").toString())) {
			if ("1".equals(entity.getMap().get("group_time").toString())) {
				sql += " t1.y,t1.y,t1.p_index,(select p.full_name from KONKA_BASE_PROVINCE_LIST_FOUR p where p.p_index = t1.p_index) as full_name,sum(t1.all_num) as all_num,sum(t1.all_money) as all_money ";
				//省市县名称
				sql += sql_p_name;
				sql += gdp_pindexfieldlist;
				sql += " from (select t.y, ";
			} else if ("2".equals(entity.getMap().get("group_time").toString())) {
				sql += " t1.y,t1.q,t1.p_index,(select p.full_name from KONKA_BASE_PROVINCE_LIST_FOUR p where p.p_index = t1.p_index) as full_name,sum(t1.all_num) as all_num,sum(t1.all_money) as all_money ";
				//省市县名称
				sql += sql_p_name;
				sql += gdp_pindexfieldlist;
				sql += " from (select t.y,t.q, ";
			} else if ("3".equals(entity.getMap().get("group_time").toString())) {
				sql += " t1.y,t1.m,t1.p_index,(select p.full_name from KONKA_BASE_PROVINCE_LIST_FOUR p where p.p_index = t1.p_index) as full_name,sum(t1.all_num) as all_num,sum(t1.all_money) as all_money ";
				//省市县名称
				sql += sql_p_name;
				sql += gdp_pindexfieldlist;
				sql += " from (select t.y,t.m, ";
			} else if ("4".equals(entity.getMap().get("group_time").toString())) {
				sql += " t1.y,t1.ww,t1.p_index,(select p.full_name from KONKA_BASE_PROVINCE_LIST_FOUR p where p.p_index = t1.p_index) as full_name,sum(t1.all_num) as all_num,sum(t1.all_money) as all_money ";
				//省市县名称
				sql += sql_p_name;
				sql += gdp_pindexfieldlist;
				sql += " from (select t.y,t.ww, ";
			} else if ("5".equals(entity.getMap().get("group_time").toString())) {
				sql += " t1.y,t1.sale_date,t1.p_index,(select p.full_name from KONKA_BASE_PROVINCE_LIST_FOUR p where p.p_index = t1.p_index) as full_name,sum(t1.all_num) as all_num,sum(t1.all_money) as all_money ";
				//省市县名称
				sql += sql_p_name;
				sql += gdp_pindexfieldlist;
				sql += " from (select t.y,to_char(t.sale_date,'yyyy-MM-dd') as sale_date, ";
			} else {
				// 时间分组未选
				return null;
			}
		...
		if (entity.getMap().get("group_p_index") != null
				&& StringUtils.isNotBlank(entity.getMap().get("group_p_index").toString())) {
			if ("1".equals(entity.getMap().get("group_p_index").toString())) {
				sql += " substr(t.entp_p_index,1,2)||'0000' as p_index, ";
			} else if ("2".equals(entity.getMap().get("group_p_index").toString())) {
				sql += " substr(t.entp_p_index,1,4)||'00' as p_index, ";
			} else if ("3".equals(entity.getMap().get("group_p_index").toString())) {
				sql += " substr(t.entp_p_index,1,6) as p_index, ";
			} else if ("4".equals(entity.getMap().get("group_p_index").toString())) {
				sql += " t.entp_p_index as p_index, ";
			} else {
				// 行政区域分组未选
				return null;
			}
		...
		sql += " value(t.NUM,0) as all_num,value(t.ALL_PRICE,0) as all_money from V_A_DETAILS_OF_SALES_DATA t ";
		sql += " where 1 = 1 ";
		...
		//易TV
		if (entity.getMap().get("md_serise_ytv") != null && StringUtils.isNotBlank(entity.getMap().get("md_serise_ytv").toString())) {
			sql += " and t.MD_SERISE in (" + entity.getMap().get("md_serise_ytv").toString() + ") ";
		}
		...
		String KONKA_PINDEX_AREA_GDP="(select "+gdp_pindexfield+",sum(gdp) gdp,sum(column_1) as mianji,sum(column_2) as jdkhsl from KONKA_PINDEX_AREA_GDP group by " + gdp_pindex + ")";//区域统计信息
		sql += " ) t1 left join " + KONKA_PINDEX_AREA_GDP + " t3 on t1.p_index=t3.p_index where 1 = 1 ";
		String group_by = "";
		String order_by = "";
		if (entity.getMap().get("group_time") != null
				&& StringUtils.isNotBlank(entity.getMap().get("group_time").toString())) {
			if ("1".equals(entity.getMap().get("group_time").toString())) {
				group_by += " t1.y,t1.p_index ";
				order_by += " t1.y desc,t1.p_index asc ";
			} else if ("2".equals(entity.getMap().get("group_time").toString())) {
				group_by += " t1.y,t1.q,t1.p_index ";
				order_by += " t1.y desc,t1.q desc,t1.p_index asc ";
			} else if ("3".equals(entity.getMap().get("group_time").toString())) {
				group_by += " t1.y,t1.m,t1.p_index ";
				order_by += " t1.y desc,t1.m desc,t1.p_index asc ";
			} else if ("4".equals(entity.getMap().get("group_time").toString())) {
				group_by += " t1.y,t1.ww,t1.p_index ";
				order_by += " t1.y desc,t1.ww desc,t1.p_index asc ";
			} else if ("5".equals(entity.getMap().get("group_time").toString())) {
				group_by += " t1.y,t1.sale_date,t1.p_index ";
				order_by += " t1.y desc,t1.sale_date desc,t1.p_index asc ";
			} else {
				// 时间分组未选
				return null;
			}
