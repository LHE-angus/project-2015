...
import com.ebiz.mmt.web.struts.webservice.KonkaR3RankInterfaceAction;
...
public class Frames3Action extends BaseAction {
    ...
    public ActionForward index(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        // sysModule.getMap().put("bi_hidden", true);// 实现bi报表授权，前台不显示
    ...
    public ActionForward main(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (role_id_eq_10) {
            ...
            List<KonkaR3Shop> staticByFGSList = super.getFacade().getKonkaR3ShopService().getKonkaR3ShopStaticsGroupByFGS(s);
        ...
        if (!role_id_eq_188 || role_id_ne_188) {
                ...
                for (KonkaXxZmdAuditUserInfo temp1 : kInfoList) {
                    ...
                    if (null != temp1.getMap().get("role_id")) {
                        ...
                        _tr.setEventDo("<a style=\"cursor:pointer;color:blue;\" onclick=\"javascript:window.location.href='../zmd/KonkaXxZmdAuditUserInfoAudit.do?method=audit&zmd_user_id="
                                + temp1.getZmd_user_id() + "&role_id=" + temp1.getMap().get("role_id") + "&mod_id=810200" + "'\">审核</a>");
                        list.add(_tr);
                    }
                }
            }
                ...
                for (KonkaXxZmd temp1 : zmdList) {
                    ...
                    if (null != temp1.getMap().get("role_id")) {
                        ...
                        _tr.setEventDo("<a style=\"cursor:pointer;color:blue;\" onclick=\"javascript:window.location.href='../zmd/KonkaXxZmdVerification.do?method=edit&zmd_id="
                                + temp1.getZmd_id() + "&role_id=" + temp1.getMap().get("role_id") + "&mod_id=810200" + "'\">审核</a>");
                        list.add(_tr);
                    }
            ...
            List<KonkaoaSsuedDocument> entityList = super.getFacade().getKonkaoaFilesService().getKonkaoaSsuedDocumentPaginatedList(_entity);
        ...
        if (role_id_lt_30) {
            ...
            List<KonkaR3Backmoney> sailList = super.getFacade().getKonkaR3BackmoneyService().getKonkaR3SailAndJsDataToIndexPaginatedList(entity);// 零售

            List<KonkaR3Backmoney> rwList = super.getFacade().getKonkaR3BackmoneyService().getKonkaR3BackmoneyFoRwDataPaginatedList(entity); // 回款任务
            ...
            if (rwList.size() > 0) {
                if (rwList.get(0) != null && rwList.get(0).getMap().get("rw_money") != null && !"0".equals(rwList.get(0).getMap().get("rw_money").toString())) {
                    rw_wcl =
                            (new BigDecimal(dy_js_money)).divide(new BigDecimal(rwList.get(0).getMap().get("rw_money").toString()), 2, BigDecimal.ROUND_HALF_UP).multiply(
                                    new BigDecimal(100));
                }
            }
		...
		String order_date_end = dateformat.format(calendar.getTime());// 结束时间
		calendar.add(Calendar.MONTH, -1);// 初始化为当月1号
		String order_date_start = dateformat.format(calendar.getTime());// 开始时间
...
//        List<KonkaOrderInfo> entityList = super.getFacade().getKonkaOrderInfoService().getKonkaOrderInfoResultForR3CodePaginatedList(entity);
        List<HashMap> entityList = super.getFacade().getKonkaOrderInfoService().getWaitForAuditOrderList(entity);
    	// 得到分公司
		for (HashMap konkaOrderInfo : entityList) {
			if (null != konkaOrderInfo.get("ADD_DEPT_ID")) {
				KonkaDept konkaDept = new KonkaDept();
				konkaDept.setDept_id(Long.parseLong(konkaOrderInfo.get("ADD_DEPT_ID").toString()));
				...
				if (null != konkaDept && null != konkaDept.getDept_name() && !"".equals(konkaDept.getDept_name())) {
					konkaOrderInfo.put("FGSNAME", konkaDept.getDept_name());
				}
			}
		}
    ...
    public ActionForward main1(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        // DynaBean dynaBean = (DynaBean) form;
        // String url = (String) dynaBean.get("url");
        //
        // if (StringUtils.isNotBlank(url)) {
    ...
    public ActionForward main3(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        List<KonkaR3Backmoney> kBackmoneysList = super.getFacade().getKonkaR3BackmoneyService().getKonkaR3BackmoneyForMonth(backmoney);
    ...
    public ActionForward main4(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        PeProdUser ui = (PeProdUser) request.getSession().getAttribute(Constants.USER_INFO);
        PeRoleUser _peRoleUser = new PeRoleUser();
        _peRoleUser.setUser_id(ui.getId());
        List<PeRoleUser> peRoleUserList = this.getFacade().getPeRoleUserService().getPeRoleUserList(_peRoleUser);
        ...
        if (!role_id_eq_188) {
            ...
            sum_count = super.getFacade().getKonkaXxZmdAuditUserInfoService().getKonkaXxZmdAndUserInfoZmdForCount(kInfo);
            ...
            dy_xz_count = super.getFacade().getKonkaXxZmdAuditUserInfoService().getKonkaXxZmdAndUserInfoZmdForCount(kInfo1);
            ...
            dsp_count = super.getFacade().getKonkaXxZmdAuditUserInfoService().getKonkaXxZmdAndUserInfoZmdForCount(kInfo2);
        ...
        if (zmdList.size() > 0) {
            for (KonkaXxZmd temp1 : zmdList) {
                ...
                if (null != temp1.getMap().get("role_id")) {
                    ...
                    _tr.setEventDo("<a style=\"cursor:pointer;color:blue;\" onclick=\"javascript:window.location.href='../zmd/KonkaXxZmdVerification.do?method=edit&zmd_id="
                            + temp1.getZmd_id() + "&role_id=" + temp1.getMap().get("role_id") + "&mod_id=810200" + "'\">审核</a> "
                            + "<a style=\"cursor:pointer;color:blue;\" onclick=\"javascript:window.location.href='../zmd/KonkaXxZmd.do?method=view&zmd_id=" + temp1.getZmd_id()
                            + "&mod_id=810300" + "'\">查看</a>");
                    list.add(_tr);
                }
    ...
    // 顶级菜单导航#协同办公
    public ActionForward main5(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    public ActionForward top(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (role_id_ge_30_lt_200_or_ge_300) {
            ...
            String dept_name = "";
            if (konkaDept != null) dept_name = konkaDept.getDept_desc();
    ...
    public ActionForward middle(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {

        return mapping.findForward("middleFrame");
    }

    public ActionForward footer(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    /**
     * Ajax请求决策分析首页，查询当月总体情况
     * @date 2015-08-06 优化决策分析首页，根据main方法进行修改
     * @param mapping
     * @param form
     * @param request
     * @param response
     ...
     * @throws Exception
     */
    public ActionForward total_data(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        boolean is_sys = false; // 总部人员
        // 取当前时间
        SimpleDateFormat formaty = new SimpleDateFormat("yyyy");
        SimpleDateFormat formatm = new SimpleDateFormat("MM");
        Date now = new Date();
        ...
        KonkaR3Backmoney entity = new KonkaR3Backmoney();// 结算数据
        if (!is_sys) {
            entity.getMap().put("dept_id_start",ui.getDept_id());
            String sb = "{is_admin_data:false}";
            super.renderJson(response, sb);
        ...
        String sb = "{is_admin_data:true,dy_js_money:"+dy_js_money+",dy_sail_money:"+dy_sail_money+",dy_sail_num:"+
                dy_sail_num+",rw_wcl:"+rw_wcl+",user_id:"+ui.getId()+",year:"+this_year+",month:"+this_month+"}";
        super.renderJson(response, sb);
     ...
     * @throws Exception
     */
    public ActionForward file_list(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        boolean role_id_eq_188 = false; //是否促销员
        boolean role_id_lt_30 = false; // 总部人员
        boolean role_id_eq_60 = false;
        // 取当前时间
...
//        String this_month = formatm.format(now);// 当前月份
        ...
        if (!role_id_eq_188) {
            ...
            for (KonkaoaFiles t : cList) {
                ...
                title = title.replace("<a", "<span").replace("</a>", "</span>");
                _t.getMap().put("title", title);
                _t.getMap().put("add_date", formaty.format(_t.getEnterDate()));
                list.add(_t);
            }
            ...
            allmap.put("todo_list", list);    //待办事项
            ...
            konkaOrderInfo.getMap().put("querybyrole_userid_eq", ui.getId()); // 按下一个审核角色查询
            ...
            switch (max_dlevel) {
                ...
                case 8:
                    // 分公司及以下部门可见
                    KonkaDept dept_fgs = super.getKonkaDeptForFgs(Long.valueOf(ui.getDept_id())); // 查询部门分公司
                    if (null != dept_fgs && null != dept_fgs.getDept_id()) {
                        dept_id = dept_fgs.getDept_id(); // 分公司部门ID
                    }
                    break;
                case 7:
                    ...
                    break;
                case 0:
                    konkaOrderInfo.getMap().put("querybycust_userid_eq", ui.getId()); // 按客户查询
                    break;
                default:
                    // 出错
            }
            ...
            konkaOrderInfo.getMap().put("par_or_children_dept_id", dept_id);
            konkaOrderInfo.getMap().put("session_user_id", ui.getId());// 获取当前客户所查看的数据部门
            konkaOrderInfo.setIs_submit(1); // 0:表示暂存；1：表示正式提交

            String order_date_end = formaty.format(calendar.getTime());// 结束时间
            calendar.add(Calendar.MONTH, -1);// 初始化为当月1号
            String order_date_start = formaty.format(calendar.getTime());// 开始时间
            konkaOrderInfo.getMap().put("start_date", order_date_start+" 00:00:00");
            konkaOrderInfo.getMap().put("start_end", order_date_end+" 23:59:59");

            konkaOrderInfo.getRow().setCount(5);
            konkaOrderInfo.getRow().setFirst(0);

            List<KonkaOrderInfo> entityList = super.getFacade().getKonkaOrderInfoService().getKonkaOrderInfoResultForR3CodePaginatedList(konkaOrderInfo);
            // 得到分公司
            for (KonkaOrderInfo OrderInfo : entityList) {
                if (null != OrderInfo.getAdd_dept_id()) {
                    KonkaDept konkaDept = new KonkaDept();
                    konkaDept.setDept_id(OrderInfo.getAdd_dept_id());
                    konkaDept.getMap().put("dept_type_eq", 3);
                    konkaDept = super.getFacade().getKonkaDeptService().getKonkaDeptSuperiorByDeptId(konkaDept);
                    if (null != konkaDept && null != konkaDept.getDept_name() && !"".equals(konkaDept.getDept_name())) {
                        OrderInfo.getMap().put("fgsName", konkaDept.getDept_name());
                    }
                }
                OrderInfo.getMap().put("order_date",formaty.format(OrderInfo.getOrder_date()));
            }
            allmap.put("auditentityList", entityList);    //待审订单
            // 待审订单结束 WangKunLin 2014-06-10
            ...
            List<KonkaoaSsuedDocument> downfiles = super.getFacade().getKonkaoaFilesService().getKonkaoaSsuedDocumentPaginatedList(_entity);
            for (KonkaoaSsuedDocument file : downfiles) {
                file.getMap().put("add_date",formaty.format(file.getAdd_date()));
            }

            allmap.put("_entityList", downfiles);    //下发文件
            ...
            super.renderJson(response, jsonArray.toString().substring(start + 1, end+1));
