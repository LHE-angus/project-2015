...
public class ShoppingCarAction extends BaseMemberAction {
	...
	public ActionForward list(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		for (EcShoppingCart temp : ecShoppingCartList) {
			...
			if (ecUser.getUser_type().intValue() == 1 && ecUser.getIs_epp_fgs() != null
					&& ecUser.getIs_epp_fgs().intValue() == 0) {
				ecGoodsPriceList = getEcGoodsPriceListWithPindexAndGoodsId(ecUser.getUser_type(), konkaBcompPd.getId(),
						p_index_array, ecUser.getDept_id(), ecUser.getCust_id());
			} else {
				ecGoodsPriceList = getEcGoodsPriceListWithPindexAndGoodsIdNew(ecUser.getUser_type(),
						konkaBcompPd.getId(), p_index_array, ecUser.getDept_id(), ecUser.getCust_id(),
						ecUser.getPlat_sys());
			}
			...
			if (ecUser.getUser_type().intValue() == 1 && ecUser.getIs_epp_fgs() != null
					&& ecUser.getIs_epp_fgs().intValue() == 0) {
				ecStocksList = getEcStocksListWithPindexAndGoodsId(ecUser.getUser_type(), konkaBcompPd.getId(),
						p_index_array, 0L);
			} else {
		...
		return new ActionForward("/../webservice/wap/ShoppingCar/step_one.jsp");
	}
	...
	public ActionForward buyByGoodsId(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (null == konkaBcompPd) {
			...
			super.renderJavaScript(response, "window.onload=function(){alert('未找到该产品');history.back();}");
			return null;
		}
		// 处理主图
		...
		if (ecUser.getUser_type().intValue() == 1 && ecUser.getIs_epp_fgs() != null
			...
			ecGoodsPriceList = getEcGoodsPriceListWithPindexAndGoodsIdNew(ecUser.getUser_type(), konkaBcompPd.getId(),
					p_index_array, ecUser.getDept_id(), ecUser.getCust_id(), ecUser.getPlat_sys());
		}
		if (null != ecGoodsPriceList && 0 != ecGoodsPriceList.size()) {
			konkaBcompPd.setEcGoodsPrice(ecGoodsPriceList.get(0));
		} else {
			String msg = super.getMessage(request, "对不起该地区未上架");
			super.renderJavaScript(response, "window.onload=function(){alert('" + msg + "');history.back();}");
			return null;
		}
	...
	public ActionForward stepTwo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (0 == goods_ids.length) {
			String msg = super.getMessage(request, "ec.bcomp.pd.none");
			super.renderJavaScript(response, "window.onload=function(){alert('该地区暂未上架');history.back();}");
			return null;
		}
		...
		return new ActionForward("/../webservice/wap/ShoppingCar/step_two.jsp");
	}
	...
	public ActionForward createOrder(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (StringUtils.isBlank(buy_json_object)) {
			String msg = super.getMessage(request, "未找到该产品");
			super.renderJavaScript(response, "window.onload=function(){alert('" + msg + "');history.back();}");
			return null;
		}
		...
		if (payPrice.doubleValue() < 0.001 || pshowOrder.getPay_way() == 0 || pshowOrder.getPay_way() == 1
				|| pshowOrder.getPay_way() == 9) {
			super.renderJavaScript(response, "window.onload=function(){location.href='" + super.getCtxPath(request)
					+ "/webservice/wap/Payment.do?method=view&trade_index=" + trade_index + "';}");
			return null;
		}
		super.renderJavaScript(response, "window.onload=function(){location.href='" + super.getCtxPath(request)
				+ "/webservice/wap/Payment.do?trade_index=" + trade_index + "&pay_way=" + pshowOrder.getPay_way()
				+ "';}");
		return null;
	}
	...
	public ActionForward checkPayPwd(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if ("1".equals(show)) {
			...
			return new ActionForward("/../webservice/wap/ShoppingCar/checkpwd.jsp");
		}
