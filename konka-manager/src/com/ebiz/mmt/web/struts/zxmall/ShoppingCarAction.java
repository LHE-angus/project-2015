...
public class ShoppingCarAction extends BaseMemberAction {
	...
	public ActionForward list(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		// 从Session中取用户并判断
		EcUser ecUser = (EcUser) super.getSessionAttribute(request, "zxmall");
		if (null == ecUser) {
		...
		List<EcShoppingCart> ecShoppingCartList = super.getFacade().getEcShoppingCartService()
				.getEcShoppingCartList(ecShoppingCart);
		...
		for (EcShoppingCart temp : ecShoppingCartList) {
			...
			// 查询产品价格
			List<EcGoodsPrice> ecGoodsPriceList = getEcGoodsPriceListWithPindexAndGoodsIdNew(own_sys,
					konkaBcompPd.getId(), p_index_array, ecUser.getDept_id(), ecUser.getCust_id(), ecUser.getPlat_sys());
			if (null != ecGoodsPriceList && 0 != ecGoodsPriceList.size()) {
				konkaBcompPd.setEcGoodsPrice(ecGoodsPriceList.get(0));
			}

			// 查询库存
			List<EcStocks> ecStocksList = getEcStocksListWithPindexAndGoodsIdNew(own_sys, konkaBcompPd.getId(),
					p_index_array, ecUser.getDept_id(), ecUser.getPlat_sys());
			...
			es.setGoods_id(konkaBcompPd.getId());
			...
			if (xsqy_flg.intValue() == 1) {
				request.setAttribute("xsqy_flg", xsqy_flg);
			}
			konkaBcompPd.getMap().put("xsqy_flg", xsqy_flg);
			konkaBcompPdList.add(konkaBcompPd);

			BaseProvinceListFour bf = new BaseProvinceListFour();
			bf.setP_index(new Long(p_index));
			bf = super.getFacade().getBaseProvinceListFourService().getBaseProvinceListFour(bf);
			request.setAttribute("p_name", bf.getFull_name());
		}

		request.setAttribute("konkaBcompPdList", konkaBcompPdList);
		return new ActionForward("/../zxmall/ShoppingCar/step_one.jsp");
	}
	...
	public ActionForward buyByGoodsId(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			...
			for (EcRuleGoods ecRuleGoods : ecgList) {
				...
				if (ecUser.getUser_type().intValue() == 2) {
					if (konkaBcompPd.getDept_sn().equals("0")) {
						...
						eg.setId(Long.valueOf(konkaBcompPd.getDept_sn()));
						eg = super.getFacade().getEcGroupService().getEcGroup(eg);
						if (eg != null && eg.getLink_dept_id() != null) {
							er.setDept_id(eg.getLink_dept_id());
						} else {
							er.setDept_id(9999L);// 没有找到管理分公司
						}
					}
		...
		ecShoppingCart.setOwn_sys(own_sys);// 1 - 工卡
		...
		// 查询产品价格
		List<EcGoodsPrice> ecGoodsPriceList = getEcGoodsPriceListWithPindexAndGoodsIdNew(own_sys, konkaBcompPd.getId(),
		...
		if (goods_ids != null && goods_ids.length > 0) {
			for (int i = 0; i < goods_ids.length; i++) {
				...
				if (konkaBcompPd != null) {
					...
					// 查询产品价格
					ecGoodsPriceList = getEcGoodsPriceListWithPindexAndGoodsIdNew(own_sys, konkaBcompPd.getId(),
							p_index_array, ecUser.getDept_id(), ecUser.getCust_id(), ecUser.getPlat_sys());
					if (null != ecGoodsPriceList && 0 != ecGoodsPriceList.size()) {
						konkaBcompPd.setEcGoodsPrice(ecGoodsPriceList.get(0));
					}
					// 查询库存
					ecStocksList = getEcStocksListWithPindexAndGoodsIdNew(ecUser.getUser_type(), konkaBcompPd.getId(),
							p_index_array, ecUser.getDept_id(), ecUser.getPlat_sys());
					if (null != ecStocksList && 0 != ecStocksList.size()) {
						EcStocks ecStocks = new EcStocks();
						ecStocks = ecStocksList.get(0);
						BigDecimal total_stocks = (BigDecimal) ecStocks.getMap().get("total_stocks");
						Long num = 0L;
						if (total_stocks != null) {
							num = total_stocks.longValue();
						}
						ecStocks.setStocks(new Long(num));
	...
	public ActionForward stepTwo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		for (String id : goods_ids) {
				...
				if (null != egList1 && egList1.size() > 0) {
					...
					if (null != ecRule && ecRule.getRule_type() == 1 && ecRule.getInfo_state() == 1
						...
						if (null != pshowList && pshowList.size() > 0) {
							...
							if ((num.intValue() + Integer.valueOf(buy_num).intValue()) > ecRule.getRule_num_max()
									.intValue()) {
								super.renderJavaScript(response,
										"window.onload=function(){alert('该商品每人每月只能购买" + ecRule.getRule_num_max()
												+ "台');history.back();}");
								return null;
							}
						} else {
					...
					if (null != ecRule && ecRule.getRule_type() == 1 && ecRule.getInfo_state() == 1
						...
						if (Integer.valueOf(buy_num).intValue() < ecRule.getRule_num_min().intValue()) {
							super.renderJavaScript(response,
									"window.onload=function(){alert('该商品至少购买" + ecRule.getRule_num_min()
											+ "台');history.back();}");
							return null;
						}
					}
			...
			if (konkaBcompPd.getProd_type() != null) {
				if ((konkaBcompPd != null && konkaBcompPd.getProd_type().intValue() != 0 && konkaBcompPd.getProd_type()
						.intValue() != 10)
						|| (konkaBcompPd.getProd_type().intValue() == 0 && konkaBcompPd.getLabel_of_cate() != null && konkaBcompPd
								.getLabel_of_cate() == 9)) {
					is_hdfk = new Integer(1);
				}
			} else {
				is_hdfk = new Integer(0);
			}
			...
			if (konkaBcompPd != null && konkaBcompPd.getProd_type() != null
			...
			map.put("goods_id", id);
			map.put("pd_type", "" + konkaBcompPd.getProd_type());
			map.put("goods__type", "" + konkaBcompPd.getLabel_of_cate());
			konkaBcompPd.setPd_name((String) dynaBean.get("pd_name_" + id));
			map.put("pd_name", "");// 防止汉字乱码出错去掉商品名称
			...
			if (ecBcompPdRebates != null && ecBcompPdRebates.getB_value() != null
				...
				if (ecBcompPdRebates.getB_type().intValue() == 0) { // 按比例
					all_rebates = all_rebates.add(price.multiply(new BigDecimal(buy_num))
							.multiply(ecBcompPdRebates.getB_value()).divide(new BigDecimal("100"))
		...
		// 将需要购买的商品数据加密，然后到前台
		request.setAttribute("buy_json_object",
				new DESPlus().encrypt(URLEncoder.encode(JSON.toJSONString(json), "UTF-8")));
		request.setAttribute("konkaBcompPdList", konkaBcompPdList);
		...
		if ("1".equals(touch)) {
			...
			List<EcUserAddrs> ecUserAddrsList = super.getFacade().getEcUserAddrsService()
					.getEcUserAddrsList(ecUserAddrs);
		...
		return new ActionForward("/../zxmall/ShoppingCar/step_two.jsp");
	}
	...
	public ActionForward createOrder(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		// 3:银联 4:财付通
		pshowOrder.setOrder_from(own_sys); // 1-工卡，2-触网，3-会员
		...
		for (int i = 0; i < json.size(); i++) {
			...
			EcGoodsRebate ecBcompPdRebates = new EcGoodsRebate();	
			
			ecBcompPdRebates.setGoods_id(konkaBcompPd.getId());
			ecBcompPdRebates.setPlat_sys(ecUser.getPlat_sys());
			ecBcompPdRebates.setOwn_sys(2);
			ecBcompPdRebates.setDept_id(ecUser.getDept_id());
			ecBcompPdRebates.setState(0);
			ecBcompPdRebates.setC_id(ecUser.getCust_id());
			ecBcompPdRebates = super.getFacade().getEcGoodsRebateService().getEcGoodsRebate(ecBcompPdRebates);
			
			if(ecUser.getCust_id()!=null&&ecBcompPdRebates==null){
				ecBcompPdRebates = new EcGoodsRebate();
				ecBcompPdRebates.setGoods_id(konkaBcompPd.getId());
				ecBcompPdRebates.setPlat_sys(ecUser.getPlat_sys());
				ecBcompPdRebates.setOwn_sys(2);
				ecBcompPdRebates.setDept_id(ecUser.getDept_id());
			...
			EcGoodsIntegral ecGoodsIntegral = new EcGoodsIntegral();
			ecGoodsIntegral.setGoods_id(konkaBcompPd.getId());
			ecGoodsIntegral.setPlat_sys(ecUser.getPlat_sys());
			ecGoodsIntegral.setOwn_sys(2);
			ecGoodsIntegral.setIntegral_type(1);// 付款积分
			ecGoodsIntegral.setDept_id(ecUser.getDept_id());
			ecGoodsIntegral.setState(0);
			ecGoodsIntegral.setC_id(ecUser.getCust_id());
			ecGoodsIntegral = super.getFacade().getEcGoodsIntegralService().getEcGoodsIntegral(ecGoodsIntegral);
			if(ecUser.getCust_id()!=null&&ecGoodsIntegral==null){
				ecGoodsIntegral = new EcGoodsIntegral();
				ecGoodsIntegral.setGoods_id(konkaBcompPd.getId());
				ecGoodsIntegral.setPlat_sys(ecUser.getPlat_sys());
				ecGoodsIntegral.setOwn_sys(2);
				ecGoodsIntegral.setIntegral_type(1);// 付款积分
			...
			EcGoodsIntegral ecGoodsIntegral2 = new EcGoodsIntegral();
			ecGoodsIntegral2.setGoods_id(konkaBcompPd.getId());
			ecGoodsIntegral2.setPlat_sys(ecUser.getPlat_sys());
			ecGoodsIntegral2.setOwn_sys(2);
			ecGoodsIntegral2.setIntegral_type(2);// 奖励积分
			ecGoodsIntegral2.setDept_id(ecUser.getDept_id());
			ecGoodsIntegral2.setState(0);
			ecGoodsIntegral2.setC_id(ecUser.getCust_id());
			ecGoodsIntegral2 = super.getFacade().getEcGoodsIntegralService().getEcGoodsIntegral(ecGoodsIntegral2);
			if(ecUser.getCust_id()!=null&&ecGoodsIntegral2==null){
				ecGoodsIntegral2 = new EcGoodsIntegral();
				ecGoodsIntegral2.setGoods_id(konkaBcompPd.getId());
				ecGoodsIntegral2.setPlat_sys(ecUser.getPlat_sys());
				ecGoodsIntegral2.setOwn_sys(2);
				ecGoodsIntegral2.setIntegral_type(2);// 奖励积分
			...
			pod.setTotal_price(pod.getPrice().multiply(new BigDecimal(pod.getNum().toString())).add(service_price)
					.add(rule_price.multiply(new BigDecimal(buy_num.toString())))
					.subtract(rule_price_2.multiply(new BigDecimal(buy_num.toString())))
					.setScale(2, BigDecimal.ROUND_HALF_UP)); // 明细总价
			pod.setStore_id(Long.valueOf(store_id));
			// 处理奖励积分
			if (null != ecGoodsIntegral2 && ecGoodsIntegral2.getIntegral_value().longValue() > 0) {
				if (ecGoodsIntegral2.getIntegral_way().intValue() == 2) { // 按比例
					pod.setIntegral(price.multiply(new BigDecimal(buy_num))
							.multiply(ecGoodsIntegral2.getIntegral_value()).multiply(x).divide(new BigDecimal("10000"))
							.setScale(2, BigDecimal.ROUND_HALF_UP));
				}
				if (ecGoodsIntegral2.getIntegral_way().intValue() == 1) { // 按固定
					pod.setIntegral(new BigDecimal(buy_num).multiply(ecGoodsIntegral2.getIntegral_value()).multiply(x)
							.divide(new BigDecimal("100")).setScale(2, BigDecimal.ROUND_HALF_UP));
				}
			} else {
				pod.setIntegral(new BigDecimal("0.0"));
			}

			// 处理付款积分
			if (null != ecGoodsIntegral && ecGoodsIntegral.getIntegral_value().longValue() > 0) {
				if (ecGoodsIntegral.getIntegral_way().intValue() == 2) { // 按比例
					pod.setPay_integral(price.multiply(new BigDecimal(buy_num))
							.multiply(ecGoodsIntegral.getIntegral_value()).multiply(x).divide(new BigDecimal("10000"))
							.setScale(2, BigDecimal.ROUND_HALF_UP));
				}
				if (ecGoodsIntegral.getIntegral_way().intValue() == 1) { // 按固定
					pod.setPay_integral(new BigDecimal(buy_num).multiply(ecGoodsIntegral.getIntegral_value())
							.multiply(x).divide(new BigDecimal("100")).setScale(2, BigDecimal.ROUND_HALF_UP));
				}
			} else {
				pod.setPay_integral(new BigDecimal("0.0"));
			}

			// 处理返利
			if (null != ecBcompPdRebates&&ecBcompPdRebates.getRebate_way()!=null) {
				if (ecBcompPdRebates.getRebate_way().intValue() == 2) { // 按比例
					pod.setRebates(price.multiply(new BigDecimal(buy_num)).multiply(ecBcompPdRebates.getRebate_value())
							.divide(new BigDecimal("100")).setScale(2, BigDecimal.ROUND_HALF_UP));
				}
				if (ecBcompPdRebates.getRebate_way().intValue() == 1) { // 按固定
					pod.setRebates(new BigDecimal(buy_num).multiply(ecBcompPdRebates.getRebate_value()).setScale(2,
							BigDecimal.ROUND_HALF_UP));
				}
				
				//上级佣金
				if(ecUser.getLink_user_name()!=null&&ecBcompPdRebates.getSuper_rebate_way()!=null){
					if (ecBcompPdRebates.getSuper_rebate_way().intValue() == 2) { // 按比例
						pod.setSuper_rebate(price.multiply(new BigDecimal(buy_num)).multiply(ecBcompPdRebates.getSuper_rebate_value())
								.divide(new BigDecimal("100")).setScale(2, BigDecimal.ROUND_HALF_UP));
					}
					if (ecBcompPdRebates.getSuper_rebate_way().intValue() == 1) { // 按固定
						pod.setSuper_rebate(new BigDecimal(buy_num).multiply(ecBcompPdRebates.getSuper_rebate_value()).setScale(2,
								BigDecimal.ROUND_HALF_UP));
					}
				}
			}
		...
		if (vouchers != null && vouchers.length > 0) {
			for (String vouchers_id : vouchers) {
				...
				ecVouchers.getMap().put("is_effective", "1");// 有效期
		...
		// 初始化结束
		// 建立订单信息
		super.getFacade().getPshowOrderService().createPshowOrderWithDetailsForZxMall(pshowOrder);
		// 清空购物车信息
		Cookie cookie = new Cookie("SHOPING_CAR_COOKIE", null);
		cookie.setMaxAge(0);
		response.addCookie(cookie);
		EcShoppingCart ecShoppingCart = new EcShoppingCart();
		ecShoppingCart.setUser_id(ecUser.getId());
		ecShoppingCart.setOwn_sys(own_sys);
	...
	public ActionForward ajaxDelCar(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		shoppingCar.setOwn_sys(own_sys);
	...
	public ActionForward ajaxGETCar(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		DynaBean dynaBean = (DynaBean) form;
		...
		maps.put("shoppingCarTableList", shoppingCarTableList);
		maps.put("num", shoppingCarTableList.size());
		maps.put("status", "1");
		super.renderJson(response, JSON.toJSONString(maps));
		return null;
	}
	...
	public ActionForward ajaxVouchers(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (pay_pwd != null && pay_pwd.equals(ecUser.getMap().get("pay_pwd").toString())) {
			...
			voucher.getMap().put("is_effective", "1");// 有效期内
			List<EcVouchers> list = super.getFacade().getEcVouchersService().getEcVouchersList(voucher);
			super.renderJson(response, JSON.toJSONString(getEcVouchersList(list, buy_json_object)));
			return null;
		}
		super.renderJson(response, JSON.toJSONString(maps));
		return null;
	...
	public ActionForward checkPayPwd(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if ("1".equals(show)) {
			...
			return new ActionForward("/../zxmall/ShoppingCar/checkpwd.jsp");
		}

	}

	/**
	 * 根据购物车 buy_json_object 信息获取可以使用的购物券
	 * 
	 * @param list
	 * @param buy_json_object
	 * @return List&lt;EcVouchers&gt;
	 * @throws Exception
	 */
	public List<EcVouchers> getEcVouchersList(List<EcVouchers> list, String buy_json_object) throws Exception {
		List<EcVouchers> tempList = new ArrayList<EcVouchers>();
		String str = new DESPlus().decrypt(buy_json_object);
		str = URLDecoder.decode(str, "UTF-8");
		JSONArray json = JSON.parseArray(str);
		String[] goods_ids = new String[json.size()];
		Long[] buy_nums = new Long[json.size()];
		BigDecimal[] prices = new BigDecimal[json.size()];
		String[] pd_types = new String[json.size()];
		String[] goods_types = new String[json.size()];

		for (int i = 0; i < json.size(); i++) {
			JSONObject obj = json.getJSONObject(i);
			String goods_id = obj.getString("goods_id"); // 商品ID
			Long buy_num = obj.getLong("buy_num"); // 购买数量
			BigDecimal price = obj.getBigDecimal("price"); // 购买单价
			String pd_type = obj.getString("pd_tpye");
			String goods_type = obj.getString("goods_type");
			...
			if (goods_id != null && !"".equals(goods_id)) {
				...
				pd_type = "" + pd.getProd_type();
				goods_type = "" + pd.getLabel_of_cate();
				goods_ids[i] = goods_id;
				buy_nums[i] = buy_num;
				prices[i] = price;
				pd_types[i] = pd_type;
				goods_types[i] = goods_type;
				//log.info("goods_id:" + goods_id + ",pd_type:" + pd_type + ",goods_type:" + goods_type);
			}
		}

		EcVouchers vouchuers = new EcVouchers();
		for (int i = 0; i < list.size(); i++) {
			boolean flg = false;
			vouchuers = new EcVouchers();
			vouchuers = list.get(i);
			// 判断商品ID
			String goods_id = vouchuers.getGoods_id();
			long m_num = 0;
			float m_price = 0.0f;
			String temp_goods_ids = ",";
			if (goods_id != null && goods_id.length() > 0) {
				goods_id = "," + goods_id + ",";
				for (int y = 0; y < goods_ids.length; y++) {
					if (goods_ids[y] != null && !"".equals(goods_ids[y])) {
						if (goods_id.indexOf(goods_ids[y]) != -1) {
							flg = true;
							temp_goods_ids += goods_ids[y];
						}
					}
				}
			} else {
			...
			String pd_type = vouchuers.getPd_type();
			if (pd_type != null && pd_type.length() > 0) {
				pd_type = "," + pd_type + ",";
				for (int y = 0; y < pd_types.length; y++) {
					flg = false;
					if (pd_types[y] != null && !"".equals(pd_types[y])) {
						if (pd_type.indexOf(pd_types[y]) != -1) {
							flg = true;
						}
					}
					if (!flg) {
						if (goods_ids[y] != null && !"".equals(goods_ids[y])) {
							temp_goods_ids = temp_goods_ids.replaceAll("," + goods_ids[y] + ",", ",");
						}
					}
				}
			} else {
			...
			String goods_type = vouchuers.getGoods_type();
			if (goods_type != null && goods_type.length() > 0) {
				goods_type = "," + goods_type + ",";
				for (int y = 0; y < goods_types.length; y++) {
					flg = false;
					if (goods_types[y] != null && !"".equals(goods_types[y])) {
						if (goods_type.indexOf(goods_types[y]) != -1) {
							flg = true;
						}
					}
					if (!flg) {
			...
			if (temp_goods_ids != null && temp_goods_ids.length() > 3) {
				for (int y = 0; y < goods_ids.length; y++) {
					if (temp_goods_ids.indexOf(goods_ids[y]) != -1) {
						m_num = m_num + buy_nums[y].intValue();
						m_price = m_price + prices[y].floatValue() * buy_nums[y].intValue();
					}
				}
			}
			...
			if (m_num > 0) {
				flg = true;
				if (vouchuers.getM_num() != null && m_num >= vouchuers.getM_num().intValue()) {
					// log.info("--------m_num:"+m_num+"vouchuers:m_num"+vouchuers.getM_num());
				} else if (vouchuers.getM_num() != null) {
					flg = false;
				}

				if (vouchuers.getM_menoy() != null && m_price >= vouchuers.getM_menoy().floatValue()) {
					// log.info("--------M_menoy:"+m_price+"vouchuers:M_menoy"+vouchuers.getM_menoy());
				} else if (vouchuers.getM_menoy() != null) {
					flg = false;
				}
			} else {
				flg = false;
			...
			// 未通过条件判断,删除 不显示, 如果需要显示 不能使用的购物券 vouchuers.geMap.put("flg", flg);
			// 中增加标识
			if (flg) {
				tempList.add(vouchuers);
			}
		}
		return tempList;
	}
