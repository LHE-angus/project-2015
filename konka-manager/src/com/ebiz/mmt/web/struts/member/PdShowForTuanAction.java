 ...
 * @version 2013-09-13
 */
public class PdShowForTuanAction extends BaseMemberAction {
	...
	public ActionForward list(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (null != ecList && ecList.size() > 0) {
			ecg1 = ecList.get(0);
			konkaBcompPd.getMap().put("start_time", sf.format(ecg1.getStart_time()));
			konkaBcompPd.getMap().put("end_time", sf.format(ecg1.getEnd_time()));
		} else {
			super.renderJavaScript(response, "window.onload=function(){alert('网络异常，请稍后重试！');location.href='"
			        + super.getCtxPath(request) + "/member/PdShowForTuan.do?method=list&goods_id=100018651';}");
			return null;
		}
		...
		konkaBcompPd.getMap().put("buy_num", num1 * 20);
	...
	public ActionForward stepTwo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		return new ActionForward("/../member/PdShowForTuan/step_two.jsp");
	}
