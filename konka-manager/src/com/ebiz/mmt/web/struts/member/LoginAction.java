...
public class LoginAction extends BaseAction {
	...
	public ActionForward showLoginForm(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		if (null != userInfo&&"1".equals(session.getAttribute("memberUser"))) {
	...
	public ActionForward login(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		if (userInfo.getIs_epp_fgs()!=null&&userInfo.getIs_epp_fgs().intValue() == 1) {
			super.renderJavaScript(response, "window.onload=function(){alert('对不起，您不是EPP工卡用户');location.href='login.do'}"); 
			return null;
		}
		if (!f){
			String logmsg = createLoginErrLogs("table_ec_user_"+user_name, password, 5, 30);
			...
			session.setAttribute("memberUser", "1");
			...
			//会员自动升级	
			if(userInfo.getUser_type().intValue()==1&&userInfo.getEcBaseCardLevel()!=null&&userInfo.getEcBaseCardLevel().getCard_level_name()!=null){
				String inte= super.getFacade().getEcUserScoreRecService().getEcUserScoreRecForPayJf(userInfo.getId());
				Float jf =new Float(0);
				if(inte!=null&&!"".equals(inte)){ jf=new Float(inte); } 
				if (userInfo.getEcBaseCardLevel().getCard_level_name().equals("金卡会员")&&jf.intValue()>=100000){
					...
					if(entity.getPay_pwd()==null||"".equals(entity.getPay_pwd())){
						entity.setPay_pwd(entity.getPass_word());
					}
					entity.getMap().put("password", new DESPlus().decrypt(entity.getPass_word()));
					entity.getMap().put("pay_pwd", new DESPlus().decrypt(entity.getPay_pwd()));
	...
	public ActionForward view(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		return new ActionForward("/../member/view.jsp");
	}
	...
	public ActionForward cache(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		super.renderJavaScript(response,"window.onload=function(){alert('缓存已刷新');location.href='" + super.getCtxPath(request)+ "/member/login.do';}");
		return null;			
	}
