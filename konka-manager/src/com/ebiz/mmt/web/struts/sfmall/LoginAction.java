...
public class LoginAction extends BaseAction {
	...
	public ActionForward showLoginForm(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		...
		if (null != userInfo&&"1".equals(session.getAttribute("sfmallUser"))) {
		...
		img.setNews_module(100230L); // 在线商城首页轮播图
	...
	public ActionForward login(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		//增加登录错误次数验证
		String valiMsg = validateLoginErrLogs("table_ec_user_"+user_name, 5, 30);
		if (null != valiMsg) {
				super.renderJavaScript(response, "window.onload=function(){alert('" + valiMsg+ "');location.href='login.do'}");
				return null;
		}
		...
		if (!f){
			...
			if(userInfo.getUser_type().intValue()==2){
				if(userInfo.getCust_id()==null){
					msg = super.getMessage(request, "login.failed.usertype.invalid");
					super.renderJavaScript(response, "window.onload=function(){alert('" + msg + "');location.href='login.do'}");
					return null;
				}
			}else if(userInfo.getPay_pwd()!=null&&userInfo.getPay_pwd().equals(entity.getPass_word())&&!userInfo.getPass_word().equals(entity.getPass_word())){ 
				session.setAttribute("sfmall", "1");
			}else{
				session.setAttribute("sfmall", "");
			}
			...
			session.setAttribute("sfmallUser", "1");
			...
			t.setOper_type(type);
			String name = userInfo.getUser_name() + "-登录顺丰商城系统成功";
			t.setPpdm_name(name);
			if (userInfo != null) {
			...
			// 更新购物车
			// 取cookie中购物车商品信息 
			Cookie shopingCArCookie = WebUtils.getCookie(request, "SHOPING_CAR_COOKIE"); 
			List<HashMap<String, String>> shopingCarCookieList = new ArrayList<HashMap<String, String>>();
			if(shopingCArCookie!=null){
				String shopingCArCookieValue = URLDecoder.decode(shopingCArCookie.getValue(), "UTF-8");
				if (shopingCArCookieValue != null && !"".equals(shopingCArCookieValue)) {
					shopingCarCookieList = ShoppingCarJsonUtil.jsonToList(shopingCArCookieValue);
				}
			}
			// 取table中购物车商品信息
			...
			// 合并购物车
 			List<HashMap<String, String>> shopingCarList = ShoppingCarJsonUtil.joinShoppingCarList(shopingCarCookieList,shoppingCarTableList);
			// 保存购物车
			if (shopingCarList != null && shopingCarList.size() > 0) {
					...
					super.getFacade().getEcShoppingCartService().removeEcShoppingCartWithGoodsIdAndUserId(esc);//清空购物车table
					// 写table
					List<HashMap<String, String>> newcookielist = new ArrayList<HashMap<String, String>>();
					for (int i = 0; i < shopingCarList.size(); i++) {
					...
					Cookie cookie = new Cookie("SHOPING_CAR_COOKIE",null); //清空旧cookie
					cookie.setMaxAge(0);  
					response.addCookie(cookie);  
					ShoppingCarJsonUtil shoppingCarUtil = new ShoppingCarJsonUtil();//创建新cookie
	...
	public ActionForward ajax(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		if (null == userInfoList || userInfoList.size() == 0) {
			...
			String type = "账户名：" + user_name + ",密码：" + password;
			t.setOper_type(type);
			String name = user_name + "用户不存在，登录开心猫顺丰商城失败";
			t.setPpdm_name(name);
			t.setOper_uid(0l);
			t.setOper_uname(user_name);
			getFacade().getOperLogService().createOperLog(t);
			...
			String type = "账户名：" + user_name + ",密码：" + password;
			t.setOper_type(type);
			String name = user_name + "用户名重复，登录开心猫顺丰商城失败";
			t.setPpdm_name(name);
			t.setOper_uid(0l);
			t.setOper_uname(user_name);
			getFacade().getOperLogService().createOperLog(t);
		...
		if (!f){
			...
			String type = "账户名：" + userInfo.getUser_name() + ",密码：" + userInfo.getPass_word();
			t.setOper_type(type);
			String name = userInfo.getUser_name() + "-登录开心猫顺丰商城系统成功";
			t.setPpdm_name(name);
			if (userInfo != null) {
				t.setOper_uid(userInfo.getId());
				t.setOper_uname(userInfo.getUser_name());
			...
			if (ecShoppingcarList != null && ecShoppingcarList.size() > 0) {
				for (EcShoppingCart ecShoppingCart : ecShoppingcarList) {
					...
					map.put("img_url", "" + ecShoppingCart.getImg_url());
					map.put("p_index", "" + ecShoppingCart.getP_index());
					if( ecShoppingCart.getService_ids()!=null){
						map.put("service_ids", "" + ecShoppingCart.getService_ids());
					}else{
						map.put("service_ids", "");
					}
