...
public class BaseMemberAction extends BaseAction {
	...
	public List<KonkaBcompPd> getKonkaBcompPdWithPindexAndTypeAndCountList(Integer own_sys, Long p_index,
		...
		if (2 == own_sys && null != dept_sn_array) { // 触网项目产品处理
			konkaBcompPd.getMap().put("goods_id_not_in_sfmall_by_deptids", dept_sn_array);
	 ...
	 */
	public List<KonkaBcompPd> getKonkaBcompPdWithPindexAndTypeAndCountList(Integer own_sys, Long p_index,
	        Integer label_of_cate, Integer count, Long dept_id, Long c_id, int prod_types[]) {
		// 处理地区信息
		Long[] p_index_array = new Long[4];
		p_index_array[0] = -1L;
		p_index_array[1] = -1L;
	...
	public List<EcGoodsPrice> getEcGoodsPriceListWithPindexAndGoodsId(Integer own_sys, Long goods_id,
		...
		if (2 == own_sys && null != dept_id_array) { // 处理触网的价格
			...
			ecGoodsPrice.getMap().put("sfmall_r3_user_id", dept_id_array[2]);
			ecGoodsPrice.getMap().put("sfmall_fgs_id", dept_id_array[1]);
		}
	...
	protected EcUser getSfEcUser(HttpServletRequest request) throws Exception {
		HttpSession session = request.getSession();
		String sfmallUser = (String) session.getAttribute("sfmallUser");
		if (!"1".equals(sfmallUser)) {
			session.setAttribute(Constants.EPP_USER, null);
		}
		...
		if (ecUser == null) {
			EcUser user = new EcUser();
			user.setIs_del(0);
			List<EcUser> userList = getFacade().getEcUserService().getEcUserList(user);
			if (userList.size() > 0) {
				ecUser = userList.get(0);
			}
		}
		return ecUser;
	}
