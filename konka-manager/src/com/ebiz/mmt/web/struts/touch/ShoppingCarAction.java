...
public class ShoppingCarAction extends BaseMemberAction {
	...
	public ActionForward list(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		for (EcShoppingCart temp : ecShoppingCartList) {
			...
			// 查询产品价格
			List<EcGoodsPrice> ecGoodsPriceList = getEcGoodsPriceListWithPindexAndGoodsId(ecUser.getUser_type(),
			...
			// 查询库存
			List<EcStocks> ecStocksList = getEcStocksListWithPindexAndGoodsId(ecUser.getUser_type(), konkaBcompPd
			        .getId(), p_index_array, null);
			int r3store_num = 0;
				...
				try {
					if (konkaBcompPd != null) {
						if (null != ecStocksList && 0 != ecStocksList.size() && ecStocksList.get(0) != null) {
							...
							} else if (stock_type.intValue() == 2 && s.getStocks().intValue() < 1
							        && ecStocksList.size() > 1) {
		...
		return new ActionForward("/../touch/ShoppingCar/step_one.jsp");
	}
	...
	public ActionForward buyByGoodsId(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		// 查询库存
		List<EcStocks> ecStocksList = getEcStocksListWithPindexAndGoodsId(ecUser.getUser_type(), konkaBcompPd.getId(),
		        p_index_array, null);
		int r3store_num = 0;
		if (konkaBcompPd.getProd_type() != null && konkaBcompPd.getProd_type().intValue() == 0) {
			try {
		...
		if (goods_ids != null && goods_ids.length > 0) {
			for (int i = 0; i < goods_ids.length; i++) {
				...
				if (konkaBcompPd != null) {
					...
					// 查询产品价格
					ecGoodsPriceList = getEcGoodsPriceListWithPindexAndGoodsId(ecUser.getUser_type(), konkaBcompPd
					        .getId(), p_index_array, ecUser.getDept_id(), ecUser.getCust_id());
					if (null != ecGoodsPriceList && 0 != ecGoodsPriceList.size()) {
						konkaBcompPd.setEcGoodsPrice(ecGoodsPriceList.get(0));
					}
					// 查询库存
	...
	public ActionForward stepTwo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		for (String id : goods_ids) {
			...
			konkaBcompPd.getMap().put("original_price", (String) dynaBean.get("original_price_" + id));
			map.put("original_price", (String) dynaBean.get("original_price_" + id));
		...
		return new ActionForward("/../touch/ShoppingCar/step_two.jsp");
	}
	...
	public ActionForward createOrder(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		for (int i = 0; i < json.size(); i++) {
			...
			BigDecimal price = obj.getBigDecimal("original_price");// 触网订单存市场价
		...
		if (payPrice.doubleValue() < 0.001 || pshowOrder.getPay_way() == 0 || pshowOrder.getPay_way() == 1
			...
			super.renderJavaScript(response, "window.onload=function(){location.href='" + super.getCtxPath(request)
			        + "/touch/Payment.do?method=view&trade_index=" + trade_index + "';}");
			return null;
		}
		super.renderJavaScript(response, "window.onload=function(){location.href='" + super.getCtxPath(request)
		        + "/touch/Payment.do?trade_index=" + trade_index + "&pay_way=" + pshowOrder.getPay_way() + "';}");
		return null;
	}
	...
	public ActionForward checkPayPwd(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if ("1".equals(show)) {
			...
			return new ActionForward("/../touch/ShoppingCar/checkpwd.jsp");
		}
