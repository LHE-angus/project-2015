...
<body style="font-family:Microsoft Yahei;">
	...
	<html-el:form action="/manager/JxcKonkaOrderRegisterV1.do">
	...
	<div class="rtabcont1">
	  ...
	  <input name="input" type="button" id="gotoAdd" class="bgButtonAdd" value="新 增" onclick="location.href='${ctx}/customer/manager/JxcKonkaOrderRegisterV1.do?method=add&mod_id=${af.map.mod_id}&cust_id=${af.map.cust_id}'" />
	</div>
	<div class="rtabcont1">
	    ...
	    <c:forEach items="${konkaOrderInfoList}" var="cur" varStatus="vs">
	      <tr>
	        ...
	        <td align="center" nowrap="nowrap">
	        	<span style="cursor:pointer;"  class="fblue"  onclick="doNeedMethod(null, 'JxcKonkaOrderRegisterV1.do', 'view','&mod_id=${af.map.mod_id}&order_id=${cur.id}&'+$('#bottomPageForm').serialize())">查看明细</span>
	         	<c:if test="${cur.is_submit eq 1}">
	        		<span style="color:#ccc;"title="订单已经提交，不能修改！">修改</span>
	        	</c:if>
	        	<c:if test="${cur.is_submit eq 0}">
	        		<span style="cursor:pointer;"  class="fblue"  onclick="doNeedMethod(null, 'JxcKonkaOrderRegisterV1.do', 'edit','&mod_id=${af.map.mod_id}&order_id=${cur.id}&cust_id=${cur.cust_id}&'+$('#bottomPageForm').serialize())">修改</span>
	        	</c:if>
	        </td>
	      </tr>
	      <c:if test="${vs.last}">
	  ...
	  <c:if test="${not empty konkaOrderInfoList}">
	    <div class="rtabcont3">
	      <form id="bottomPageForm" name="bottomPageForm" method="post" action="JxcKonkaOrderRegisterV1.do?method=list">
