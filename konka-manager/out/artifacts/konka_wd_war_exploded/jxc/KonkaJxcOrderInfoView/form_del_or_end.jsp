...
<div style="width:100%">
  ...
  <div class="rtabcont1">
    <html-el:form action="/KonkaJxcOrderInfoView.do">
     <c:if test="${af.map.type_operate eq 1}">
     <html-el:hidden property="method" value="delete" />
     </c:if>
     <c:if test="${af.map.type_operate eq 2}">
     <html-el:hidden property="method" value="end" />
     </c:if>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="1" class="tableClass">
        ...
        <tr>
          ...
          <td><font color="red">${fn:escapeXml(af.map.trade_index)}</font></span></td>
        </tr>
        <tr>
          <td class="title_item" width="15%">配送方式：</td>
          <td colspan="3"><c:if test="${af.map.send_type eq 1}">自提</c:if>
        ...
        <tr>
          <td colspan="4"><table width="100%" border="0" cellpadding="0" cellspacing="1" class="tableClass">
              ...
              <tr class="title_top">
                ...
                <td width="8%" nowrap="nowrap">折扣金额</td>
                <td width="8%" nowrap="nowrap">产品备注</td>
              </tr>
        ...
        <tr>
          <td class="title_item" width="15%"><c:if test="${(af.map.audit_state eq 0) or (af.map.audit_state eq 1)}"> 当前审核角色： </c:if>
        ...
        <c:if test="${not empty konkaOrderAuditList}">
          <c:forEach items="${konkaOrderAuditList}" var="cur" varStatus="st">
            <tr>
              <td class="title_item" width="15%">${fn:escapeXml(cur.map.role_name)}审核信息：</td>
              <td colspan="3"> 审核结果【
                <c:if test="${cur.audit_result eq -1}">不同意</c:if>
                <c:if test="${cur.audit_result eq 1}">同意</c:if>
                】
                &nbsp;&nbsp;审核意见【
                <c:if test="${empty cur.audit_comment}">无</c:if>
                <c:if test="${not empty cur.audit_comment}">${fn:escapeXml(cur.audit_comment)}</c:if>
                】
                &nbsp;&nbsp;审核人【${fn:escapeXml(cur.audit_user)}】
                &nbsp;&nbsp;审核时间【
                <fmt:formatDate value="${cur.audit_datetime }" pattern="yyyy-MM-dd HH:mm:ss"></fmt:formatDate>
                】 </td>
            </tr>
          </c:forEach>
        </c:if>
        <tr>
          ...
          <c:if test="${af.map.type_operate eq 1}">
          <html-el:button property="btn_del" styleClass="bgButtonSave" styleId="btn_del" value="废除"/>
          </c:if>
          <c:if test="${af.map.type_operate eq 2}">
          <html-el:button property="btn_end" styleClass="bgButtonSave" styleId="btn_end" value="完结"/>
          </c:if>
            &nbsp;&nbsp;
...
$("#btn_del").click(function(){
	if (Validator.Validate(this.form, 1)){
		$("#btn_del").attr("value", "正在提交...").attr("disabled", "true");
        $("#order_back").attr("disabled", "true");
		f.submit();
	}
});
$("#btn_end").click(function(){
	$("#btn_end").attr("value", "正在提交...").attr("disabled", "true");
    $("#order_back").attr("disabled", "true");
	f.submit();
});
//]]></script>
