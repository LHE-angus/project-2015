  ...
  <div class="rtabcont1">
    <html-el:form action="/KonkaJxcHhApply.do" enctype="multipart/form-data">
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="1" class="tableClass">
        <tr>
          <th colspan="2">换货信息</th>
        </tr>
        <tr>
          <td class="title_item" width="10%">产品类别：</td>
        ...
        <tr>
          <td class="title_item">审批状态：</td>
          <td><c:if test="${af.map.is_audit eq 0}">未审批</c:if>
            <c:if test="${af.map.is_audit eq -1}"><span style="color:#F00;">未通过</span></c:if>
            <c:if test="${af.map.is_audit eq 1}"><span style="color:#060;">已通过</span></c:if></td>
        </tr>
        <c:if test="${af.map.is_audit eq 1}">
          <tr>
            <td class="title_item">审批原因：</td>
            <td>${fn:escapeXml(af.map.audit_reason)}</td>
          </tr>
        </c:if>
        <tr>
          <td class="title_item">是否确认：</td>
          <td><c:if test="${af.map.hh_is_confirm ne 1}"><span style="color:#F00;">未确认</span></c:if>
            <c:if test="${af.map.hh_is_confirm eq 1}"><span style="color:#060;">已确认</span></c:if></td>
        </tr>
        <tr>
