...
<div class="rtabcont1">
  <html-el:form action="/KonkaJxcShopStatisticsByArea">
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tableTop">
      <tr>
        <td nowrap="nowrap"><strong class="fb"><span style="color:red">*</span>管辖区域：</strong>
          <html-el:select property="area_code_search" styleId="area_code_search" style="width:160px;">
          ...
          <html-el:select property="is_agent" styleId="is_agent" style="width:80px;">
            <html-el:option value="1">是</html-el:option>
            <html-el:option value="0">否</html-el:option>
          </html-el:select>
          &nbsp;&nbsp;<strong class="fb">网点：</strong>
          <html-el:select property="id" styleId="id" style="width:160px;">
            <html-el:option value="">请选择...</html-el:option>
         ...
         <strong class="fb">R3编码：</strong>
          <html-el:text property="r3_code_like" styleClass="webinput" styleId="r3_code_like" />
          &nbsp;&nbsp;
          <input class="bgSearch" type="button" name="buttonSearch" id="buttonSearch" value="搜 索" /></td>
      </tr>
      <!--<tr>
        <td nowrap="nowrap">
       &nbsp;&nbsp;<strong class="fb">起止日期：</strong>
       <html-el:text property="start_date" styleId="start_date" size="9" maxlength="9" readonly="true" styleClass="webinput" style="cursor:pointer;text-align:left;width:80px;" onclick="new Calendar(2011, 2031, 0).show(this);" />
        &nbsp;&nbsp;<span>至</span>
        <html-el:text property="end_date" styleId="end_date" size="9" maxlength="9" readonly="true" styleClass="webinput" style="cursor:pointer;text-align:left;width:80px;" onclick="new Calendar(2011, 2031, 0).show(this);" /></td>
        <td><input class="bgSearch" type="button" name="buttonSearch" id="buttonSearch" value="搜 索" /></td>
      </tr>
    -->
    </table>
  </html-el:form>
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="1" class="tableClass">
      ...
      <c:forEach items="${konkaR3ShopList}" var="cur" varStatus="status">
        <tr>
          ...
          <td align="center"><span style="cursor:pointer;"  class="fblue"  onclick="doNeedMethod(null, 'KonkaJxcShopStatisticsByArea.do', 'view','id=${cur.id}&shop_name=${cur.customer_name}&'+$('#bottomPageForm').serialize())">报表</span>
            <c:if test="${af.map.is_agent eq 1}"> <span style="cursor:pointer;"  class="fblue"  onclick="doNeedMethod(null, 'KonkaJxcShopStatisticsByArea.do', 'list','agent_id=${cur.id}&'+$('#listForm').serialize())">代理商下网点</span> </c:if></td>
        </tr>
      </c:forEach>
    </table>
...
$(document).ready(function(){
	...
	$(".bgSearch").click(function(){
		...
		$("#area_code_search").attr("dataType", "Require").attr("msg", "请选择区域");
