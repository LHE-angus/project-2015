...
<div class="oarcont" style="position:relative;overflow:hidden;">
  ...
  <div class="rtabcont2">
    <html-el:form action="/zmd/KonkaXxZmdAddSalesOrder">
      <html-el:hidden property="method" value="${af.map.stock_from eq 1 ? 'save' : 'save2'}" />
      <html-el:hidden property="mod_id" />
      <html-el:hidden property="sell_bill_id" />
      <html-el:hidden property="sell_state" />
      <html-el:hidden property="area_id" value="${af.map.area_id}" />
      <html-el:hidden property="queryString" />
      <html-el:hidden property="stock_from" />
      <table id="main_table" width="100%" border="0" cellspacing="0" cellpadding="0" class="rtable3">
        <!-- 销售单信息Begin -->
        <tr>
          <td colspan="3" style="font-weight:900;">销售单信息
          ...
          <td align="right"><c:if test="${af.map.return_money_0 eq 1}"><a style="color: blue;cursor: pointer;font-weight:900;" href="${ctx}/manager/zmd/KonkaXxZmdAddSalesOrder.do?method=add&mod_id=${af.map.mod_id}">切换到返利模式订单</a></c:if></td>	
        </tr>
        <tr>
        ...
        <tr>
          <td colspan="4" style="font-weight:900;">商品库存来源：${af.map.stock_from eq 1 ? "分公司库存" : "专卖店库存"}。
            <!-- <label for="order_cate_0" style="cursor:pointer;"><input type="radio" name="order_cate" value="0" id="order_cate_0" /> 分公司统一库存</label><span style="font-weight:normal;">（“${af.map.dept_name}”分公司负责发货）</span>
		  			<label for="order_cate_1" style="cursor:pointer;"><input type="radio" name="order_cate" value="1" id="order_cate_1" checked="checked" /> 专卖店自有库存</label> --></td>
        </tr>
        <tr>
          <td colspan="4" align="left" style="padding-top:5px;padding-bottom:5px;"><div id="div_2">
              ...
              <table width="100%" border="0" cellpadding="0" cellspacing="1" class="rtable2">
                ...
                <tr id="pdModel" style="display:none;height:40px;">
                  ...
                  <td align="center" nowrap="nowrap"></td>
                  <td align="center" nowrap="nowrap"><c:if test="${af.map.stock_from eq 1}">
                    ...
                    <c:if test="${af.map.stock_from eq 0}">
                      <html-el:text property="counts" styleId="counts" style="width:40px;border:0;border-bottom:1 solid black;" maxlength="5" value="1" />
                    </c:if></td>
                ...
                <tbody id="showSalesOrderTbody">
                  <c:forEach items="${sellBillDetailsList}" var="cur_" varStatus="vs">
                    <tr id="hx_tr_${vs.count}">
                      ...
                      <td align="center"><c:if test="${af.map.stock_from eq 1}">
                          <html-el:text property="counts" styleId="counts" style="width:40px;cursor:pointer;border:0;border-bottom:1 solid black;background:#CBF7C5;" maxlength="5" value="${cur_.counts}" readonly="true" onclick="openDeliveryDiv('${cur_.md_name}', '${cur_.pd_cls_name}', $('#hx_tr_${vs.count}'), '${af.map.reserverd_stock}')" />
                        </c:if>
                        <c:if test="${af.map.stock_from eq 0}">
                          <html-el:text property="counts" styleId="counts" style="width:40px;cursor:pointer;border:0;border-bottom:1 solid black;background:#CBF7C5;" maxlength="5" value="${cur_.counts}" />
                        </c:if></td>
        ...
        <!-- 销售单状态End -->
        <tr>
          <td colspan="4" height="40"  align="center"><input type="hidden" name="pay_way" value="1" />
  ...
  <div id="message_tip_2" style="display:none;">
    ...
    <div style="position:fixed;width:280px;height:130px;left:35%;top:30%;padding:10px;" class="ui-widget ui-widget-content ui-corner-all">
      <div style="width:95%;margin-bottom:10px;margin-top:10px;font-size:15px;"><img src="${ctx}/styles/message/images/16.gif" />&nbsp;恭喜您，添加销售单成功！
        <c:if test="${!empty jf_count}">您将获得${jf_count}积分</c:if>
      </div>
		...
		$("#pdIdList", lastTR).multiselect({
			...
			click: function(event, ui){
		       if(ui.value != ""){
				   ...
				   if ("${af.map.stock_from}" == 0) $("#counts", lastTR).val(1).removeAttr("readonly", "true");
				   ...
				   if ("${af.map.stock_from}" == 1) $("#ck", lastTR).attr("dataType", "Require").attr("msg", "请选择产品【" + md_name + "】库位填写购买数量！");

				   $("#counts", lastTR).click(function(){
					   if ("${af.map.stock_from}" == 0) return;
...
function getPdStocks(zmd_id, md_name, parObj) {
	...
	if ("${af.map.stock_from}" == "0") {
		flag = true;
		parObj.children().eq(4).html("<span style='color:green;'>有</span>");
		return flag;
	}
	...
	$.ajax({
        ...
        success: function (result) {
            if ("" == result[0].pdStocksNum || undefined == result[0].pdStocksNum) {
            	...
            	if (n > 0) parObj.children().eq(4).html("<span style='color:green;'>有</span>");
