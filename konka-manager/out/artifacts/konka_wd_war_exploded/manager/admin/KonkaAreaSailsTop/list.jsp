...
<div class="oarcont" id="body_oarcont">
  ...
  <html-el:form action="/admin/KonkaAreaSailsTop">
  ...
  <h1 align="center" style="padding-top: 20px;font-size: 22px;">${yyyy}年${mm}月份大区彩电销售业绩排名</h1>
  ...
  <div class="rtabcont1">
    ...
    <table class="rtable6" width="100%" cellspacing="1" cellpadding="0" border="0">
      <tr class="tabtt6">
        ...
        <td width="6%" align="center" rowspan="3">大区</td>
      ...
      <c:forEach var="cur" items="${entityList}" varStatus="vs">
	      <tr>
	        ...
	        <td nowrap="nowrap" align="left">
	          <c:choose>
	        	<c:when test="${cur.map.p_area eq 10}">华东</c:when>
	        	<c:when test="${cur.map.p_area eq 20}">山东</c:when>
	        	<c:when test="${cur.map.p_area eq 30}">东北</c:when>
	        	<c:when test="${cur.map.p_area eq 40}">华北</c:when>
	        	<c:when test="${cur.map.p_area eq 50}">华南</c:when>
	        	<c:when test="${cur.map.p_area eq 60}">西南</c:when>
	        	<c:when test="${cur.map.p_area eq 70}">华中</c:when>
	        	<c:when test="${cur.map.p_area eq 80}">西北</c:when>
	          </c:choose>
	        </td>
     ...
     <tr>
       ...
       <td nowrap="nowrap" align="right" style="font-weight:600;"><fmt:formatNumber value="${t_cur_money_of_month_task}" pattern="###,##0.00" /></td>
