...
<div id="navTab" class="tabsPage" style="text-align:left;">
  <div class="navTab-panel tabsPageContent">
    <div class="pageContent">
      <html-el:form action="/admin/SelectPePdModel">
        <html-el:hidden property="queryString" styleId="queryString" />
        <html-el:hidden property="method" styleId="method" value="save" />
        <html-el:hidden property="rowCount" styleId="rowCount" value="${af.map.rowCount}" />
        <html-el:hidden property="id" styleId="id" />
        <div style="height:5px;"></div>
        <table width="100%" border="0" align="left" class="list">
          <tr>
            <td><html-el:select property="cls_id" styleId="cls_id" onchange="getQueryNames($('#key_word').val());">
                <option value="">-请选择品类-</option>
                <html-el:optionsCollection label="tree_name" value="cls_id"  name="clsList" />
              </html-el:select>
              搜索关键字 ：
              <html-el:text property="key_word" styleId="key_word" size="24" maxlength="100" style="width:210px;" title="可以输入中文、全拼、简拼进行关键字即时搜索" /></td>
          </tr>
          <tr>
                  ...
                  <td>已选择区
                    <html-el:select property="select2" multiple="true" style="width:290px;height:358px;" styleId="select2" onchange="moveSelected(this.form.select2, this.form.select1);">
                      <c:if test="${not empty selectList}">
                        <html-el:optionsCollection label="md_name" value="pd_id"  name="selectList" />
                      </c:if>
                    </html-el:select></td>
                </tr>
              </table></td>
...
$(document).ready(function(){
	...
	$("#btn_submit").click(function(){
		...
		if(rowCount!=""){
			if ($("#select2 option").length > rowCount){
				alert("不能选择超过"+ rowCount +"个型号");
				return false;
			}
		}
		window.returnValue = {
...
function getQueryNames(key_word) {
   	...
   	$.ajax({
		type: "POST",
		cache: false,
		url: "${ctx}/manager/admin/SelectPePdModel.do",
