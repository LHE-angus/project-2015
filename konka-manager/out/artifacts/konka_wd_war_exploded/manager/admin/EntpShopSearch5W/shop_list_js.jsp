...
function openChild(num){
	...
	if(num == 1){
		var returnValue = window.showModalDialog("EntpShopSearch5W.do?method=listBaseBrandInfo&time="+now, window, "dialogWidth:800px;status:no;dialogHeight:680px");
	    if(returnValue != null){
		...
		var returnValue = window.showModalDialog("EntpShopSearch5W.do?method=listJdxxRootEntp&time="+now, window, "dialogWidth:800px;status:no;dialogHeight:680px");
	    if(returnValue != null){
			var value = returnValue.split(",");
		...
		var returnValue = window.showModalDialog("EntpShopSearch5W.do?method=listYjhxRootEntp&time="+now, window, "dialogWidth:800px;status:no;dialogHeight:680px");
	    if(returnValue != null){
			var value = returnValue.split(",");
		...
		success: function(oper) {
			if ("" != oper.result){
				...
				for(i = 0; i < strs.length - 1; i++){
					...
					html = html + "<li style='width:120px;float:left;'><input type='checkbox' name='brand_ids' id='brand_ids_" + temps[1] + "' value='" +  temps[1] + "' /><label for='brand_ids_"+ temps[1] +"'>&nbsp;" + temps[0] + "</label></li>";
				}
				if(strs.length > 80){
