...
function resetGMarkersOfEntpShopToMap(map, entp_shop_list_json){
	...
	for (var i = 0; i < entp_shop_list_json.length; i++) {
		...
		}else if(cur.is_R3Shop == "3"){
			letteredIcon.image = "${ctx}/images/marks/icon_c/icon_"+letter+".gif";
		}
	...
	$Container.find("li").click(function() {
		...
		getShopInfo($(this).attr("shop_id"),"1");
...
function resetGMarkersToMap(map, options) {
	...
	$("#pager_control").pager("${ctx}/manager/admin/KonkaShopMapFight.do", $.extend({}, {
		...
		$("#count_A").text(ret.count_A);// 设置R3用户数
		$("#count_B").text(ret.count_B);// 设置经销商数
		$("#count_C").text(ret.count_C);// 设置待开拓网点数
		$("#count_D").text(ret.count_D);// 设置5.5W其他网点数(不含R3用户,经销商,待开拓网点)		
		// $(".glatlng").val("");// 清空经纬度缓存
		// List
...
function setMapCenterByPIndex(map, p_index) {
	...
	$.post("${ctx}/manager/admin/KonkaShopMapFight.do", {method : 'ajaxGetPIndexNamesStr', p_index : p_index}, function(ret){
		if (!/^\s*$/.test(ret)) {
...
function writeDisplayEntpShopListHtml(ctx, entp_shop_list){
	...
	for (var i = 0; i < entp_shop_list.length; i++) {
		...
		if(cur.is_R3Shop == "1"){
			...
			$li.attr("is_R3Shop", "3");
			$li[0].style.backgroundColor = "#FFC";
		}else{
			$li.attr("is_R3Shop", "0");
		}
