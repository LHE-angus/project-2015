...
<div class="oarcont" id="body_oarcont">
      <html-el:form action="/od/SelectKonkaOaCategory">
        ...
        <div style="height:5px;"></div>
         <div class="rtabcont1">
        <table width="100%" border="0" align="left" class="rtable1">
          <tr>
            <td>&nbsp;&nbsp;&nbsp;
              搜索关键字（网点名称或R3编码） ：
              <html-el:text property="key_word" styleId="key_word" size="24" maxlength="100" style="width:210px;" title="可以输入中文、全拼、简拼进行关键字即时搜索" onchange="getQueryNames(this)"/>
            </td>
          </tr>
          <tr>
            <td>
...
function getQueryNames(key_word) {
   	...
   	$.ajax({
		type: "POST",
		cache: false,
		url: "${ctx}/manager/oa/SelectKonkaR3Shop.do",
		...
		success: function(data) {
			if (data.length >= 1) {
				for(var i = 0; i < data.length - 1; i++) {
					var opt = new Option( data[i].customer_name,data[i].r3_shop_id); 
