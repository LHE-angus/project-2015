...
<link rel=stylesheet type=text/css  href="${ctx}/styles/sfmall/css/shoppingcar.css" />
<style type="text/css">
...
.yhqz_div{position: absolute;border: solid 1px #d1e3f5;text-align: center;border:1px solid #E03425;background: #f5f4f4;left:30%;width:550px;padding-bottom: 0px;padding-top: 0px;display: none;z-index:10002;} 
...
<!-- buy start -->
<form action="<c:url value='/sfmall/ShoppingCar.do' />" method="post" id="step_two_form">
...
<div class="card_center">
  ...
  <div class="shoppingcart_c">
            ...
            <td><ul>
                <table border="0"> 
                    <tr>
                      ...
                      <td align="left" valign="middle"><label class="bank--bank_card" for="pay_way_6" style="cursor:pointer;">民生e支付</label>&nbsp;&nbsp;<div style="margin-top:0px;width:550px;">&nbsp;&nbsp;即时到账，支持绝大数银行借记卡和部分银行信用卡&nbsp;&nbsp;<a href="${ctx}/sfmall/KonkaGroupPeArticleInfo.do?method=view&id=837917" style="color: #08C;">查看银行及限额</a></div></td>
                     </tr> 
                </table>
              </ul> 
              </td>
                ...
                <table border="0">
                   <tr>
                      ...
                      <td align="left" valign="middle" colspan="5"><label for="pay_way_1" style="cursor:pointer;">户名：康佳集团股份有限公司
						帐号：744557963343
						开户行：中国银行深圳沙河支行（银行转账时请备注：康佳开心猫平台购机款 ,收款人手机:18207556878）</label>
						</td>
                     </tr> 
                </table>
              </ul> 
    ...
    <div class="HideBox" >
     ...
     <div class="yhqz_div" id="yhqz_div" style="display:none;z-index:10000;position:absolute;">
		<fieldset>		
			<h3 style="font-size:30px;text-align:center;font-weight:900;">温馨提示<span id="yhqzx_close" title="关闭" style="float:right;font-size:18px;color:red;cursor:pointer;">×</span></h3>
			...
			<div class="cookiechk">
				<input name="cancel" type="button" id="yhqzlogin_close" class="buttom"  style="margin-left: 225px"  value="确认" />
			</div>
		</fieldset>
	  </div>
      ...
      <tbody>
        ...
        <c:forEach items="${konkaBcompPdList}" var="cur">
	        <tr>
	          ...
	          <td style="border-bottom:solid 1px #E6E6E6;"><a href="<c:url value='/sfmall/PdShow.do?goods_id=${cur.id}' />" target="_blank"><c:if test="${fn:contains(cur.pd_name, 'HYUNDAI')}">HYUNDAI</c:if>
               <c:if test="${not fn:contains(cur.pd_name, 'HYUNDAI')}">康佳</c:if>${cur.pd_sn}</a></td>
    ...
    </table><c:if test="${ecUser.user_type eq 1 and sfmall eq 1 }">
  ...
  <div class="but_ca">
    <ul>
      <li><a href="<c:url value='/sfmall/ShoppingCar.do' />" id="step_one_go_shopping">返回购物车</a></li>
...
$(document).ready(function(){
	...
	$(document).delegate("#bill_head_0, #bill_head_1", "click", function(){
	 ...
	 $("#yhqzlogin_close").click(function(){$("#chk_pic_print").removeAttr("checked");$("#show_1").css("display",'none');$("#yhqz_div").fadeOut(500);});  
	 $("#yhqzx_close").click(function(){$("#chk_pic_print").removeAttr("checked");$("#show_1").css("display",'none');$("#yhqz_div").fadeOut(500);}); 
...
function showDialog(){   
	 if(is_v==0){
		...
		$("#val_div").html("<font color='red'>验证支付密码...</font>");
	  	var url="<c:url value='/sfmall/ShoppingCar.do' />?method=checkPayPwd&time="+Math.random();  
