  ...
  <div id="top_topt">
    <div class="top_top1">
      <div class="top_topr">欢迎您！
        <c:if test="${not empty sfmallUserInfo}">${sfmallUserInfo.real_name}&nbsp;|<a href="${ctx}/sfmall/center/Index.do">会员中心</a>|<a href="${ctx}/sfmall/login.do?method=logout">退出登陆</a></c:if>
        <c:if test="${empty sfmallUserInfo}">[<a id="top_login" style="cursor:pointer;">请登录</a>]</c:if>
        </div>
      <div class="top_topl">
        <h4><cite><img src="${ctx}/styles/sfmall/images/ico_2.gif" /></cite>&nbsp;&nbsp;<a href="<c:url value='/sfmall/manager/Index.do' />">欢迎光临康佳触网平台!</a></h4>
       
      </div>
      <div class="top_clear"></div>
    </div>
  </div>
</div><c:if test="${empty sfmallUserInfo}">
<div id="login" style="display:none;z-index:99999;">
  <form action="${ctx }/sfmall/login.do" method="post" style="margin:0;padding:0;">
...
$(document).ready(function(){
	...
	$(document).delegate("#top_login", "click", function(){
		// showLogin();
		location.href = "<c:url value='/sfmall/login.do' />";
	}); 
	//点击关闭按钮 
