...
<link rel=stylesheet type=text/css  href="${ctx}/styles/sfmall/css/jifen.css" />
      ...
      <div class="liscont1">
        ...
        <div class="jifen-box">
          <div class="jifen-font">
            <h1>${fn:escapeXml(ecUser.real_name)} [<a href="${ctx}/sfmall/login.do?method=logout">退出</a>]</h1>
            <span>兑换积分余额：${surplus_integral}积分</span><span>会员等级：${ecUser.ecBaseCardLevel.card_level_name} </span><span> <a href="#" alt="">可兑换商品</a> </span><span></span></div>
        </div>
        <div class="clear"></div>
      </div>
      ...
      <tbody>
        <tr>
          ...
          <td><div class="listul2">
              <div class="rpic2 box_295_220"><a title="" href="<c:url value='/sfmall/EcGift.do?method=view&gift_id=${cur.id}' />"><img alt="" src="${ctx}/${fn:substringBefore(cur.main_pic, '.')}.jpg" width="280" height="210" align="absmiddle" /></a></div>
              <div class="ltt2"><a href="<c:url value='/sfmall/EcGift.do?method=view&gift_id=${cur.id}' />">
                <h4>康佳&nbsp;${fn:escapeXml(cur.pd_sn)}</h4>
                <font class="redfont">${fn:escapeXml(cur.pd_name)}</font></a></div>
              <div class="ltt3">
                <h3><font class="jifen-font">所需积分:</font>${fn:escapeXml(cur.need_integral)}</h3>
                <a href="<c:url value='/sfmall/EcGift.do?method=stepTwo&gift_id=${cur.id}' />"><img alt="立即兑换" src="${ctx}/styles/sfmall/images/but_jifen.jpg" style="cursor:pointer;" /></a>
              </div>
            </div></td>
