...
<div class="maincont">
  ...
  <div class="touch_right padbot45">
    <div class="position"><a href="${ctx }/touch/Index.do">首页</a> &gt; <a href="${ctx }/touch/center/Index.do">会员中心</a> &gt; 我的订单</div>
    <div class="touchtab3">
          ...
          <c:forEach var="cur" items="${entityList}" varStatus="vs">
            <tr height="60">
              ...
              <td align="center" nowrap="nowrap"><c:forEach items="${cur.pshowOrdeDetailsList}" var="cur2">
                  ...
                  <a href="${ctx }/touch/PdShow.do?goods_id=${cur2.pd_id}"><c:forEach items="${imgs }" var="img1" begin="0" end="0"><img src="${ctx }/${img1}" alt="${cur2.map.pd_name }" width="51" height="50"/></c:forEach></a>
                  <br/>
                </c:forEach>
              </td>
              ...
              <td align="center" nowrap="nowrap"><c:if test="${cur.pay_way eq 0}">货到付款</c:if>
                ...
                <c:if test="${cur.pay_way eq 5}">民生e支付</c:if><c:if test="${cur.pay_way eq 9}">线下处理</c:if>
              </td>
              <td align="center" nowrap="nowrap"><a href="Orders.do?method=view&id=${cur.id}"><c:if test="${cur.state eq -30 }">退货成功</c:if>
                ...
                <c:if test="${cur.state eq 0 }"><c:if test="${cur.pay_way eq 0 or cur.pay_way eq 1}">待审核  </c:if> <c:if test="${cur.pay_way ne 9 and cur.pay_way ne 0 and cur.pay_way ne 1 }">待付款  </c:if></c:if>
                ...
                <c:if test="${cur.state eq 60 }">交易完成</c:if> <c:if test="${cur.pay_way eq 9}">&nbsp;线下处理</c:if></a>
              </td>
              <td align="center" nowrap="nowrap" style="line-height:23px;">
              	<a href="Orders.do?method=view&id=${cur.id}"><font color="#0022ff">订单详情</font></a> <br/>
               <c:if test="${cur.state eq 0 }"> <c:if test="${cur.pay_way ne 9 and cur.pay_way ne 0 and cur.pay_way ne 1}"> 
                <a class="btn btn-4" href="<c:url value='/touch/Payment.do?trade_index=${cur.trade_index}' />"><s></s>付款</a><br/>
                </c:if></c:if>
                <c:if test="${cur.state eq 0 }"><a href="#" id="t${cur.id }" onclick="deleteInfo(this);" ><font color="#0022ff">取消订单</font></a></c:if>
                <c:if test="${cur.pay_way ne 9}"><c:if test="${cur.state eq 60 }"> 
