...
<div class="card_center">
  ...
  <form action="<c:url value='/touch/ShoppingCar.do' />" method="post" id="shopping_car_form">
    ...
    <c:forEach items="${konkaBcompPdList}" var="cur" varStatus="vs">
	  <div class="card_content" id="div_${cur.id}">
	    ...
	    <ul>
	      ...
	      <li class="c_two">
	      <a href="<c:url value='/touch/PdShow.do?goods_id=${cur.id}' />" target="_blank" title="${cur.pd_name}">${fnx:abbreviate(cur.pd_name, 2 * 26, '...')}</a>
	      </li>
	      <li class="c_three"><table border="0" cellspacing="0" cellpadding="0"><tr><td height="70" width="100" style="overflow:hidden;" valign="middle" align="center"><a href="<c:url value='/touch/PdShow.do?goods_id=${cur.id}' />"><img alt="${cur.pd_name}" src="${ctx}/${fn:substringBefore(cur.main_pic, '.')}_060.jpg" style="max-width:60px;max-height:60px;" /></a></td></tr></table></li>
	      <li class="c_four">
	        <div class="fout_left"><img src="${ctx}/styles/touch/images/jian_1.jpg" width="11" height="11" class="num_jian_class" data="${cur.id}" style="cursor:pointer;" /></div>
	        ...
	        <div class="fout_right"><img src="${ctx}/styles/touch/images/jia_1.jpg" width="11" height="11" class="num_jia_class" data="${cur.id}" style="cursor:pointer;" /></div>
	      ...
	      <li class="c_service" style="padding:0;">
	      	<c:if test="${empty cur.serviceIds}"><span style="margin-left:10px;line-height:70px;">没有选择增值服务</span></c:if>
	      	<c:forEach items="${cur.ecBindingPdListForService}" var="cur1">
	      		<c:forEach items="${cur.serviceIds}" var="cur2">
	      			<c:if test="${cur1.id eq cur2}"><span style="margin-left:5px;line-height:18px;">${cur1.goods_name} ￥<fmt:formatNumber value="${cur1.price}" pattern="#,#00" /></span><br /></c:if>
	      		</c:forEach>
	      	</c:forEach>	      	
  ...
  <div class="but_card" style="font-size:14px;font-weight:bold;">
    <ul>
      <li><a href="<c:url value='/touch/Index.do' />" id="step_one_go_shopping">&lt;&lt; 继续购物</a></li>
