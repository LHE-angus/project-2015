...
<div style="width: 100%;">
  ...
  <form id="listForm" name="listForm" method="post" action="JxcHhApply.do?method=delete">
    <div class="rtabcont1">
      ...
      <input class="bgButtonAdd" type="button" name="Submit2" value="新 增" onclick="location.href='JxcHhApply.do?method=add&keySeq=${af.map.keySeq}';" />
    ...
    <div class="rtabcont1">
      <table width="100%" border="0" cellspacing="1" cellpadding="0" class="tableClass">
        <tr>
          ...
          <th width="7%" nowrap="nowrap" align="center">换货数量</th>
          <th  nowrap="nowrap" align="center">换货原因</th>
          <th width="12%" nowrap="nowrap" align="center">换货日期</th>
          <th width="7%" nowrap="nowrap" align="center">审批状态</th>
        ...
        <c:forEach var="cur" items="${entityList}" varStatus="vs">
          <tr>
            ...
            <td align="center">${fn:escapeXml(cur.hh_reason)}</td>
            ...
            <td align="center" nowrap="nowrap"><span style="cursor:pointer;"  class="fblue"  onclick="doNeedMethod(null, 'JxcHhApply.do', 'view','id=${cur.id}&'+$('#bottomPageForm').serialize())">查看</span> |
              <c:if test="${(cur.is_audit eq 1) and (cur.hh_is_confirm ne 1)}" var="canQr"> <span style="cursor:pointer;"  class="fblue"  onclick="doNeedMethod('点击确认后，将不能修改，是否确认？', 'JxcHhApply.do', 'confirm','id=${cur.id}&'+$('#bottomPageForm').serialize())">确认</span> |</c:if>
              <c:if test="${not canQr}"> <span style="color:#ccc;"  class="fblue"  >确认</span> | </c:if>
              <c:if test="${cur.is_audit ne 0}"><span style="color:#ccc;"  class="fblue"  >删除</span></c:if>
              <c:if test="${(cur.is_audit eq 0) and (cur.is_del eq 0)}"><span style="cursor:pointer;"  class="fblue"  onclick="confirmDelete(null, 'JxcHhApply.do','id=${cur.id}&'+$('#bottomPageForm').serialize())">删除</span></c:if></td>
          </tr>
  ...
  <form id="bottomPageForm" name="bottomPageForm" method="post" action="JxcHhApply.do">
