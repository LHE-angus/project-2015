...
<div style="width:100%">
  ...
  <div class="rtabcont1">
    <form action="<c:url value='/jxcnokey/JxcSellBill.do'/>" method="post">
  ...
  <div class="rtabcont1">
    ...
    <span style="padding-left: 500px;  "> <span style="color:#0f0;">*</span>表示订单中存在节能惠民产品   </span>
  </div>
  <div class="rtabcont1">
    <table width="100%" border="0" cellpadding="0" cellspacing="1" class="tableClass">
      ...
      <c:forEach items="${sellBillList}" var="cur" varStatus="status">
        <tr>
          ...
          <td  align="center">${fn:escapeXml(cur.sn)} <c:if test="${not empty cur.map.allowance_money}"><span style="color: #0f0;">*</span> </c:if> </td>
          <td align="center" >${fn:escapeXml(cur.map.customer_name)}</td>
          <td align="center" >${fn:escapeXml(cur.map.allowance_money)}</td>
          ...
          <td align="center" ><a href="#" onclick="go('${ctx}/jxcnokey/JxcSellBill.do?method=sellShow&sell_bill_id='+${cur.id})">查看明细</a>
 <!--          	&nbsp;<c:if test="${not empty cur.map.allowance_money}">
          	
          			<a href="#" onclick="go('${ctx}/jxcnokey/JxcSellBill.do?method=printBtqrd&sell_bill_id='+${cur.id})" title="打印补贴确认单"><span style="color:#060;">打印补贴确认单</span></a>
          		</c:if> -->
<!--          		<c:if test="${empty cur.map.allowance_money}"><span title="无节能惠民补贴，不可打印补贴确认单">打印</span></c:if>-->
<!--          		&nbsp;<a href="#" onclick="go('${ctx}/jxcnokey/JxcSellBill.do?method=fileUpload&sell_bill_id='+${cur.id})" title="上传身份证复印件、发票复印件和《“节能产品惠民工程”补贴确认函》"><span style="color:#060;">上传附件</span></a>-->
          	</td>
        </tr>
      </c:forEach>
      <tr>
  ...
  <c:if test="${not empty sellBillList}">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td height="36" align="center" ><form id="bottomPageForm" name="bottomPageForm" method="post" action="<c:url value='/jxcnokey/JxcSellBill.do?method=list'/>">
...
<script type="text/javascript">//<![CDATA[
	$(document).ready(function(){
		$("#gotoAdd").click(function(){
			go("${ctx}/jxcnokey/JxcSellBill.do?method=add");
		});
	...
	$("#print").click(function(){
		window.showModalDialog("${ctx}/jxcnokey/JxcSellBill.do?method=print&"+param, window, "dialogWidth:900px;status:no;dialogHeight:580px"); 
	});
//]]></script>
