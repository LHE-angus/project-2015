...
Validator = {
	...
	Mobile : /^((\(\d{2,3}\))|(\d{3}\-))?((13\d{9})|(14[0-9]\d{8})|(15[0-9]\d{8})|(18[0-9]\d{8})|(17[0-9]\d{8}))$/,
	...
	Validate : function(theForm, mode){
		...
		for(var i=0;i<count;i++){
			with(obj.elements[i]){
				// modified by zcwang for dataType1 msg1 and dataType2 msg2 handle start
				this.ClearState(obj.elements[i]);
				
				var dataTypes = [getAttribute("dataType"), getAttribute("dataType1"), getAttribute("dataType2")];
				var msgs = [getAttribute("msg"), getAttribute("msg1"), getAttribute("msg2")];
				var itemHasError = false;
				for (var j = 0; j < dataTypes.length; j++) {
					var _dataType = dataTypes[j];
					var _msg = msgs[j];
					if(typeof(_dataType) == "object" || typeof(this[_dataType]) == "undefined")  continue;
					// 该项目已经有错误就不再检查其他dataType的内容
					...
					switch(_dataType){
						...
						case "Filter" :
							...
							break;
						case "Ms" : // Fix select multiple can not get values problems! Fix by Wu,Yang 2011-03-12
							var multiSelect = obj.elements[i];
							if(multiSelect.multiple) {
								var hasChecked = 0;
								var min = getAttribute('min') || 1;
								var max = getAttribute('max') || multiSelect.options.length;
								for(var j = 0; j < multiSelect.options.length; j++) {  
									if(multiSelect.options[j].selected) {  
										hasChecked++  ;
									}  
								}
...
//						break;
//				}
				// modified by zcwang for dataType1 msg1 and dataType2 msg2 handle end
			}
		}
