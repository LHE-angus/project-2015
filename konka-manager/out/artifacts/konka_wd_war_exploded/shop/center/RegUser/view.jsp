   ...
   <c:if test="${ecUser.is_act ne 0 or ecUser.login_count <20 }">
    ...
    <div class="shoptab3" id="div_info">
     <p style="margin-left:20px;margin-top:15px;font-size:16px;line-height:36px;"> 
     	${ecUser.real_name }您好，
     	<c:if test="${is_act eq 0 }"> 您的资料已经填写完成，审核通过,欢迎 <a href="${ctx }/shop/Index.do">购物</a>,建议您先进入会员中心 查看您的会员权益!</c:if>
     	<c:if test="${is_act eq 1 }"> 您的资料没有填写完整 ，请完善 >> <a href="${ctx }/shop/center/RegUser.do?">用户资料</a> </c:if>
     	<c:if test="${is_act eq 2 }"> 您的资料已经填写完成，等待审核。。。审核结果稍后将发送到您登记的邮箱<br/></c:if>
     	<c:if test="${is_act eq 3 }"> 您的资料审核不通过，请重新完善 >> <a href="${ctx }/shop/center/RegUser.do?method=edit">用户资料</a> </c:if>
     </p>
    </div>
  </div>
</div>
