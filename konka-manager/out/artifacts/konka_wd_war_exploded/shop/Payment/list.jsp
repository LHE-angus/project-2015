...
<title>在线支付  - <c:if test="${ecUser.user_type eq 1 }">康佳EPP内部优惠购机平台</c:if><c:if test="${ecUser.user_type eq 2 }">康佳粉丝会</c:if></title>
...
<form action="<c:url value='/shop/Payment.do' />" method="post" id="pay_form" target="_blank">
  ...
  <table cellpadding="0" cellspacing="0" border="0" align="center" style="width:550px;margin-top:70px;">
  	 <tr>
  		...
  		<td width="30%"><p style="margin-top:10px;font-size:12px;">稍后支付&gt;&gt;<a href="${ctx }/shop/center/Orders.do?">查看我的订单</a></p></td>
	  </tr>
	  <tr ><td colspan="2"><hr/></td></tr>
	  <tr>
	  	 <td height="140" colspan="2">
	  	 
  			<div style="margin-left:10px;margin-top:15px;line-height:40px;">
  			<label class="bank bank--alipay btn_sub" data="2">支付宝</label> 
...
$(document).ready(function(){ 
	...
	$(document).delegate("#heckOrderStatus", "click", function(){
		location.href = "${ctx}/shop/Payment.do?method=checkOrderStatus&trade_index=${af.map.trade_index}"
	}); 
