  ...
  <div id="top_topt">
    <div class="top_top1">
      <div class="top_topr">欢迎您！
        <c:if test="${not empty shopUserInfo}">${shopUserInfo.real_name}&nbsp;|<a href="${ctx}/shop/center/Index.do">会员中心</a>|<a href="${ctx}/shop/login.do?method=logout">退出登陆</a></c:if>
        <c:if test="${empty shopUserInfo}">[<a id="top_login" style="cursor:pointer;">请登录</a>]</c:if>
        </div>
      <div class="top_topl">
        <h4><cite><img src="${ctx}/styles/shop/images/ico_2.gif" /></cite>&nbsp;&nbsp;<a href="<c:url value='/shop/manager/Index.do' />">欢迎光临康佳触网平台!</a></h4>
       
      </div>
      <div class="top_clear"></div>
    </div>
  </div>
</div><c:if test="${empty shopUserInfo}">
<div id="login" style="display:none;z-index:99999;">
  <form action="${ctx }/shop/login.do" method="post" style="margin:0;padding:0;">
...
$(document).ready(function(){
	...
	$(document).delegate("#top_login", "click", function(){
		// showLogin();
		location.href = "<c:url value='/shop/login.do' />";
	}); 
	//点击关闭按钮 
