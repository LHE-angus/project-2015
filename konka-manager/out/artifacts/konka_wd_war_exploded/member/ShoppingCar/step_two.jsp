...
.login_div{position: absolute;border: solid 1px #d1e3f5;text-align: center;border:1px solid #E03425;background: #f5f4f4;left:30%;width:550px;padding-bottom: 0px;padding-top: 0px;display: none;z-index:10002;} 
fieldset{text-align:left;padding:10px;margin-top:5px; background:#fff;}fieldset legend {color:#1E7ACE;font-weight:bold;padding:3px 20px 3px 20px;border:1px solid #E03425;background:#fff;}fieldset label {float:left;width:100px;text-align:right;padding:4px;margin:1px;}fieldset div {clear:left;margin-top:15px;margin-bottom:18px;}fieldset .buttom {width:44px; padding:1px 10px; margin-left:10px;font-size:12px;border:1px #1E7ACE solid;background:#D0F0FF;}
#login_bg_div {display: none; position: absolute; top: 0%; left: 0%; width: 100%; height: 100%; background-color: black; z-index:10000; -moz-opacity: 0.7; opacity:.70; filter: alpha(opacity=60);}fieldset p {line-height:24px;}	
</style>
...
<input type="hidden" name="tuangou" id="tuangou" value="${af.map.tuangou}" />
<input type="hidden" name="buy_json_object" value="${buy_json_object}" />
<c:if test="${af.map.tuangou eq 6 }"><input type="hidden" name="bill_is_add" value="0" id="bill_is_add" /></c:if>
<div class="card_center">
  <div class="card_a">我 的 购 物 车</div>
  <div class="card_b"> <div class="card_dd"><ul><li>1、我的购物车</li><li><b>2、确认订单信息</b></li><li>3、提交订单</li></ul></div>  </div>
  <div class="shoppingcart_c">
    <h3 class="TitleName"><b>友情提示</b></h3>
    <div class="HideBox"><div style="font-size:14px; padding:10px 0px;">此地址同时用于邮寄发票，请务必认真填写，发票在一个月内寄出。</div>    </div>
    ...
    <div class="HideBox" >
      <table border="0" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td align="left">
              <input name="is_zt" type="radio" value="0" id="is_zt_0" checked="checked" /> <label for="is_zt_0" style="cursor:pointer;">送货上门</label>
              <input name="is_zt" type="radio" value="1" id="is_zt_1"  /> <label for="is_zt_1" style="cursor:pointer;">自提</label>
            </td>
          </tr>
    </table>
    </div>
    <div id="self_div" style="margin-left:45px;display:none" >
        <c:if test="${empty esaList}"><span style="color: red;">  对不起，该区域顺丰没有自提点，请联系客服！</span></c:if> 
    	<c:forEach items="${esaList}" var="cur" varStatus="vs"><input id="self_way_${vs.count}" type="radio" name="self_way" value="${cur.id}"  style="margin-top:8px;"/>${cur.self_name} &nbsp;&nbsp;地址：${cur.self_addr}&nbsp;${cur.self_tel}<br/></c:forEach>
    </div>
    <h3 class="TitleName"><b>收货人信息</b><span><a style="cursor:pointer;display:${empty ecUserAddrs ? 'none' : ''}" id="rel_modify_btn">[修改]</a></span></h3>
   
    <c:forEach items="${ecUserAddrsList}" var="cur"><input type="radio" name="ecUserAddrsList" value="${cur.id }" onclick="ajaxAddr(${cur.id });"  ${ecUserAddrs.id eq cur.id ?'checked':'data=""'} /> <b>${cur.rel_name }</b>&nbsp; ${cur.map.full_name }&nbsp;${cur.rel_addr }&nbsp;${cur.rel_zip } 手机电话 ：${cur.rel_phone }&nbsp;${cur.rel_tel }    <br/></c:forEach>	
    <div class="HideBox"> 
        ...
        <tbody>
          ...
          <tr>
            ...
            <td><span id="p_name_txt">${p_name} </span> &nbsp;           
              ...
              <font id="showmsg" color="red"></font> <font ><strong>如果 所在地区不一致，请到商品页面重新选择配送区域</strong></font> 
              </td>
          </tr>
          <!--   
    ...
    <h3 class="TitleName"> <b>支付方式</b></h3>
    <div class="HideBox"><c:if test="${af.map.tuangou eq 6 }">  
     <table border="0" cellspacing="0" cellpadding="0"  >
        <tbody>
          <tr>
    ...
    </c:if><c:if test="${af.map.tuangou ne 6 }">  
            ...
            <td><ul>
                <table border="0"> 
                    <tr><c:if test="${empty is_dell}">  
                      ...
                      <td align="left" valign="middle"><label class="bank--bank_card" for="pay_way_6" style="cursor:pointer;">民生e支付 </label>&nbsp;&nbsp;<div style="margin-top:0px;width:550px;">&nbsp;&nbsp;即时到账，支持绝大数银行借记卡和部分银行信用卡&nbsp;&nbsp;<a href="${ctx}/member/KonkaGroupPeArticleInfo.do?method=view&id=837917"  target="_blank" style="color: #08C;">查看银行及限额</a></div></td>
                     </tr> 
                </table>
              </ul> 
              </td>
          </tr>
           <c:if test="${is_xsqg eq 0}">
           <tr>
                ...
                <table border="0">
                   <tr>
                      ...
                      <td align="left" valign="middle" colspan="5"><label for="pay_way_1" style="cursor:pointer;">户名：康佳集团股份有限公司
						帐号：744557963343
						开户行：中国银行深圳沙河支行   （银行转账时请备注：康佳epp内购平台购机款 ,收款人手机:18664553495）</label>
						</td>
                     </tr> 
                </table>
              </ul> 
    ...
    <div class="HideBox">
      <table border="0" cellspacing="0" cellpadding="0" >
        <tbody>
          <tr>
            <td class="item"><font color="red">*</font> 送 货 日 期： </td>
            <td><ul>
                <table border="0">
                  <tbody>
                    <tr>
          ...
          <tr>
            <td class="item">发货前电话确认： </td>
            <td><ul>
                <table border="0">
                  <tbody>
                    <tr>
                      <td><input id="deliver_is_call_1" type="radio" name="deliver_is_call" value="1"  checked="true"/>
                      ...
                      <td><input id="deliver_is_call_0" type="radio" name="deliver_is_call" value="0" />
                        <label for="deliver_is_call_0"  style="cursor:pointer;">否</label>
                         <c:if test="${not empty is_dell }">  由于戴尔电脑的直销模式，下单后立刻生产，预计7到15个工作日送货上门。（广东区域5天）</c:if>
          ...
          </tr><c:if test="${ecUser.user_type eq 1 && is_az eq 0 }">
          <tr>
            <td class="item">购买简易挂架：</td>
            <td><ul>
                <table border="0">
                  <tbody>
                    <tr>
                      <td><input id="jianyiguajia_1" type="radio" name="jianyiguajia" value="1"  /> <label for="jianyiguajia_1"  style="cursor:pointer;">是</label></td>
                      <td><input id="jianyiguajia_0" type="radio" name="jianyiguajia" value="0" checked="checked"/>
                        <label for="jianyiguajia_0"  style="cursor:pointer;">否 <span style="color: red;margin-left:10px; ">上门安装简易挂架需要另外收费，具体费用及简易挂架的价格请拨打400-880-0016咨询<a href="${ctx}/member/KonkaGroupPeArticleInfo.do?method=view&id=838940" target="_blank" style="color: #08C;">（参考费用）</a></span></label></td>  
                    </tr>
                  </tbody>
                </table></ul>
              </td>
           ...
           <tr>
            <td class="item">购买其他配件：</td>
            <td><ul>
                <table border="0">
                  <tbody>
                    <tr>
                      <td><input id="xuanzhuanguajia_1" type="radio" name="xuanzhuanguajia" value="1"  /> <label for="xuanzhuanguajia_1"  style="cursor:pointer;">是</label></td>
                      <td><input id="xuanzhuanguajia_0" type="radio" name="xuanzhuanguajia" value="0" checked="checked"/>
                        <label for="xuanzhuanguajia_0"  style="cursor:pointer;">否 <span style="color: red;margin-left:10px;">  请返回首页到配件专区选购配件，加入购物车，一起购买。
                        </span></label></td>  
                    </tr>
                  </tbody>
                </table></ul>
          ...
          <tr>
            <td class="item">是否需要调试：</td>
            <td>
                <table border="0">
                  <tbody>
                    <tr>
                      <td><input id="deliver_way_1" type="radio" name="deliver_way" value="1"  /> <label for="deliver_way_1"  style="cursor:pointer;">是</label></td>
                      <td><input id="deliver_way_0" type="radio" name="deliver_way" value="0" checked="checked"/>
                        <label for="deliver_way_0"  style="cursor:pointer;">否<span style="color: red;margin-left:10px;"> 目前第一次上门调试时免费，详情请咨询400-880-0016<a href="${ctx}/member/KonkaGroupPeArticleInfo.do?method=view&id=838940" target="_blank" style="color: #08C;">（参考费用）</a></span></label></td>  
                    </tr>
                  </tbody>
                </table>
              </td>
    ...
    </div><c:if test="${af.map.tuangou ne 6 }">  
    ...
    <div class="HideBox" >
      <table border="0" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td align="left">
              <input name="bill_is_add" type="radio" value="1" id="bill_is_add_1" /><label for="bill_is_add_1" style="cursor:pointer;">是</label>
              <input name="bill_is_add" type="radio" value="0" id="bill_is_add_0" /><label for="bill_is_add_0" style="cursor:pointer;">否</label>
              <c:if test="${not empty is_dell }"> 由于笔记本产品属于团购商品，发票抬头统一为“康佳集团”保修凭笔记本的保修卡即可</c:if>
            </td>
          </tr>
          ...
          <tr class="bill_top" style="display:none;">
            ...
            <td align="left"><input name="bill_head" type="radio" id="bill_head_0" value="0" checked="checked" /><label for="bill_head_0" style="cursor:pointer;">个人</label>
              <input type="radio" name="bill_head" value="1" id="bill_head_1" /><label for="bill_head_1" style="cursor:pointer;">单位 </label>
              &nbsp;发票与商品一同寄出，放在康佳信封中，贴在外包装箱顶部，请注意查找 
              </td>
          </tr>
      ...
      <div class="login_div" id="login_div" style="display:none;z-index:10000;position:absolute;">
		<fieldset>		
			...
			<div style="display: none" id="show_1">
				...
				<p>3、配送人员如果不提供开箱验货服务，请直接拒收，并联系售后或致电 0755-61368827，对配送方</p>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="mytable3">
      <tbody>
        <tr class="item">
         ...
         <c:if test="${cur.prod_type eq 21}"><c:set var="is_shiping" value="1"/></c:if>
	    ...
	    <tr>
	          ...
	          <td style="border-bottom:solid 1px #E6E6E6;">
	          	<a href="<c:url value='/member/PdShow.do?goods_id=${cur.id}' />" target="_blank"><c:if test="${fn:contains(cur.pd_name, 'HYUNDAI')}">HYUNDAI</c:if><c:if test="${fn:contains(cur.pd_name, '戴尔')}">戴尔</c:if>
              	<c:if test="${not fn:contains(cur.pd_name, 'HYUNDAI') and not fn:contains(cur.pd_name, '戴尔') and cur.prod_type eq 0}">康佳</c:if>&nbsp;${cur.pd_sn}</a>
              </td>
	          ...
	          <td style="border-bottom:solid 1px #E6E6E6;">
		          <c:if test="${cur.map.rule_price eq '0.00'}">无</c:if>
		      	  <c:if test="${cur.map.rule_price ne '0.00'}">￥<fmt:formatNumber value="${cur.map.rule_price}" pattern="#,#00" />/台 </c:if>
	          </td>
	          <td style="border-bottom:solid 1px #E6E6E6;">
		          <c:if test="${cur.map.rule_price_2 eq '0.00'}">无</c:if>
		      	  <c:if test="${cur.map.rule_price_2 ne '0.00'}">￥<fmt:formatNumber value="${cur.map.rule_price_2}" pattern="#,#00" />/台 </c:if>
	          </td>
	          <td valign="middle" style="text-align:left;border-bottom:solid 1px #E6E6E6;">
	          	 ...
	          	 <c:forEach items="${cur.ecBindingPdListForService}" var="cur1">
	          	 	<c:forEach items="${cur.serviceIds}" var="cur2"><c:if test="${cur1.id eq cur2}"> <c:set var="total_price" value="${total_price+ cur1.price*cur.map.buy_num}" />${cur1.goods_name} ￥<fmt:formatNumber value="${cur1.price}" pattern="#0.00" />*${cur.map.buy_num}<br /></c:if></c:forEach>
	          	 </c:forEach>
	          </td>
	     </tr></c:forEach>
      </tbody>
    </table>
    <c:if test="${is_xsqg eq 0 and empty is_shiping}">       
    <c:if test="${ecUser.user_type eq 1 and touch eq 1 and empty is_dell}">
    <c:if test="${is_rebate eq 1 }">
    <p style="margin-left:20px;margin-top:10px;line-height:24px;"><c:forEach items="${konkaBcompPdList}" var="cur"><c:if test="${not empty cur.ecBcompPdRebates}">
	     ...
	     <c:if test="${cur.ecBcompPdRebates.b_type eq 0}"><fmt:formatNumber value="${cur.ecBcompPdRebates.b_value*cur.map.price/100}" pattern="#0.00" /></c:if>
	     <c:if test="${cur.ecBcompPdRebates.b_type eq 1}"><fmt:formatNumber value="${cur.ecBcompPdRebates.b_value}" pattern="#0.00" /></c:if>
	     </c:if>
	     <c:if test="${empty cur.ecBcompPdRebates}">0</c:if></font>* ${cur.map.buy_num}<br/></c:if></c:forEach>
	             佣金合计 ：￥<font color="red"><fmt:formatNumber value="${all_rebates}" pattern="#,#00.00" /></font>     
    </p>
    <p style="margin-left:20px;margin-top:10px;color:#FF2200;font-weight:bold;line-height:24px;">     	
    	佣金抵扣：<input type="radio" name="is_deduction" id="is_deduction1" value="1" onclick="dedu(${all_rebates});"/><label onclick="dedu(${all_rebates});" for="is_deduction1" style="cursor:pointer;">使用佣金抵扣货款</label>
    	   <input type="radio" name="is_deduction" id="is_deduction0" value="0" onclick="dedu(0);"/><label onclick="dedu(0);" for="is_deduction0" style="cursor:pointer;">不使用佣金抵扣货款</label>
    </p></c:if>
    <p style="margin-left:20px;margin-top:10px;color:#FF2200;font-weight:bold;line-height:24px;">
  ...
  <table>
  	...
  	<td width="25%" align="center"><ul><li><a href="<c:url value='/member/ShoppingCar.do' />" id="step_one_go_shopping">返回购物车</a></li><li><a style="cursor:pointer;" id="step_two_btn">提交订单</a></li> </ul>
  	</td>
  	</tr>
  </table>
  </div>
...
function showDialog(){
	 if(is_v==0){
			...
			$("#val_div").html("<font color='red'>验证支付密码...</font>");
		  	var url="<c:url value='/member/ShoppingCar.do' />?method=checkPayPwd&time="+Math.random();  
		    ...
		    if(str==1){
				$("#val_div").html("<font color='red'>密码验证成功...正在获取购物券信息</font>");			  		  
			  	var ipt_html="<font color='red'>暂无购物券</font>";//加载购物券
			  	$.ajax({
					type: "POST",
					url: "<c:url value='/member/ShoppingCar.do' />",
