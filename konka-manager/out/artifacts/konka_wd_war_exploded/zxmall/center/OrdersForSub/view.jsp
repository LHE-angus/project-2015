...
<div class="maincont">
  ...
  <div class="zxmall_right padbot45">
    <div class="position"><a href="${ctx }/zxmall/Index.do">首页</a> &gt; <a href="${ctx }/zxmall/center/Index.do">会员中心</a> &gt; <a href="${ctx }/zxmall/center/Orders.do">我的订单</a> </div>
    <div class="zxmalltab3">
      <p style="margin-top:15px;font-size:16px;"> 我的订单</p>
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="zxmall_form_table0"  style="border:1px solid #e3e3e3;">
        <tr >
        ...
        <tr>
          <td colspan="2">订单状态：
            ...
            <c:if test="${af.map.pay_way ne 0 and  af.map.pay_way ne 1}">
             <c:if test="${af.map.state eq 0 }">&nbsp;&nbsp; &nbsp;&nbsp; <a class="btn btn-4" href="<c:url value='/zxmall/Payment.do?trade_index=${af.map.trade_index}' />"><s></s>付款</a></c:if>
             </c:if>
          </td>
        </tr>
      </table>
      ...
      <table id="tbl1" width="100%" border="0" cellpadding="0" cellspacing="0" class="zxmall_form_table0" >
      ...
      <table id="tbl2" width="100%" border="0" cellpadding="0" cellspacing="0" class="zxmall_form_table0" style="border:1px solid #e3e3e3;display:none;" >
        <tr>
          <td align="left">付款方式：
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="zxmall_form_table0" style="border:1px solid #e3e3e3;" >
        <tr>
        ...
        <tr >
          ...
          <p style="margin-top:1px;font-size:14px;color:#ff0000;font-weight:bold;">订单备注</p> 
        	${fn:escapeXml(af.map.logistic_sn)} <br/>
          </td>
        </tr>
      </table>
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="zxmall_form_table0" style="border:1px solid #e3e3e3;" >
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="zxmall_form_table0" style="border:1px solid #e3e3e3;" >
        ...
        <tr style="height:60px;">
          ...
          <c:forEach items="${pshowOrdeDetails}" var="cur">康佳${fn:escapeXml(cur.map.pd_sn)}  
	         <font color="#ff5300"><fmt:formatNumber value="${fn:escapeXml(cur.price)}" pattern="￥0.00" /> * ${cur.num}</font> 
	         <c:if test="${cur.rebates_status eq 4 }">- 佣金： <font color="#ff5300"><fmt:formatNumber value="${fn:escapeXml(cur.rebates)}" pattern="￥0.00" /></font> </c:if>
	         <c:forEach items="${ecBindingDetailList}" var="cur2">
            	 <c:if test="${cur.bill_item_id eq cur2.details_id }">
          ...
          <td width="50%" align="left" ><br/>
           ...
           - 优惠金额：<font color="#ff5300">
            <fmt:formatNumber value="${af.map.total_price-af.map.dedu_price-af.map.pay_price}" pattern="￥0.00元" />
            </font>    
...
$(document).ready(function(){  
  ...
  $("#syncBtn").click(function(){
		...
		$.ajax({
			type: "post",
			url: "${ctx}/zxmall/center/OrdersForSub.do?method=sfList",
			data: {"id" : id},
			dataType: "json",
